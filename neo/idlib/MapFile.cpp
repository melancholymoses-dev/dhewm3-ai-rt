...
bool idMapPatch::Write(idFile *fp, int primitiveNum, const idVec3 &origin) const
{
    ...
    if (GetExplicitlySubdivided())
    {
        ...
        fp->WriteFloatString("  \"%s\"\n  ( %d %d %d %d 0 0 0 )\n", GetMaterial(), GetWidth(), GetHeight(),
                             GetHorzSubdivisions(), GetVertSubdivisions());
    }
    else
    {
        fp->WriteFloatString("// primitive %d\n{\n patchDef2\n {\n", primitiveNum);
    ...
    for (i = 0; i < GetWidth(); i++)
    {
        ...
        for (j = 0; j < GetHeight(); j++)
        {
            ...
            fp->WriteFloatString(" ( %f %f %f %f %f )", v->xyz[0] + origin[0], v->xyz[1] + origin[1],
                                 v->xyz[2] + origin[2], v->st[0], v->st[1]);
        }
        fp->WriteFloatString(" )\n");
    }
    fp->WriteFloatString("  )\n }\n}\n");
...
idMapBrush *idMapBrush::Parse(idLexer &src, const idVec3 &origin, bool newFormat, float version)
{
    ...
    do
    {
        ...
        else
        {
            ...
            if (!src.Parse1DMatrix(3, planepts[0].ToFloatPtr()) || !src.Parse1DMatrix(3, planepts[1].ToFloatPtr()) ||
...
bool idMapBrush::Write(idFile *fp, int primitiveNum, const idVec3 &origin) const
{
    ...
    for (i = 0; i < GetNumSides(); i++)
    {
        ...
        fp->WriteFloatString("( ( %f %f %f ) ( %f %f %f ) ) \"%s\" 0 0 0\n", side->texMat[0][0], side->texMat[0][1],
                             side->texMat[0][2], side->texMat[1][0], side->texMat[1][1], side->texMat[1][2],
                             side->material.c_str());
    }
...
idMapEntity *idMapEntity::Parse(idLexer &src, bool worldSpawn, float version)
{
    ...
    do
    {
        ...
        if (token == "{")
        {
            ...
            {
                mapBrush =
                    idMapBrush::Parse(src, origin, (!token.Icmp("brushDef2") || !token.Icmp("brushDef3")), version);
                if (!mapBrush)
                {
                    return NULL;
                }
