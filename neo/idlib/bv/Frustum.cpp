...
static int boxVertPlanes[8] = {
    ((1 << 0) | (1 << 2) | (1 << 4)), ((1 << 1) | (1 << 2) | (1 << 4)), ((1 << 1) | (1 << 3) | (1 << 4)),
    ((1 << 0) | (1 << 3) | (1 << 4)), ((1 << 0) | (1 << 2) | (1 << 5)), ((1 << 1) | (1 << 2) | (1 << 5)),
...
bool idFrustum::CullLocalBox(const idVec3 &localOrigin, const idVec3 &extents, const idMat3 &localAxis) const
{
    ...
    d2 = idMath::Fabs(extents[0] * localAxis[0][0]) + idMath::Fabs(extents[1] * localAxis[1][0]) +
...
============
*/
bool idFrustum::CullLocalFrustum(const idFrustum &localFrustum, const idVec3 indexPoints[8],
...
============
*/
bool idFrustum::BoundsCullLocalFrustum(const idBounds &bounds, const idFrustum &localFrustum,
                                       const idVec3 indexPoints[8], const idVec3 cornerVecs[4]) const
{
    int index;
    float dx, dy, dz;
...
bool idFrustum::FromProjection(const idBox &box, const idVec3 &projectionOrigin, const float dFar)
{
    ...
    for (j = 0; j < 2; j++)
    {
        ...
        BoxToPoints((box.GetCenter() - projectionOrigin) * axis.Transpose(), box.GetExtents(),
                    box.GetAxis() * axis.Transpose(), points);
        ...
        if (j == 0)
        {
            dir += idMath::Tan16(0.5f * (idMath::ATan16(points[minY].y, points[minY].x) +
                                         idMath::ATan16(points[maxY].y, points[maxY].x))) *
                   axis[1];
            dir += idMath::Tan16(0.5f * (idMath::ATan16(points[minZ].z, points[minZ].x) +
    ...
    this->dFar = dFar;
    this->dLeft =
        Max(idMath::Fabs(points[minY].y / points[minY].x), idMath::Fabs(points[maxY].y / points[maxY].x)) * dFar;
    this->dUp =
        Max(idMath::Fabs(points[minZ].z / points[minZ].x), idMath::Fabs(points[maxZ].z / points[maxZ].x)) * dFar;
...
============
*/
void idFrustum::AxisProjection(const idVec3 indexPoints[8], const idVec3 cornerVecs[4], const idVec3 &dir, float &min,
                               float &max) const
{
    float dx, dy, dz;
    int index;
...
============
*/
void idFrustum::AddLocalLineToProjectionBoundsSetCull(const idVec3 &start, const idVec3 &end, int &startCull,
                                                      int &endCull, idBounds &bounds) const
{
    idVec3 dir, p;
...
#ifdef FRUSTUM_DEBUG
    static idCVar r_showInteractionScissors("r_showInteractionScissors", "0", CVAR_RENDERER | CVAR_INTEGER, "", 0, 2,
...
============
*/
void idFrustum::AddLocalLineToProjectionBoundsUseCull(const idVec3 &start, const idVec3 &end, int startCull,
                                                      int endCull, idBounds &bounds) const
{
    idVec3 dir, p;
    float d1, d2, fstart, fend, lstart, lend, f;
...
============
*/
bool idFrustum::BoundsRayIntersection(const idBounds &bounds, const idVec3 &start, const idVec3 &dir, float &scale1,
                                      float &scale2) const
{
    idVec3 end, p;
    float d1, d2, f;
...
bool idFrustum::ProjectionBounds(const idBox &box, idBounds &projectionBounds) const
{
    ...
    if (outside != 1 && outside != 2 && outside != 4 && outside != 8)
    {
        ...
        {
            BoundsRayIntersection(bounds, localOrigin, localScaled[0] - localScaled[1] - localScaled[2], scale1,
                                  scale2);
...
bool idFrustum::ProjectionBounds(const idFrustum &frustum, idBounds &projectionBounds) const
{
    ...
    if (localFrustum.dNear > 0.0f)
    {
        for (i = 0; i < 4; i++)
        {
            p1 = i;
            p2 = (i + 1) & 3;
            AddLocalLineToProjectionBoundsUseCull(points[p1], points[p2], pointCull[p1], pointCull[p2],
                                                  projectionBounds);
        }
    }
...
bool idFrustum::ProjectionBounds(const idWinding &winding, idBounds &projectionBounds) const
{
    ...
    for (i = 0; i < winding.GetNumPoints(); i += 2)
    {
        ...
        AddLocalLineToProjectionBoundsSetCull(localPoints[p1], localPoints[p2], pointCull[p1], pointCull[p2],
                                              projectionBounds);
        culled &= pointCull[p1] & pointCull[p2];
        outside |= pointCull[p1] | pointCull[p2];
    }
    ...
    for (i = 1; i < winding.GetNumPoints(); i += 2)
    {
        p1 = i;
        p2 = (i + 1) % winding.GetNumPoints();
        AddLocalLineToProjectionBoundsUseCull(localPoints[p1], localPoints[p2], pointCull[p1], pointCull[p2],
                                              projectionBounds);
    }
...
============
*/
bool idFrustum::ClipLine(const idVec3 localPoints[8], const idVec3 points[8], int startIndex, int endIndex,
                         idVec3 &start, idVec3 &end, int &startClip, int &endClip) const
{
    float d1, d2, fstart, fend, lstart, lend, f, x;
    float leftScale, upScale;
    float scale1, scale2;
...
ID_INLINE bool idFrustum::AddLocalCapsToProjectionBounds(const idVec3 endPoints[4], const int endPointCull[4],
                                                         const idVec3 &point, int pointCull, int pointClip,
...
============
*/
bool idFrustum::ClippedProjectionBounds(const idFrustum &frustum, const idBox &clipBox,
                                        idBounds &projectionBounds) const
{
    ...
    for (i = 0; i < 4; i++)
    {
        p1 = i;
        p2 = 4 + i;
        AddLocalLineToProjectionBoundsSetCull(clipPoints[p1], clipPoints[p2], clipPointCull[p1], clipPointCull[p2],
                                              projectionBounds);
    }

    // get cull bits for the clipped frustum
    outside = clipPointCull[0] | clipPointCull[1] | clipPointCull[2] | clipPointCull[3] | clipPointCull[4] |
              clipPointCull[5] | clipPointCull[6] | clipPointCull[7];
    ...
    if (outside)
    {
        ...
        if (!nearCull && localFrustum.dNear > 0.0f)
        {
            for (i = 0; i < 4; i++)
            {
                p1 = i;
                p2 = (i + 1) & 3;
                AddLocalLineToProjectionBoundsUseCull(clipPoints[p1], clipPoints[p2], clipPointCull[p1],
        ...
        for (i = 0; i < 4; i++)
        {
            ...
            if (!(boxPointCull[p1] & boxPointCull[p2]))
            {
                if (frustum.ClipLine(localPoints1, localPoints2, p1, p2, start, end, startClip, endClip))
                {
                    ...
                    AddLocalCapsToProjectionBounds(clipPoints + 4, clipPointCull + 4, start, pointCull[0], startClip,
                                                   projectionBounds);
                    AddLocalCapsToProjectionBounds(clipPoints + 4, clipPointCull + 4, end, pointCull[1], endClip,
                                                   projectionBounds);
