...
{
    idVec3 lp = p - center;
    if (idMath::Fabs(lp * axis[0]) > extents[0] || idMath::Fabs(lp * axis[1]) > extents[1] ||
...
{
    float d1 = dir * center;
    float d2 = idMath::Fabs(extents[0] * (dir * axis[0])) + idMath::Fabs(extents[1] * (dir * axis[1])) +
...
ID_INLINE void idBox::AxisProjection(const idMat3 &ax, idBounds &bounds) const
{
    for (int i = 0; i < 3; i++)
    {
        ...
        float d2 = idMath::Fabs(extents[0] * (ax[i] * axis[0])) + idMath::Fabs(extents[1] * (ax[i] * axis[1])) +
