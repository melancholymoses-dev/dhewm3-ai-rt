...
================
*/
int idCollisionModelManagerLocal::RotateEdgeThroughEdge(cm_traceWork_t *tw, const idPluecker &pl1, const idVec3 &vc,
                                                        const idVec3 &vd, const float minTan, float &tanHalfAngle)
{
    ...
    v[0] = (a[0] * cos(t) + a[1] * sin(t)) * (b[0] * -sin(t) + b[1] * cos(t)) - (b[0] * cos(t) + b[1] * sin(t)) * (a[0]
    * -sin(t) + a[1] * cos(t)); v[1] = (a[0] * cos(t) + a[1] * sin(t)) * b[2] - (b[0] * cos(t) + b[1] * sin(t)) * a[2];
    ...
    v[0] = (a[0] * cos(t) + a[1] * sin(t)) * (b[0] * -sin(t) + b[1] * cos(t)) - (b[0] * cos(t) + b[1] * sin(t)) * (a[0]
    * -sin(t) + a[1] * cos(t)); v[0] = (a[1] * b[1] - a[0] * b[0]) * cos(t) * sin(t) + (a[0] * b[1] + a[1] * b[0] *
    cos(t)^2) - (a[1] * b[0]) - ((b[1] * a[1] - b[0] * a[0]) * cos(t) * sin(t) + (b[0] * a[1] + b[1] * a[0]) * cos(t)^2
    - (b[1] * a[0])) v[0] = - (a[1] * b[0]) - ( - (b[1] * a[0])) v[0] = (b[1] * a[0]) - (a[1] * b[0])
...
================
*/
int idCollisionModelManagerLocal::EdgeFurthestFromEdge(cm_traceWork_t *tw, const idPluecker &pl1, const idVec3 &vc,
...
================
*/
void idCollisionModelManagerLocal::RotateTrmEdgeThroughPolygon(cm_traceWork_t *tw, cm_polygon_t *poly,
    ...
    for (i = 0; i < poly->numEdges; i++)
    {
        ...
        if ((f1 < 0.0f && f2 > 0.0f) || (f1 > 0.0f && f2 < 0.0f))
        {
            ...
            else
            {
                ...
                if (!RotateEdgeThroughEdge(tw, trmEdge->plzaxis, v1->p + epsDir, v2->p + epsDir, idMath::Fabs(startTan),
                                           tanHalfAngle))
                {
        ...
        if (!CollisionBetweenEdgeBounds(tw, trmEdge->start, trmEdge->end, v1->p, v2->p, tanHalfAngle, collisionPoint,
                                        collisionNormal))
        {
            continue;
        }
...
================
*/
int idCollisionModelManagerLocal::RotatePointThroughPlane(const cm_traceWork_t *tw, const idVec3 &point,
                                                          const idPlane &plane, const float angle, const float minTan,
                                                          float &tanHalfAngle)
{
    double v0, v1, v2, a, b, c, d, sqrtd, q, frac1, frac2;
    idVec3 p, normal;
...
================
*/
int idCollisionModelManagerLocal::PointFurthestFromPlane(const cm_traceWork_t *tw, const idVec3 &point,
                                                         const idPlane &plane, const float angle, float &tanHalfAngle,
                                                         float &dir)
{
...
================
*/
int idCollisionModelManagerLocal::RotatePointThroughEpsilonPlane(const cm_traceWork_t *tw, const idVec3 &point,
                                                                 const idVec3 &endPoint, const idPlane &plane,
                                                                 const float angle, const idVec3 &origin,
...
================
*/
void idCollisionModelManagerLocal::RotateTrmVertexThroughPolygon(cm_traceWork_t *tw, cm_polygon_t *poly,
                                                                 cm_trmVertex_t *v, int vertexNum)
{
    ...
    if (!RotatePointThroughEpsilonPlane(tw, v->p, v->endp, poly->plane, tw->angle, v->rotationOrigin, tanHalfAngle,
...
================
*/
void idCollisionModelManagerLocal::RotateVertexThroughTrmPolygon(cm_traceWork_t *tw, cm_trmPolygon_t *trmpoly,
                                                                 ...
                                                                 idVec3 &rotationOrigin)
{
    ...
    // if the polygon vertex is behind the trm plane it cannot collide with the trm polygon within a 180 degrees
    // rotation
    ...
    // rotate the vertex through the epsilon plane
    if (!RotatePointThroughEpsilonPlane(tw, v->p, endp, trmpoly->plane, -tw->angle, rotationOrigin, tanHalfAngle,
                                        collisionPoint, endDir))
    {
        return;
    }
...
================
*/
void idCollisionModelManagerLocal::BoundsForRotation(const idVec3 &origin, const idVec3 &axis, const idVec3 &start,
                                                     const idVec3 &end, idBounds &bounds)
{
    int i;
    float radiusSqr;
    idVec3 v1, v2;
                                               ...
                                               cmHandle_t model, const idVec3 &modelOrigin, const idMat3 &modelAxis)
{
    ...
    tw.angle =
        idMath::ClampFloat(-180.0f, 180.0f, tw.angle); // DG: enforce it for the rare cases the assert would trigger
    ...
    if (!trm || (trm->bounds[1][0] - trm->bounds[0][0] <= 0.0f && trm->bounds[1][1] - trm->bounds[0][1] <= 0.0f &&
                                            ...
                                            cmHandle_t model, const idVec3 &modelOrigin, const idMat3 &modelAxis)
{
    ...
    {
        idCollisionModelManagerLocal::ContentsTrm(results, start, trm, trmAxis, contentMask, model, modelOrigin,
                                                  modelAxis);
        return;
    }
    ...
    if (rotation.GetAngle() >= 180.0f || rotation.GetAngle() <= -180.0f)
    {
        ...
        {
            // partial rotation
            idCollisionModelManagerLocal::Rotation180(results, rotation.GetOrigin(), rotation.GetVec(), lasta, a, start,
    ...
    idCollisionModelManagerLocal::Rotation180(results, rotation.GetOrigin(), rotation.GetVec(), 0.0f,
    ...
    if (cm_debugCollision.GetBool())
    {
        ...
        if (idCollisionModelManagerLocal::Contents(results->endpos, trm, results->endAxis, -1, model, modelOrigin,
                                                   modelAxis) &
            contentMask)
        {
            trace_t tr;
            ...
            idCollisionModelManagerLocal::Contents(results->endpos, trm, results->endAxis, -1, model, modelOrigin,
                                                   modelAxis);
            // re-run collision detection to find out where it failed
            idCollisionModelManagerLocal::Rotation(&tr, start, rotation, trm, trmAxis, contentMask, model, modelOrigin,
                                                   modelAxis);
        }
    }
#endif
