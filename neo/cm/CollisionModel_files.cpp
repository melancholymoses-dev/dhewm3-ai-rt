...
void idCollisionModelManagerLocal::WritePolygons(idFile *fp, cm_node_t *node)
{
    ...
    for (pref = node->polygons; pref; pref = pref->next)
    {
        ...
        fp->WriteFloatString(" ) ( %f %f %f ) %f", p->plane.Normal()[0], p->plane.Normal()[1], p->plane.Normal()[2],
                             p->plane.Dist());
...
void idCollisionModelManagerLocal::WriteBrushes(idFile *fp, cm_node_t *node)
{
    ...
    for (bref = node->brushes; bref; bref = bref->next)
    {
        ...
        {
            fp->WriteFloatString("\t\t( %f %f %f ) %f\n", b->planes[i].Normal()[0], b->planes[i].Normal()[1],
                                 b->planes[i].Normal()[2], b->planes[i].Dist());
        }
...
void idCollisionModelManagerLocal::WriteCollisionModel(idFile *fp, cm_model_t *model)
{
    ...
    {
        fp->WriteFloatString("\t/* %d */ ( %f %f %f )\n", i, model->vertices[i].p[0], model->vertices[i].p[1],
    ...
    {
        fp->WriteFloatString("\t/* %d */ ( %d %d ) %d %d\n", i, model->edges[i].vertexNum[0],
                             model->edges[i].vertexNum[1], model->edges[i].internal, model->edges[i].numUsers);
    }
    fp->WriteFloatString("\t}\n");
    // nodes
...
================
*/
void idCollisionModelManagerLocal::WriteCollisionModelsToFile(const char *filename, int firstModel, int lastModel,
                                                              unsigned int mapFileCRC)
{
    ...
    if (!fp)
    {
        common->Warning("idCollisionModelManagerLocal::WriteCollisionModelsToFile: "
...
================
*/
bool idCollisionModelManagerLocal::WriteCollisionModelForMapEntity(const idMapEntity *mapEnt, const char *filename,
                                                                   const bool testTraceModel)
{
    ...
    if (!fp)
    {
        common->Printf("idCollisionModelManagerLocal::"
                       "WriteCollisionModelForMapEntity: Error opening file %s\n",
                       name.c_str());
        FreeModel(model);
        return false;
    }
...
bool idCollisionModelManagerLocal::LoadCollisionModelFile(const char *name, unsigned int mapFileCRC)
{
    ...
    while (1)
    {
        ...
        src->Error("idCollisionModelManagerLocal::LoadCollisionModelFile: bad "
                   "token \"%s\"",
                   token.c_str());
    }
