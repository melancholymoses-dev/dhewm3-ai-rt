...
idCVar cm_drawMask("cm_drawMask", "none", CVAR_GAME, "collision mask", cm_contentsNameByIndex,
...
const char *idCollisionModelManagerLocal::StringFromContents(const int contents) const
{
    ...
    for (i = 1; cm_contentsFlagByIndex[i] != 0; i++)
    {
        if (contents & cm_contentsFlagByIndex[i])
        {
            ...
            length += idStr::snPrintf(contentsString + length, sizeof(contentsString) - length, "%s",
...
================
*/
void idCollisionModelManagerLocal::DrawPolygon(cm_model_t *model, cm_polygon_t *p, const idVec3 &origin,
...
================
*/
void idCollisionModelManagerLocal::DrawNodePolygons(cm_model_t *model, cm_node_t *node, const idVec3 &origin,
...
void idCollisionModelManagerLocal::DebugOutput(const idVec3 &origin)
{
    ...
    sscanf(cm_testBox.GetString(), "%f %f %f %f %f %f", &bounds[0][0], &bounds[0][1], &bounds[0][2], &bounds[1][0],
    ...
    {
        Translation(&trace, start, testend[i], &itm, boxAxis, CONTENTS_SOLID | CONTENTS_PLAYERCLIP,
                    cm_testModel.GetInteger(), vec3_origin, modelAxis);
    }
    timer.Stop();
    ...
    common->Printf("%s translations: %4u milliseconds, (min = %u, max = %u, av = %1.1f)\n", buf, t, min_translation,
                   max_translation, (float)total_translation / num_translation);
    ...
    if (cm_testRotation.GetBool())
    {
        ...
        for (i = 0; i < cm_testTimes.GetInteger(); i++)
        {
            ...
            Rotation(&trace, start, rotation, &itm, boxAxis, CONTENTS_SOLID | CONTENTS_PLAYERCLIP,
                     cm_testModel.GetInteger(), vec3_origin, modelAxis);
        }
        timer.Stop();
        t = timer.Milliseconds();
        ...
        common->Printf("%s rotation: %4d milliseconds, (min = %d, max = %d, av = %1.1f)\n", buf, t, min_rotation,
                       max_rotation, (float)total_rotation / num_rotation);
    }
