...
cm_model_t *idCollisionModelManagerLocal::AllocModel(void)
{
    ...
    model->numPolygons = model->polygonMemory = model->numBrushes = model->brushMemory = model->numNodes =
        model->numBrushRefs = model->numPolygonRefs = model->numInternalEdges = model->numSharpEdges =
            model->numRemovedPolys = model->numMergedPolys = model->usedMemory = 0;
...
cm_polygonRef_t *idCollisionModelManagerLocal::AllocPolygonReference(cm_model_t *model, int blockSize)
{
    ...
    {
        prefBlock =
            (cm_polygonRefBlock_t *)Mem_Alloc(sizeof(cm_polygonRefBlock_t) + blockSize * sizeof(cm_polygonRef_t));
...
void idCollisionModelManagerLocal::AddPolygonToNode(cm_model_t *model, cm_node_t *node, cm_polygon_t *p)
{
    ...
    pref =
        AllocPolygonReference(model, model->numPolygonRefs < REFERENCE_BLOCK_SIZE_SMALL ? REFERENCE_BLOCK_SIZE_SMALL
...
void idCollisionModelManagerLocal::AddBrushToNode(cm_model_t *model, cm_node_t *node, cm_brush_t *b)
{
    ...
    bref = AllocBrushReference(model, model->numBrushRefs < REFERENCE_BLOCK_SIZE_SMALL ? REFERENCE_BLOCK_SIZE_SMALL
                                                                                       : REFERENCE_BLOCK_SIZE_LARGE);
    bref->b = b;
...
cm_polygon_t *idCollisionModelManagerLocal::TryMergePolygons(cm_model_t *model, cm_polygon_t *p1, cm_polygon_t *p2)
{
    ...
    if (!keep2)
    {
        ...
        GetEdge(model, model->vertices[model->edges[abs(edgeNum1)].vertexNum[INTSIGNBITSET(edgeNum1)]].p,
                model->vertices[model->edges[abs(edgeNum2)].vertexNum[INTSIGNBITNOTSET(edgeNum2)]].p, &newEdgeNum2, -1);
        if (newEdgeNum2 == 0)
        {
            keep2 = true;
        }
...
int idCollisionModelManagerLocal::GetVertex(cm_model_t *model, const idVec3 &v, int *vertexNum)
{
    ...
    for (vn = cm_vertexHash->First(hashKey); vn >= 0; vn = cm_vertexHash->Next(vn))
    {
        ...
        if (idMath::Fabs(vert[2] - (*p)[2]) < VERTEX_EPSILON && idMath::Fabs(vert[0] - (*p)[0]) < VERTEX_EPSILON &&
