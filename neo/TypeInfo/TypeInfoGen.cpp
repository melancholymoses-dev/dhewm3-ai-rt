...
================
*/
idClassTypeInfo *idTypeInfoGen::ParseClassType(const char *scope, const char *templateArgs, bool isTemplate,
                                               bool typeDef, idParser &src)
{
    idToken token;
    ...
    if (src.CheckTokenString(":"))
    {
        ...
        while (token == "public" || token == "protected" || token == "private")
        {
            ...
            src.Warning("multiple inheritance not supported for '%s%s'", typeInfo->scope.c_str(),
...
void idTypeInfoGen::WriteTypeInfo(const char *fileName) const
{
    ...
    for (i = 0; i < classes.Num(); i++)
    {
        ...
        file->WriteFloatString("\t{ \"%s\", \"%s\", sizeof(%s), %s_typeInfo },\n", typeName.c_str(),
                               info->superType.c_str(), typeName.c_str(), typeInfoName.c_str());
    }
