...
================
*/
idAFConstraint_BallAndSocketJoint::idAFConstraint_BallAndSocketJoint(const idStr &name, idAFBody *body1,
                                                                     idAFBody *body2)
{
    assert(body1);
...
================
*/
void idAFConstraint_BallAndSocketJoint::SetConeLimit(const idVec3 &coneAxis, const float coneAngle,
                                                     const idVec3 &body1Axis)
{
    ...
    if (body2)
    {
        coneLimit->Setup(body1, body2, anchor2, coneAxis * body2->GetWorldAxis().Transpose(), coneAngle,
...
void idAFConstraint_BallAndSocketJoint::Evaluate(float invTimeStep)
{
    ...
    if (master)
    {
        ...
        c1.SubVec3(0) =
            -(invTimeStep * ERROR_REDUCTION) * (a2 + master->GetWorldOrigin() - (a1 + body1->GetWorldOrigin()));
    }
    else
    {
        a2.Zero();
...
void idAFConstraint_Hinge::SetLimit(const idVec3 &axis, const float angle, const idVec3 &body1Axis)
{
    ...
    if (body2)
    {
        coneLimit->Setup(body1, body2, anchor2, axis * body2->GetWorldAxis().Transpose(), angle,
                         body1Axis * body1->GetWorldAxis().Transpose());
    }
    else
    {
...
void idAFConstraint_Hinge::Evaluate(float invTimeStep)
{
    ...
    J1.Set(mat3_identity, -SkewSymmetric(a1), mat3_zero,
           idMat3(vecX[0], vecX[1], vecX[2], vecY[0], vecY[1], vecY[2], 0.0f, 0.0f, 0.0f));
    J1.SetSize(5, 6);
...
================
*/
void idAFConstraint_Spring::SetSpring(const float stretch, const float compress, const float damping,
...
void idAFConstraint_Contact::ApplyFriction(float invTimeStep)
{
    ...
    if (af_useImpulseFriction.GetBool())
    {
        ...
        forceDenominator =
            body1->GetInverseMass() + ((body1->GetInverseWorldInertia() * r.Cross(normal)).Cross(r) * normal);
...
================
*/
void idAFConstraint_ConeLimit::Setup(idAFBody *b1, idAFBody *b2, const idVec3 &coneAnchor, const idVec3 &coneAxis,
...
void idAFConstraint_PyramidLimit::Setup(idAFBody *b1, idAFBody *b2, const idVec3 &pyramidAnchor,
                                        const idVec3 &pyramidAxis, const idVec3 &baseAxis, const float pyramidAngle1,
                                        const float pyramidAngle2, const idVec3 &body1Axis)
{
    body1 = b1;
    body2 = b2;
...
================
*/
void idAFConstraint_Suspension::Setup(const char *name, idAFBody *body, const idVec3 &origin, const idMat3 &axis,
...
================
*/
void idAFConstraint_Suspension::SetSuspension(const float up, const float down, const float k, const float d,
                                              const float f)
{
    suspensionUp = up;
    suspensionDown = down;
...
void idAFConstraint_Suspension::Evaluate(float invTimeStep)
{
    ...
    c1[0] = 0.0f;
    velocity =
        J1.SubVec6(0).SubVec3(0) * body1->GetLinearVelocity() + J1.SubVec6(0).SubVec3(1) * body1->GetAngularVelocity();
...
void idPhysics_AF::EvaluateBodies(float timeStep)
{
    ...
    for (i = 0; i < bodies.Num(); i++)
    {
        ...
        if (body->centerOfMass.Compare(vec3_origin, CENTER_OF_MASS_EPSILON))
        {
            ...
            body->inverseWorldSpatialInertia.Set(body->invMass * mat3_identity, mat3_zero, mat3_zero,
        ...
        else
        {
            ...
            // spatial inertia in world space
            body->I.Set(body->mass * mat3_identity, massMoment, massMoment.Transpose(),
                        axis * body->inertiaTensor * axis.Transpose());
...
void idPhysics_AF::ApplyFriction(float timeStep, float endTimeMSec)
{
    ...
    if (jointFrictionDentStart < MS2SEC(endTimeMSec) && jointFrictionDentEnd > MS2SEC(endTimeMSec))
    {
        ...
        {
            jointFrictionDentScale = jointFrictionDent + (1.0f - jointFrictionDent) *
    ...
    if (contactFrictionDentStart < MS2SEC(endTimeMSec) && contactFrictionDentEnd > MS2SEC(endTimeMSec))
    {
        ...
        else
        {
            contactFrictionDentScale =
                contactFrictionDent +
                (1.0f - contactFrictionDent) * (MS2SEC(endTimeMSec) - contactFrictionDentStart - halfTime) / halfTime;
        }
    }
    else
    {
    ...
    for (k = 0, i = 0; i < auxiliaryConstraints.Num(); i++)
    {
        ...
        for (j = 0; j < constraint->J1.GetNumRows(); j++, k++)
        {
            ...
            if (constraint->body2)
            {
                ...
                {
                    dstPtr[m] += j2[0] * ptr[0] + j2[1] * ptr[1] + j2[2] * ptr[2] + j2[3] * ptr[3] + j2[4] * ptr[4] +
                                 j2[5] * ptr[5];
                    ptr += 8;
                }
            }
    ...
    for (k = 0, i = 0; i < auxiliaryConstraints.Num(); i++)
    {
        ...
        for (j = 0; j < constraint->J1.GetNumRows(); j++, k++)
        {
            ...
            if (constraint->body2)
            {
                ...
                rhs[k] +=
                    j2[0] * ptr[0] + j2[1] * ptr[1] + j2[2] * ptr[2] + j2[3] * ptr[3] + j2[4] * ptr[4] + j2[5] * ptr[5];
...
bool idPhysics_AF::CollisionImpulse(float timeStep, idAFBody *body, trace_t &collision)
{
    ...
    inverseWorldInertiaTensor =
        body->current->worldAxis.Transpose() * body->inverseInertiaTensor * body->current->worldAxis;
    impulseNumerator = -(1.0f + body->bouncyness) * (velocity * collision.c.normal);
    impulseDenominator =
        body->invMass + ((inverseWorldInertiaTensor * r.Cross(collision.c.normal)).Cross(r) * collision.c.normal);
    if (info.invMass)
    {
        impulseDenominator +=
            info.invMass + ((info.invInertiaTensor * info.position.Cross(collision.c.normal)).Cross(info.position) *
                            collision.c.normal);
    }
...
void idPhysics_AF::CheckForCollisions(float timeStep)
{
    ...
    for (i = 0; i < bodies.Num(); i++)
    {
        ...
        if (body->clipMask != 0)
        {
            ...
            if (gameLocal.clip.Contents(body->current->worldOrigin, body->clipModel, body->current->worldAxis,
...
#ifdef TEST_COLLISION_DETECTION
            if (gameLocal.clip.Contents(body->next->worldOrigin, body->clipModel, body->next->worldAxis, body->clipMask,
                                        passEntity))
            {
                if (!startsolid)
                {
...
bool idPhysics_AF::EvaluateContacts(void)
{
    ...
    for (i = 0; i < bodies.Num(); i++)
    {
        ...
        numContacts = gameLocal.clip.Contacts(contactInfo, 10, body->current->worldOrigin, dir.SubVec6(0),
...
================
*/
void idPhysics_AF::SetSuspendTolerance(const float noMoveTime, const float noMoveTranslation,
...
const idBounds &idPhysics_AF::GetBounds(int id) const
{
    ...
    else
    {
        ...
        for (i = 1; i < bodies.Num(); i++)
        {
            idBounds bounds;
            idVec3 origin =
                (bodies[i]->GetWorldOrigin() - bodies[0]->GetWorldOrigin()) * bodies[0]->GetWorldAxis().Transpose();
...
bool idPhysics_AF::Evaluate(int timeStepMSec, int endTimeMSec)
{
    ...
    {
        timeStep =
            MS2SEC(timeStepMSec) * (MS2SEC(endTimeMSec) - timeScaleRampStart) / (timeScaleRampEnd - timeScaleRampStart);
    }
    ...
    if (masterBody)
    {
        ...
        if (current.atRest >= 0 &&
            (masterBody->current->worldOrigin != masterOrigin || masterBody->current->worldAxis != masterAxis))
        {
            Activate();
        }
    ...
    if (af_showTimings.GetInteger() == 1)
    {
        gameLocal.Printf("%12s: t %u pc %2d, %u ac %2d %u lcp %u cd %u\n", self->name.c_str(),
                         timer_total.Milliseconds(), numPrimary, timer_pc.Milliseconds(), numAuxiliary,
                         timer_ac.Milliseconds() - timer_lcp.Milliseconds(), timer_lcp.Milliseconds(),
...
void idPhysics_AF::DebugDraw(void)
{
    ...
    if (af_highlightConstraint.GetString()[0])
    {
        ...
        if (constraint)
        {
            ...
            if (af_showConstrainedBodies.GetBool())
            {
                ...
                if (constrainedBody1)
                {
                    collisionModelManager->DrawModel(constrainedBody1->clipModel->Handle(),
                ...
                if (constrainedBody2)
                {
                    collisionModelManager->DrawModel(constrainedBody2->clipModel->Handle(),
    ...
    if (af_showBodyNames.GetBool())
    {
        ...
        {
            body = bodies[i];
            gameRenderWorld->DrawText(body->GetName().c_str(), body->GetWorldOrigin(), 0.08f, colorCyan,
    ...
    if (af_showMass.GetBool())
    {
        ...
        {
            body = bodies[i];
            gameRenderWorld->DrawText(va("\n%1.2f", 1.0f / body->GetInverseMass()), body->GetWorldOrigin(), 0.08f,
                                      colorCyan, gameLocal.GetLocalPlayer()->viewAngles.ToMat3(), 1);
        }
    }
    ...
    {
        axis = gameLocal.GetLocalPlayer()->viewAngles.ToMat3();
        gameRenderWorld->DrawText(va("\n%1.2f", totalMass), bodies[0]->GetWorldOrigin() + axis[2] * 8.0f, 0.15f,
                                  colorCyan, axis, 1);
    }
    ...
    if (af_showInertia.GetBool())
    {
        for (i = 0; i < bodies.Num(); i++)
        {
            ...
            gameRenderWorld->DrawText(va("\n\n\n( %.1f %.1f %.1f )\n( %.1f %.1f %.1f )\n( %.1f %.1f %.1f )", I[0].x,
                                         I[0].y, I[0].z, I[1].x, I[1].y, I[1].z, I[2].x, I[2].y, I[2].z),
    ...
    if (af_showConstraintNames.GetBool())
    {
        for (i = 0; i < primaryConstraints.Num(); i++)
        {
            constraint = primaryConstraints[i];
            constraint->GetCenter(center);
            gameRenderWorld->DrawText(constraint->GetName().c_str(), center, 0.08f, colorCyan,
                                      gameLocal.GetLocalPlayer()->viewAngles.ToMat3(), 1);
        }
        if (!af_showPrimaryOnly.GetBool())
        {
...
void idPhysics_AF::BuildTrees(void)
{
    ...
    if (af_useLinearTime.GetBool())
    {
        ...
        for (i = 0; i < constraints.Num(); i++)
        {
            ...
            if (!c->body1->primaryConstraint && c->fl.allowPrimary && c->body2 != NULL &&
                !IsClosedLoop(c->body1, c->body2))
            {
...
void idPhysics_AF::GetImpactInfo(const int id, const idVec3 &point, impactInfo_t *info) const
{
    ...
    info->invInertiaTensor =
        bodies[id]->current->worldAxis.Transpose() * bodies[id]->inverseInertiaTensor * bodies[id]->current->worldAxis;
    info->position = point - bodies[id]->current->worldOrigin;
    info->velocity = bodies[id]->current->spatialVelocity.SubVec3(0) +
...
void idPhysics_AF::SetOrigin(const idVec3 &newOrigin, int id)
{
    if (masterBody)
    {
        Translate(masterBody->current->worldOrigin + masterBody->current->worldAxis * newOrigin -
        ...
        if (body->clipModel->IsTraceModel())
        {
            if (model)
            {
                gameLocal.clip.TranslationModel(
                    bodyResults, body->current->worldOrigin, body->current->worldOrigin + translation, body->clipModel,
                    body->current->worldAxis, body->clipMask, model->Handle(), model->GetOrigin(), model->GetAxis());
            }
            else
            {
...
void idPhysics_AF::ClipRotation(trace_t &results, const idRotation &rotation, const idClipModel *model) const
{
    ...
    for (i = 0; i < bodies.Num(); i++)
    {
        ...
        if (body->clipModel->IsTraceModel())
        {
            if (model)
            {
                gameLocal.clip.RotationModel(bodyResults, body->current->worldOrigin, rotation, body->clipModel,
                                             body->current->worldAxis, body->clipMask, model->Handle(),
                                             model->GetOrigin(), model->GetAxis());
            }
            else
            {
                gameLocal.clip.Rotation(bodyResults, body->current->worldOrigin, rotation, body->clipModel,
                                        body->current->worldAxis, body->clipMask, self);
            }
            if (bodyResults.fraction < results.fraction)
            {
...
int idPhysics_AF::ClipContents(const idClipModel *model) const
{
    ...
    for (i = 0; i < bodies.Num(); i++)
    {
        ...
        if (body->clipModel->IsTraceModel())
        {
            ...
            {
                contents |=
                    gameLocal.clip.ContentsModel(body->current->worldOrigin, body->clipModel, body->current->worldAxis,
                                                 -1, model->Handle(), model->GetOrigin(), model->GetAxis());
            }
            else
            {
                contents |= gameLocal.clip.Contents(body->current->worldOrigin, body->clipModel,
...
void idPhysics_AF::SetPushed(int deltaTime)
{
    ...
    if (bodies.Num())
    {
        ...
        current.pushVelocity.SubVec3(0) +=
            (body->current->worldOrigin - body->saved.worldOrigin) / (deltaTime * idMath::M_MS2SEC);
        current.pushVelocity.SubVec3(1) +=
            rotation.GetVec() * -DEG2RAD(rotation.GetAngle()) / (deltaTime * idMath::M_MS2SEC);
    }
}
