    ...
    for (bumpcount = 0; bumpcount < numbumps; bumpcount++)
    {
        ...
        if (stepUp)
        {
            ...
            if (nearGround)
            {
                ...
                gameLocal.clip.Translation(stepTrace, downTrace.endpos, stepEnd, clipModel, clipModel->GetAxis(),
                                           clipMask, self);

                // step down
                stepEnd = stepTrace.endpos + maxStepHeight * gravityNormal;
                gameLocal.clip.Translation(downTrace, stepTrace.endpos, stepEnd, clipModel, clipModel->GetAxis(),
                                           clipMask, self);
...
void idPhysics_Player::CheckGround(void)
{
    ...
    if (groundEntityPtr.GetEntity())
    {
        ...
        {
            groundEntityPtr.GetEntity()->ApplyImpulse(self, groundTrace.c.id, groundTrace.c.point,
...
bool idPhysics_Player::Evaluate(int timeStepMSec, int endTimeMSec)
{
    ...
    if (IsOutsideWorld())
    {
        gameLocal.Warning("clip model outside world bounds for entity '%s' at (%s)", self->name.c_str(),
