...
idVec3 idPlayer::colorBarTable[5] = {idVec3(0.25f, 0.25f, 0.25f), idVec3(1.00f, 0.00f, 0.00f),
...
bool idPlayer::UserInfoChanged(bool canModify)
{
    ...
    if (canModify && gameLocal.gameType == GAME_TDM && !gameLocal.mpGame.IsInGame(entityNumber) &&
...
void idPlayer::UpdateHud(void)
{
    ...
    {
        if (MPAim != -1 && gameLocal.gameType == GAME_TDM && gameLocal.entities[MPAim] &&
                                ...
                                const float damageScale, const int location, int *health, int *armor)
{
    ...
    if (gameLocal.gameType == GAME_TDM && !gameLocal.serverInfo.GetBool("si_teamDamage") &&
        !damageDef->GetBool("noTeam") && player && player != this // you get self damage no matter what
        && player->team == team)
    {
        damage = 0;
    }
