...
const opcode_t idCompiler::opcodes[] = {{"<RETURN>", "RETURN", -1, false, &def_void, &def_void, &def_void},
...
idVarDef *idCompiler::FindImmediate(const idTypeDef *type, const eval_t *eval, const char *string) const
{
    ...
    for (def = gameLocal.program.GetDefList("<IMMEDIATE>"); def != NULL; def = def->Next())
    {
        ...
        case ev_vector:
            if ((def->value.vectorPtr->x == eval->vector[0]) && (def->value.vectorPtr->y == eval->vector[1]) &&
    ...
    case TT_LITERAL: {
        ...
        for (i = 0; i < 3; i++)
        {
            ...
            if (token2.type == TT_PUNCTUATION && token2 == "-")
            {
                ...
                {
                    Error("expected a number following '-' but found '%s' in vector '%s'", token2.c_str(),
                          token.c_str());
                }
    ...
    else
    {
        ...
        if (!def)
        {
            ...
            if ((scope->Type() != ev_namespace) && (scope->scope->Type() == ev_object))
            {
                ...
                while ((op->type_a->Type() != ev_object) || (type_b != op->type_b->Type()) ||
                       (type_c != op->type_c->Type()))
                {
                    if ((op->priority == FUNCTION_PRIORITY) && (op->type_a->Type() == ev_object) &&
                        (op->type_c->Type() == ev_void) && (type_c != op->type_c->Type()))
                    {
...
idVarDef *idCompiler::GetExpression(int priority)
{
    ...
    while (1)
    {
        ...
        {
            if ((op->priority == FUNCTION_PRIORITY) && TypeMatches(type_a, op->type_a->Type()) &&
...
void idCompiler::ParseObjectDef(const char *objname)
{
    ...
    objtype = gameLocal.program.AllocType(ev_object, NULL, objname, parentType == &type_object ? 0 : parentType->Size(),
                                          parentType);
...
void idCompiler::ParseFunctionDef(idTypeDef *returnType, const char *name)
{
    ...
    assert((func->parmTotal == 0 || totalSize == func->parmTotal) &&
           "function parameter sizes differ between protype vs implementation?!");
    ...
    if (oldscope->TypeDef()->Inherits(&type_object) && !idStr::Icmp(name, "init"))
    {
        ...
        for (superClass = oldscope->TypeDef()->SuperClass(); superClass != &type_object;
        ...
        if (scope->Type() == ev_function)
        {
            ...
            else if ((type == &type_string) && (def2->TypeDef() == &type_string))
            {
                EmitOpcode(OP_STORE_S, def2, def);
                     ...
                     ((def2->TypeDef() == &type_entity) || (def2->TypeDef()->Inherits(&type_object))))
            {
