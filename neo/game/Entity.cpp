...
idEntity::~idEntity(void)
{
    ...
    if (gameLocal.GameState() != GAMESTATE_SHUTDOWN && !gameLocal.isClient && fl.networkSync &&
...
void idEntity::UpdatePVSAreas(void)
{
    ...
    {
        localNumPVSAreas = gameLocal.pvs.GetPVSAreas(idBounds(modelAbsBounds.GetCenter()).Expand(64.0f), localPVSAreas,
            ...
            if (spawnArgs.GetString("bindToJoint", "", &joint) && *joint)
            {
                    ...
                    *bindanim)
                {
                    ...
                    gameEdit->ANIM_CreateAnimFrame(
                        parentAnimator->ModelHandle(), anim->MD5Anim(0), parent->renderEntity.numJoints, frame, 0,
                        parentAnimator->ModelDef()->GetVisualOffset(), parentAnimator->RemoveOrigin());
...
bool idAnimatedEntity::ClientReceiveEvent(int event, int time, const idBitMsg &msg)
{
    ...
    case EVENT_ADD_DAMAGE_EFFECT: {
        ...
        const idDeclEntityDef *damageDef =
            static_cast<const idDeclEntityDef *>(declManager->DeclByIndex(DECL_ENTITYDEF, damageDefIndex));
        const idMaterial *collisionMaterial =
            static_cast<const idMaterial *>(declManager->DeclByIndex(DECL_MATERIAL, materialIndex));
