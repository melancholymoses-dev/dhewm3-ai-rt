...
void idTarget_SetInfluence::Event_RestoreInfluence()
{
    ...
    for (i = 0; i < guiList.Num(); i++)
    {
        ...
        for (j = 0; j < MAX_RENDERENTITY_GUI; j++)
        {
            if (ent->GetRenderEntity()->gui[j])
            {
                ent->GetRenderEntity()->gui[j] =
                    uiManager->FindGui(ent->spawnArgs.GetString(j == 0 ? "gui" : va("gui%d", j + 1)));
                update = true;
            }
        }
        if (update)
...
void idTarget_SetPrimaryObjective::Event_Activate(idEntity *activator)
{
    ...
    if (player && player->objectiveSystem)
    {
        player->objectiveSystem->SetStateString(
            "missionobjective", spawnArgs.GetString("text", common->GetLanguageDict()->GetString("#str_04253")));
    }
}
