...
void idMover::Event_UpdateMove(void)
{
    ...
    {
    case ACCELERATION_STAGE: {
        physicsObj.SetLinearExtrapolation(EXTRAPOLATION_ACCELLINEAR, gameLocal.time, move.acceleration, org, move.dir,
                                          vec3_origin);
        if (move.movetime > 0)
        {
    ...
    case LINEAR_STAGE: {
        physicsObj.SetLinearExtrapolation(EXTRAPOLATION_LINEAR, gameLocal.time, move.movetime, org, move.dir,
                                          vec3_origin);
    ...
    case DECELERATION_STAGE: {
        physicsObj.SetLinearExtrapolation(EXTRAPOLATION_DECELLINEAR, gameLocal.time, move.deceleration, org, move.dir,
                                          vec3_origin);
        move.stage = FINISHED_STAGE;
        break;
    }
...
void idMover::Event_UpdateRotation(void)
{
    ...
    {
    case ACCELERATION_STAGE: {
        physicsObj.SetAngularExtrapolation(EXTRAPOLATION_ACCELLINEAR, gameLocal.time, rot.acceleration, ang, rot.rot,
                                           ang_zero);
        if (rot.movetime > 0)
        {
    ...
    case LINEAR_STAGE: {
        ...
        {
            physicsObj.SetAngularExtrapolation(extrapolation_t(EXTRAPOLATION_LINEAR | EXTRAPOLATION_NOSTOP),
                                               gameLocal.time, rot.movetime, ang, rot.rot, ang_zero);
        }
        else
        {
            physicsObj.SetAngularExtrapolation(EXTRAPOLATION_LINEAR, gameLocal.time, rot.movetime, ang, rot.rot,
                                               ang_zero);
        }
    ...
    case DECELERATION_STAGE: {
        physicsObj.SetAngularExtrapolation(EXTRAPOLATION_DECELLINEAR, gameLocal.time, rot.deceleration, ang, rot.rot,
                                           ang_zero);
        rot.stage = FINISHED_STAGE;
        break;
    }
...
void idMover::Event_TeamBlocked(idEntity *blockedEntity, idEntity *blockingEntity)
{
    if (g_debugMover.GetBool())
    {
        gameLocal.Printf("%d: '%s' stopped due to team member '%s' blocked by '%s'\n", gameLocal.time, name.c_str(),
                         blockedEntity->name.c_str(), blockingEntity->name.c_str());
    }
}
...
void idMover::Event_MoveAccelerateTo(float speed, float time)
{
    ...
    StartSound("snd_accel", SND_CHANNEL_BODY2, 0, false, NULL);
    StartSound("snd_move", SND_CHANNEL_BODY, 0, false, NULL);
    physicsObj.SetLinearExtrapolation(EXTRAPOLATION_ACCELLINEAR, gameLocal.time, move.acceleration, org,
                                      dir * (speed - v), dir * v);
}
...
void idMover::Event_MoveDecelerateTo(float speed, float time)
{
    ...
    StartSound("snd_decel", SND_CHANNEL_BODY2, 0, false, NULL);
    StartSound("snd_move", SND_CHANNEL_BODY, 0, false, NULL);
    physicsObj.SetLinearExtrapolation(EXTRAPOLATION_DECELLINEAR, gameLocal.time, move.deceleration, org,
...
void idMover::Event_Bob(float speed, float phase, idVec3 &depth)
{
    ...
    physicsObj.GetLocalOrigin(org);
    physicsObj.SetLinearExtrapolation(extrapolation_t(EXTRAPOLATION_DECELSINE | EXTRAPOLATION_NOSTOP),
...
void idMover::Event_Sway(float speed, float phase, idAngles &depth)
{
    ...
    physicsObj.SetAngularExtrapolation(extrapolation_t(EXTRAPOLATION_DECELSINE | EXTRAPOLATION_NOSTOP),
...
void idMover_Binary::Spawn(void)
{
    ...
    physicsObj.SetAngularExtrapolation(EXTRAPOLATION_NONE, 0, 0, GetPhysics()->GetAxis().ToAngles(), ang_zero,
                                       ang_zero);
    SetPhysics(&physicsObj);
...
void idMover_Binary::SetMoverState(moverState_t newstate, int time)
{
    ...
    case MOVER_1TO2: {
        ...
        physicsObj.SetLinearExtrapolation(EXTRAPOLATION_LINEAR, time, duration, pos1,
    ...
    case MOVER_2TO1: {
        ...
        physicsObj.SetLinearExtrapolation(EXTRAPOLATION_LINEAR, time, duration, pos2,
...
void idDoor::Think(void)
{
    ...
    if (thinkFlags & TH_PHYSICS)
    {
        ...
        if (GetMasterPosition(masterOrigin, masterAxis))
        {
            if (trigger)
            {
                trigger->Link(gameLocal.clip, this, 0, masterOrigin + localTriggerOrigin * masterAxis,
                              localTriggerAxis * masterAxis);
            }
            if (sndTrigger)
            {
                sndTrigger->Link(gameLocal.clip, this, 0, masterOrigin + localTriggerOrigin * masterAxis,
                                 localTriggerAxis * masterAxis);
            }
        }
    }
...
void idRotater::Spawn(void)
{
    ...
    physicsObj.SetLinearExtrapolation(EXTRAPOLATION_NONE, gameLocal.time, 0, GetPhysics()->GetOrigin(), vec3_origin,
                                      vec3_origin);
    physicsObj.SetAngularExtrapolation(extrapolation_t(EXTRAPOLATION_LINEAR | EXTRAPOLATION_NOSTOP), gameLocal.time, 0,
                                       GetPhysics()->GetAxis().ToAngles(), ang_zero, ang_zero);
    SetPhysics(&physicsObj);
...
void idRotater::Event_Activate(idEntity *activator)
{
    ...
    physicsObj.SetAngularExtrapolation(extrapolation_t(EXTRAPOLATION_LINEAR | EXTRAPOLATION_NOSTOP), gameLocal.time, 0,
                                       physicsObj.GetAxis().ToAngles(), delta, ang_zero);
}
...
void idBobber::Spawn(void)
{
    ...
    physicsObj.SetLinearExtrapolation(extrapolation_t(EXTRAPOLATION_DECELSINE | EXTRAPOLATION_NOSTOP), phase * 1000,
                                      speed * 500, GetPhysics()->GetOrigin(), delta * 2.0f, vec3_origin);
    SetPhysics(&physicsObj);
}
...
void idPendulum::Spawn(void)
{
    ...
    physicsObj.SetLinearExtrapolation(EXTRAPOLATION_NONE, 0, 0, GetPhysics()->GetOrigin(), vec3_origin, vec3_origin);
    physicsObj.SetAngularExtrapolation(extrapolation_t(EXTRAPOLATION_DECELSINE | EXTRAPOLATION_NOSTOP), phase * 1000,
                                       500 / freq, GetPhysics()->GetAxis().ToAngles(), idAngles(0, 0, speed * 2.0f),
                                       ang_zero);
    SetPhysics(&physicsObj);
}
...
void idRiser::Event_Activate(idEntity *activator)
{
    ...
    else
    {
        ...
        physicsObj.SetLinearExtrapolation(EXTRAPOLATION_LINEAR, gameLocal.time, time * 1000, physicsObj.GetOrigin(),
                                          delta, vec3_origin);
    }
}
