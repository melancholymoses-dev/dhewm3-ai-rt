...
bool idBrittleFracture::UpdateRenderEntity(renderEntity_s *renderEntity, const renderView_t *renderView) const
{
    ...
    tris = renderEntity->hModel->AllocSurfaceTriangles(numTris * 3,
                                                       material->ShouldCreateBackSides() ? numTris * 6 : numTris * 3);
    decalTris = renderEntity->hModel->AllocSurfaceTriangles(
        numDecalTris * 3, decalMaterial->ShouldCreateBackSides() ? numDecalTris * 6 : numDecalTris * 3);
...
================
*/
void idBrittleFracture::DropShard(shard_t *shard, const idVec3 &point, const idVec3 &dir, const float impulse,
                                  const int time)
{
    int i, j, clipModelId;
    float dist, f;
    ...
    f = dist > maxShatterRadius
            ? 1.0f
            : idMath::Sqrt(dist - minShatterRadius) * (1.0f / idMath::Sqrt(maxShatterRadius - minShatterRadius));
