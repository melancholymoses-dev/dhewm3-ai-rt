...
typedef int ImGuiNavRenderCursorFlags; // -> enum ImGuiNavRenderCursorFlags_//Flags: for
                                       // RenderNavCursor()
...
{
    // Controlled by user
    ImGuiItemFlags_Disabled = 1 << 10,               // false     // Disable interactions (DOES NOT affect visuals. DO
    ...
    ImGuiItemFlags_MixedValue = 1 << 12,             // false     // [BETA] Represent a mixed/indeterminate value,
    ...
    ImGuiItemFlags_Inputable = 1 << 20,            // false     // [WIP] Auto-activate input mode when tab focused.
    ...
    ImGuiItemFlags_Default_ = ImGuiItemFlags_AutoClosePopups, // Please don't change, use PushItemFlag()
                                                              // instead.
...
enum ImGuiItemStatusFlags_
{
    ...
    ImGuiItemStatusFlags_HoveredRect = 1 << 0,      // Mouse position is within item rectangle (does NOT mean that the
    ...
    ImGuiItemStatusFlags_HoveredWindow = 1 << 7,    // Override the HoveredWindow test to allow cross-window hover
                                                    // testing.
    ImGuiItemStatusFlags_Visible = 1 << 8,          // [WIP] Set when item is overlapping the current clipping
...
enum ImGuiButtonFlagsPrivate_
{
    ...
    ImGuiButtonFlags_AllowOverlap = 1 << 12,    // require previous frame HoveredId to either match id or be null
                                                // before being usable.
    ...
    ImGuiButtonFlags_AlignTextBaseLine = 1 << 15, // vertically align button to match text baseline - ButtonEx()
    ...
    ImGuiButtonFlags_NoNavFocus = 1 << 18,        // don't override navigation focus when activated (FIXME: this is
    ...
    ImGuiButtonFlags_NoSetKeyOwner = 1 << 20,     // don't set key/input owner on the initial click (note: mouse
                                                  // buttons are keys! often, the key in question will be
                                                  // ImGuiKey_MouseLeft!)
    ImGuiButtonFlags_NoTestKeyOwner = 1 << 21,    // don't test key/input owner when polling the key (note: mouse
                                                  // buttons are keys! often, the key in question will be
                                                  // ImGuiKey_MouseLeft!)
...
enum ImGuiSliderFlagsPrivate_
{
    ...
    ImGuiSliderFlags_ReadOnly = 1 << 21, // Consider using g.NextItemData.ItemFlags
...
enum ImGuiSelectableFlagsPrivate_
{
    ...
    ImGuiSelectableFlags_SelectOnNav = 1 << 21,          // (WIP) Auto-select when moved into. This is not exposed in
    ...
    ImGuiSelectableFlags_SelectOnRelease = 1 << 23,      // Override button behavior to react on Release (default is
                                                         // Click+Release)
    ImGuiSelectableFlags_SpanAvailWidth = 1 << 24,       // Span all avail width even if we declared less for layout
                                                         // purpose. FIXME: We may be able to remove this (added in
                                                         // 6251d379, 2bcafc86 for menus)
    ImGuiSelectableFlags_SetNavIdOnHover = 1 << 25,      // Set Nav/Focus ID on mouse hover (used by MenuItem)
    ImGuiSelectableFlags_NoPadWithHalfSpacing = 1 << 26, // Disable padding each side with ItemSpacing * 0.5f
    ImGuiSelectableFlags_NoSetKeyOwner = 1 << 27,        // Don't set key/input owner on the initial click (note: mouse
                                                         // buttons are keys! often, the key in question will be
                                                         // ImGuiKey_MouseLeft!)
};
...
enum ImGuiTreeNodeFlagsPrivate_
{
    ...
    ImGuiTreeNodeFlags_UpsideDownArrow = 1 << 29, // FIXME-WIP: Turn Down arrow into an Up arrow, for reversed
                                                  // trees (#6517)
...
struct ImGuiNextItemData
{
    ...
    ImGuiSelectionUserData SelectionUserData; // Set by SetNextItemSelectionUserData() (note that
...
struct ImGuiLastItemData
{
    ...
    ImGuiKeyChord Shortcut; // Shortcut at the time of submitting item. ONLY VALID IF
...
struct ImGuiKeyRoutingTable
{
    ...
    ImVector<ImGuiKeyRoutingData> EntriesNext; // Double-buffer to avoid reallocation (could use a shared
                                               // buffer)
...
enum ImGuiScrollFlags_
{
    ...
    ImGuiScrollFlags_KeepVisibleEdgeX = 1 << 0,   // If item is not visible: scroll as little as possible on X axis
                                                  // to bring item back into view [default for X axis]
    ImGuiScrollFlags_KeepVisibleEdgeY = 1 << 1,   // If item is not visible: scroll as little as possible on Y axis
...
enum ImGuiNavMoveFlags_
{
    ...
    ImGuiNavMoveFlags_AlsoScoreVisibleSet = 1 << 5, // Store alternate result in NavMoveResultLocalVisible that only
                                                    ...
                                                    // PageUp/PageDown)
    ImGuiNavMoveFlags_ScrollToEdgeY = 1 << 6,       // Force scrolling to min/max (used by Home/End) // FIXME-NAV: Aim
...
struct IMGUI_API ImGuiTypingSelectRequest
{
    ...
    bool SelectRequest;       // Set when buffer was modified this frame, requesting a
                              // selection.
    bool SingleCharMode;      // Notify when buffer contains same character repeated, to
                              // implement special mode. In this situation it preferred
                              // to not display any on-screen search indication.
    ImS8 SingleCharSize;      // Length in bytes of first letter codepoint (1 for ascii,
...
struct ImGuiBoxSelectState
{
    ...
    ImRect BoxSelectRectPrev; // Selection rectangle in absolute coordinates (derived
...
struct ImGuiContext
{
    ...
    // Inputs
    ImVector<ImGuiInputEvent> InputEventsQueue; // Input events which will be trickled/written into IO
                                                // structure.
    ImVector<ImGuiInputEvent> InputEventsTrail; // Past input events processed in NewFrame(). This is to
    ...
    ImGuiWindow *HoveredWindowBeforeClear;       // Window the mouse is hovering. Filled even
    ...
    ImGuiDataTypeStorage ActiveIdValueOnActivation; // Backup of initial value at the time of
    ...
    ImGuiID DebugLocateId;              // Storage for DebugLocateItemOnHover() feature: this is
    ...
    ImVector<ImGuiColorMod> ColorStack;             // Stack for PushStyleColor()/PopStyleColor() - inherited by
                                                    // Begin()
    ImVector<ImGuiStyleMod> StyleVarStack;          // Stack for PushStyleVar()/PopStyleVar() - inherited by
                                                    // Begin()
    ...
    ImVector<ImGuiItemFlags> ItemFlagsStack;        // Stack for PushItemFlag()/PopItemFlag() - inherited by
                                                    // Begin()
    ...
    ImVector<ImGuiFocusScopeData> NavFocusRoute; // Reversed copy focus scope stack for NavId (should
    ...
    ImGuiSelectionUserData NavLastValidSelectionUserData; // Last valid data passed to
    ...
    ImGuiNavItemData NavInitResult; // Init request result (first item of the window, or one
    ...
    ImGuiNavItemData NavMoveResultOther;        // Best move request candidate within NavWindow's
                                                // flattened hierarchy (when using
                                                // ImGuiWindowFlags_NavFlattened flag)
    ImGuiNavItemData NavTabbingResultFirst;     // First tabbing request candidate within NavWindow
                                                // and flattened hierarchy

    // Navigation: record of last move request
    ImGuiID NavJustMovedFromFocusScopeId; // Just navigated from this focus scope id
    ...
    ImGuiKeyChord ConfigNavWindowingKeyNext; // = ImGuiMod_Ctrl | ImGuiKey_Tab (or
    ...
    float HoverItemDelayClearTimer;          // Currently used by IsItemHovered(): grace time
...
struct IMGUI_API ImGuiWindowTempData
{
    ...
    ImVec1 GroupOffset;
    ImVec2 CursorStartPosLossyness; // Record the loss of precision of CursorStartPos
    ...
    ImVector<float> TextWrapPosStack; // Store text wrap pos to restore (attention: .back() is
                                      // not == TextWrapPos)
};
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImGuiWindow *ParentWindowForFocusRoute;      // Set to manual link a window to its logical
...
{
    ImGuiTabBarFlags_DockNode = 1 << 20, // Part of a dock node [we don't use this in the master branch
                                         // but it facilitate branch syncing to keep this around]
    ImGuiTabBarFlags_IsFocused = 1 << 21,
    ImGuiTabBarFlags_SaveSettings = 1 << 22, // FIXME: Settings are handled by the docking system, this only
...
enum ImGuiTabItemFlagsPrivate_
{
    ...
    ImGuiTabItemFlags_NoCloseButton = 1 << 20, // Track whether p_open was set or not (we'll need this info on
...
struct ImGuiTableColumn
{
    ...
    float InitStretchWeightOrWidth; // Value passed to TableSetupColumn(). For Width
...
struct IMGUI_API ImGuiTable
{
    ...
    ImSpan<ImGuiTableColumnIdx> DisplayOrderToIndex; // Point within RawData[]. Store display order of
                                                     // columns (when not reordered, the values are
                                                     // 0...Count-1)
    ImSpan<ImGuiTableCellData> RowCellData;          // Point within RawData[]. Store cells background requests
                                                     // for current row.
    ...
    int CurrentRow;
    int CurrentColumn;
    ImS16 InstanceCurrent;    // Count of BeginTable() calls with same ID in the same
    ...
    ImDrawListSplitter *DrawSplitter; // Shortcut to TempData->DrawSplitter while in table.
                                      // Isolate draw commands per columns to avoid switching
                                      // clip rect constantly
    ImGuiTableInstanceData InstanceDataFirst;
    ImVector<ImGuiTableInstanceData> InstanceDataExtra; // FIXME-OPT: Using a small-vector pattern would be
                                                        // good.
    ...
    ImGuiTableColumnIdx ColumnsEnabledFixedCount;   // Number of enabled columns using fixed width
                                                    // (<= ColumnsCount)
    ImGuiTableColumnIdx DeclColumnsCount;           // Count calls to TableSetupColumn()
    ImGuiTableColumnIdx AngledHeadersCount;         // Count columns with angled headers
    ImGuiTableColumnIdx HoveredColumnBody;          // Index of column whose visible region is being
...
struct ImGuiTableSettings
{
    ...
    ImGuiTableColumnIdx ColumnsCountMax; // Maximum number of columns this settings instance can
...
IMGUI_API void DebugLocateItemOnHover(ImGuiID target_id); // Only call on reaction to a mouse Hover: because only
                                                          // 1 at the same time!
