...
void idSoundWorldLocal::ResolveOrigin(const int stackDepth, const soundPortalTrace_t *prevStack, const int soundArea,
                                      const float dist, const idVec3 &soundOrigin, idSoundEmitterLocal *def)
{
...
===================
*/
void idSoundWorldLocal::PlaceListener(const idVec3 &origin, const idMat3 &axis, const int listenerId,
                                      const int gameTime, const idStr &areaName)
{
...
===============
*/
void idSoundWorldLocal::CalcEars(int numSpeakers, idVec3 spatializedOrigin, idVec3 listenerPos, idMat3 listenerAxis,
                                 float ears[6], float spatialize)
{
...
===============
*/
void idSoundWorldLocal::AddChannelContribution(idSoundEmitterLocal *sound, idSoundChannel *chan, int current44kHz,
                                               int numSpeakers, float *finalMixBuffer)
{
    int j;
    float volume;
    ...
    if (sound->removeStatus < REMOVE_STATUS_SAMPLEFINISHED)
    {
        if (!alIsSource(chan->openalSource))
        {
            chan->openalSource = soundSystemLocal.AllocOpenALSource(
                chan, !chan->leadinSample->hardwareBuffer || !chan->soundShader->entries[0]->hardwareBuffer || looping,
        ...
        if (alIsSource(chan->openalSource))
        {
            ...
            else
            {
                ...
                alSource3f(chan->openalSource, AL_POSITION, -spatializedOriginInMeters.y, spatializedOriginInMeters.z,
            ...
            alSourcei(chan->openalSource, AL_LOOPING,
                      (looping && chan->soundShader->entries[0]->hardwareBuffer && !haveLeadin) ? AL_TRUE : AL_FALSE);
#if 1
                ...
                (looping && !haveLeadin && chan->soundShader->entries[0]->hardwareBuffer))
            {
                ...
                if (chan->triggered)
                {
                    alSourcei(chan->openalSource, AL_BUFFER,
                              looping ? chan->soundShader->entries[0]->openalBuffer : chan->leadinSample->openalBuffer);
                }
            }
            else
            {
                ...
                {
                    chan->GatherChannelSamples(chan->openalStreamingOffset * sample->objectInfo.nChannels,
                                               MIXBUFFER_SAMPLES * sample->objectInfo.nChannels, alignedInputSamples);
                    ...
                    alBufferData(buffers[j],
                                 chan->leadinSample->objectInfo.nChannels == 1 ? AL_FORMAT_MONO16 : AL_FORMAT_STEREO16,
                                 alignedInputSamples, MIXBUFFER_SAMPLES * sample->objectInfo.nChannels * sizeof(short),
                                 44100);
    ...
    else if (com_asyncSound.GetInteger() == 2)
    {
        ...
        if (numSpeakers == 6)
        {
            ...
            {
                SIMDProcessor->MixSoundSixSpeakerMono(finalMixBuffer, alignedInputSamples, MIXBUFFER_SAMPLES,
                                                      chan->lastV, ears);
            }
            else
            {
                SIMDProcessor->MixSoundSixSpeakerStereo(finalMixBuffer, alignedInputSamples, MIXBUFFER_SAMPLES,
                                                        chan->lastV, ears);
            }
        }
        else
        {
            if (sample->objectInfo.nChannels == 1)
            {
                SIMDProcessor->MixSoundTwoSpeakerMono(finalMixBuffer, alignedInputSamples, MIXBUFFER_SAMPLES,
                                                      chan->lastV, ears);
            }
            else
            {
                SIMDProcessor->MixSoundTwoSpeakerStereo(finalMixBuffer, alignedInputSamples, MIXBUFFER_SAMPLES,
                                                        chan->lastV, ears);
            }
        }
        ...
        else
        {
            ...
            if (amplitudeData)
            {
                ...
                if (offset < size)
                {
                    ...
                    {
                        sourceBuffer[j] =
                            j & 1 ? amplitudeData[(offset / 512) * 2] : amplitudeData[(offset / 512) * 2 + 1];
                    }
                }
            }
            else
