...
==================
*/
void idSoundEmitterLocal::OverrideParms(const soundShaderParms_t *base, const soundShaderParms_t *over,
    ...
    if (playing)
    {
        for (i = 0; i < SOUND_MAX_CHANNELS; i++)
        {
            ...
            if (chan->decoder != NULL &&
                chan->decoder->GetLastDecodeTime() < current44kHzTime - SOUND_DECODER_FREE_DELAY)
            {
...
=====================
*/
int idSoundEmitterLocal::StartSound(const idSoundShader *shader, const s_channelType channel, float diversity,
                                    int soundShaderFlags, bool allowSlow)
{
    int i;
...
void idSlowChannel::GenerateSlowChannel(FracTime &playPos, int sampleCount44k, float *finalBuffer)
{
    ...
    lowpass.SetContinuitySamples(in_p[numSamples - 2], in_p[numSamples - 3], out_p[numSamples - 2],
                                 out_p[numSamples - 3]);
