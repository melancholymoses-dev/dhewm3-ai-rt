...
bool idSoundShader::CheckShakesAndOgg(void) const
{
    ...
    for (i = 0; i < numLeadins; i++)
    {
        ...
        {
            common->Warning("sound shader '%s' has shakes and uses OGG file '%s'", GetName(), leadins[i]->name.c_str());
            ret = true;
        }
    }
    for (i = 0; i < numEntries; i++)
    {
        ...
        {
            common->Warning("sound shader '%s' has shakes and uses OGG file '%s'", GetName(), entries[i]->name.c_str());
            ret = true;
        }
    }
    return ret;
    ...
    for (int k = 0; k < numLeadins; k++)
    {
        ...
        if (objectp)
        {
            common->Printf("      %5dms %4dKb %s (LEADIN)\n",
                           soundSystemLocal.SamplesToMilliseconds(objectp->LengthIn44kHzSamples()),
