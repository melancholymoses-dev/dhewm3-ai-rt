...
// #modified-fva; BEGIN
idCVar cst_hudAdjustAspect("cst_hudAdjustAspect", "1", CVAR_GUI | CVAR_BOOL | CVAR_ARCHIVE,
...
const char *idWindow::ScriptNames[] = {"onMouseEnter",    "onMouseExit", "onAction",      "onActivate",
...
void idWindow::Redraw(float x, float y)
{
    ...
    {
        if (fixupFor43)
        { // DG: gotta reset that before returning this function
    ...
    while (token != "}")
    {
        ...
        else if (token == "definefloat")
        {
            ...
            if (com_editors & EDITOR_GUI)
            {
                ...
                rvGEWindowWrapper::GetWrapper(this)->GetVariableDict().Set(va("definefloat\t\"%s\"", token.c_str()),
                                                                           str);
            }
#endif
        }
        else if (token == "definevec4")
        {
            ...
            if (com_editors & EDITOR_GUI)
            {
                ...
                // Grab the string from the last marker and save it in the wrapper
                src->GetStringFromMarker(str, true);
                rvGEWindowWrapper::GetWrapper(this)->GetVariableDict().Set(va("definevec4\t\"%s\"", token.c_str()),
                                                                           str);
            }
#endif
        }
