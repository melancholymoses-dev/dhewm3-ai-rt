...
class idDeviceContext
{
  public:
    ...
    void DrawMaterial(float x, float y, float w, float h, const idMaterial *mat, const idVec4 &color,
  ...
  private:
    int DrawText(float x, float y, float scale, idVec4 color, const char *text, float adjust, int limit, int style,
                 int cursor = -1);
    void PaintChar(float x, float y, float width, float height, float scale, float s, float t, float s2, float t2,
