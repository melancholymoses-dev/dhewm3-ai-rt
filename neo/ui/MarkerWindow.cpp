...
const char *idMarkerWindow::HandleEvent(const sysEvent_t *event, bool *updateVisuals)
{
    ...
    if (event->evValue2 && key == K_MOUSE1)
    {
        ...
        for (i = 0; i < c; i++)
        {
            ...
            if (md.rect.Contains(gui->CursorX(), gui->CursorY()))
            {
                ...
                gui->GetDesktop()->SetChildWinVarVal(
                    "markerText", "text", va("Marker set at %.2i:%.2i", md.time / 60 / 60, (md.time / 60) % 60));
        ...
        if (i == c)
        {
            ...
            gui->GetDesktop()->SetChildWinVarVal(
                "markerText", "text", va("Marker set at %.2i:%.2i", currentTime / 60 / 60, (currentTime / 60) % 60));
            gui->GetDesktop()->SetChildWinVarVal("markerText", "visible", "1");
...
void idMarkerWindow::Draw(int time, float x, float y)
{
    ...
    if (r.w > 0 && r.Contains(gui->CursorX(), gui->CursorY()))
    {
        ...
        r.w = 40;
        r.h = 20;
        dc->DrawText(va("%.2i:%.2i", currentTime / 60 / 60, (currentTime / 60) % 60), 0.25, 0,
                     idDeviceContext::colorWhite, r, false);
    }
