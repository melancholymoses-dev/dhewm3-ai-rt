...
void SSDCrossHair::Draw(idDeviceContext *dc, const idVec2 &cursor)
{
    ...
    dc->DrawMaterial(x, y, crosshairWidth, crosshairHeight, crosshairMaterial[currentCrosshair], colorWhite, 1.0f,
                     1.0f);
}
...
void SSDAsteroid::Init(idGameSSDWindow *_game, const idVec3 &startPosition, const idVec2 &_size, float _speed,
...
SSDAsteroid *SSDAsteroid::GetNewAsteroid(idGameSSDWindow *_game, const idVec3 &startPosition, const idVec2 &_size,
...
SSDAstronaut *SSDAstronaut::GetNewAstronaut(idGameSSDWindow *_game, const idVec3 &startPosition, float _speed,
                                            float rotate, int _health)
{
...
void SSDExplosion::Init(idGameSSDWindow *_game, const idVec3 &_position, const idVec2 &_size, int _length, int _type,
...
SSDExplosion *SSDExplosion::GetNewExplosion(idGameSSDWindow *_game, const idVec3 &_position, const idVec2 &_size,
                                            int _length, int _type, SSDEntity *_buddy, bool _killBuddy,
                                            bool _followBuddy)
{
...
void SSDPoints::Init(idGameSSDWindow *_game, SSDEntity *_ent, int _points, int _length, int _distance,
                     const idVec4 &color)
{
...
SSDPoints *SSDPoints::GetNewPoints(idGameSSDWindow *_game, SSDEntity *_ent, int _points, int _length, int _distance,
                                   const idVec4 &color)
{
...
void SSDProjectile::Init(idGameSSDWindow *_game, const idVec3 &_beginPosition, const idVec3 &_endPosition, float _speed,
                         float _size)
{
...
SSDProjectile *SSDProjectile::GetNewProjectile(idGameSSDWindow *_game, const idVec3 &_beginPosition,
...
void SSDPowerup::OnHit(int key)
{
    ...
    if (powerupState == POWERUP_STATE_CLOSED)
    {
        ...
        SSDExplosion *explosion = SSDExplosion::GetNewExplosion(game, position, size * 2.0f, 300,
                                                                SSDExplosion::EXPLOSION_NORMAL, this, false, true);
    ...
    else
    {
        ...
        SSDExplosion *explosion =
            SSDExplosion::GetNewExplosion(game, position, size * 2, 300, SSDExplosion::EXPLOSION_NORMAL, this);
        game->entities.Append(explosion);
...
void idGameSSDWindow::SpawnAsteroid()
{
    ...
    float speed = random.RandomInt(asteroidData[gameStats.currentLevel].speedMax -
                                   asteroidData[gameStats.currentLevel].speedMin) +
                  asteroidData[gameStats.currentLevel].speedMin;
    float size =
        random.RandomInt(asteroidData[gameStats.currentLevel].sizeMax - asteroidData[gameStats.currentLevel].sizeMin) +
        asteroidData[gameStats.currentLevel].sizeMin;
    float rotate = (random.RandomFloat() *
                    (asteroidData[gameStats.currentLevel].rotateMax - asteroidData[gameStats.currentLevel].rotateMin)) +
                   asteroidData[gameStats.currentLevel].rotateMin;

    SSDAsteroid *asteroid = SSDAsteroid::GetNewAsteroid(this, startPosition, idVec2(size, size), speed, rotate,
                                                        asteroidData[gameStats.currentLevel].asteroidHealth);
    entities.Append(asteroid);

    gameStats.levelStats.nextAsteroidSpawnTime = currentTime +
...
void idGameSSDWindow::FireWeapon(int key)
{
    ...
    if (key == K_MOUSE1)
    {
        ...
        else
        {
            ...
            vec *= 8;
            SSDProjectile *newProj = SSDProjectile::GetNewProjectile(this, idVec3(0, -180, 0), vec,
...
void idGameSSDWindow::HitAsteroid(SSDAsteroid *asteroid, int key)
{
    ...
    if (asteroid->health <= 0)
    {
        ...
        SSDExplosion *explosion = SSDExplosion::GetNewExplosion(this, asteroid->position, asteroid->size * 2, 300,
    ...
    else
    {
        ...
        SSDExplosion *explosion = SSDExplosion::GetNewExplosion(this, asteroid->position, asteroid->size / 2.0f, 200,
...
void idGameSSDWindow::OnNuke()
{
    ...
    for (int i = 0; i < entities.Num(); i++)
    {
        ...
        if (entities[i]->type == SSD_ENTITY_ASTEROID)
        {
            ...
            // The asteroid has been destroyed
            SSDExplosion *explosion = SSDExplosion::GetNewExplosion(this, entities[i]->position, entities[i]->size * 2,
                                                                    300, SSDExplosion::EXPLOSION_NORMAL, entities[i]);
            entities.Append(explosion);
...
void idGameSSDWindow::RefreshGuiData()
{
    ...
    gui->SetStateString("level_complete",
                        va("%i/%i", gameStats.levelStats.savedAstronauts, levelData[gameStats.currentLevel].needToWin));
...
void idGameSSDWindow::SpawnAstronaut()
{
    ...
    float speed = random.RandomInt(astronautData[gameStats.currentLevel].speedMax -
                                   astronautData[gameStats.currentLevel].speedMin) +
                  astronautData[gameStats.currentLevel].speedMin;
    float rotate = (random.RandomFloat() * (astronautData[gameStats.currentLevel].rotateMax -
                                            astronautData[gameStats.currentLevel].rotateMin)) +
                   astronautData[gameStats.currentLevel].rotateMin;

    SSDAstronaut *astronaut =
        SSDAstronaut::GetNewAstronaut(this, startPosition, speed, rotate, astronautData[gameStats.currentLevel].health);
    entities.Append(astronaut);

    gameStats.levelStats.nextAstronautSpawnTime = currentTime +
...
void idGameSSDWindow::HitAstronaut(SSDAstronaut *astronaut, int key)
{
    ...
    if (key == K_MOUSE1)
    {
        ...
        if (astronaut->health <= 0)
        {
            ...
            SSDExplosion *explosion = SSDExplosion::GetNewExplosion(this, astronaut->position, astronaut->size * 2, 300,
        ...
        else
        {
            // This was a damage hit so create a real small quick explosion
            SSDExplosion *explosion =
                SSDExplosion::GetNewExplosion(this, astronaut->position, astronaut->size / 2.0f, 200,
...
void idGameSSDWindow::SpawnPowerup()
{
    ...
    float speed =
        random.RandomInt(powerupData[gameStats.currentLevel].speedMax - powerupData[gameStats.currentLevel].speedMin) +
        powerupData[gameStats.currentLevel].speedMin;
    float rotate = (random.RandomFloat() *
                    (powerupData[gameStats.currentLevel].rotateMax - powerupData[gameStats.currentLevel].rotateMin)) +
    ...
    gameStats.levelStats.nextPowerupSpawnTime =
        currentTime +
        random.RandomInt(powerupData[gameStats.currentLevel].spawnMax - powerupData[gameStats.currentLevel].spawnMin) +
