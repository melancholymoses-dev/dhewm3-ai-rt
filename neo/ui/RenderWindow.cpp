...
void idRenderWindow::Render(int time)
{
    ...
    if (worldEntity.hModel)
    {
        ...
        if (modelAnim)
        {
            ...
            gameEdit->ANIM_CreateAnimFrame(worldEntity.hModel, modelAnim, worldEntity.numJoints, worldEntity.joints,
                                           animLength - (animEndTime - time), vec3_origin, false);
        }
...
void idRenderWindow::Draw(int time, float x_, float y_)
{
    ...
    // DG: for scaling menus to 4:3 (like that spinning mars globe in the main
    // menu)
    float x = drawRect.x;
    float y = drawRect.y;
    float w = drawRect.w;
