...
static const char *cheatCodes[] = {"iddqd",      // Invincibility
...
#ifdef ID_DOOM_LEGACY
    if (down && keynum < 127)
    { // DG: only ASCII keys are of interest here
...
void idKeyInput::Init(void)
{
    ...
    cmdSystem->AddCommand("bind", Key_Bind_f, CMD_FL_SYSTEM, "binds a command to a key",
                          idKeyInput::ArgCompletion_KeyName);
    cmdSystem->AddCommand("bindunbindtwo", Key_BindUnBindTwo_f, CMD_FL_SYSTEM,
                          "binds a key but unbinds it first if there are more than two binds");
    cmdSystem->AddCommand("unbind", Key_Unbind_f, CMD_FL_SYSTEM, "unbinds any command from a key",
                          idKeyInput::ArgCompletion_KeyName);
