...
#ifdef DEBUG
idCVar idConsoleLocal::con_noPrint("con_noPrint", "0", CVAR_BOOL | CVAR_SYSTEM | CVAR_NOCHEAT,
...
float SCR_DrawFPS(float y)
{
    ...
    if (index > FPS_FRAMES)
    {
        ...
        renderSystem->DrawBigStringExt(635 - w, idMath::FtoiFast(y) + 2, s, colorWhite, true,
                                       localConsole.charSetShader);
    }
...
float SCR_DrawMemoryUsage(float y)
{
    ...
    SCR_DrawTextRightAlign(y, "frame alloc: %4d, %4dkB  frame free: %4d, %4dkB", allocs.num, allocs.totalSize >> 10,
                           frees.num, frees.totalSize >> 10);
...
float SCR_DrawAsyncStats(float y)
{
    ...
    else if (idAsyncNetwork::client.IsActive())
    {
        ...
        if (outgoingRate != -1 && incomingRate != -1)
        {
            SCR_DrawTextRightAlign(y, "out rate = %d B/s (% -2.1f%%), in rate = %d B/s (% -2.1f%%)", outgoingRate,
                                   outgoingCompression, incomingRate, incomingCompression);
        }
...
void idConsoleLocal::KeyDownEvent(int key)
{
    ...
    if ((key == K_DOWNARROW) || ((tolower(key) == 'n') && idKeyInput::IsDown(K_CTRL)))
    {
    ...
    if (consoleField.GetAutoCompleteLength() != 0)
    {
        ...
        if (autoCompleteLength > 0)
        {
            ...
            renderSystem->DrawStretchPic(2 * SMALLCHAR_WIDTH + consoleField.GetAutoCompleteLength() * SMALLCHAR_WIDTH,
                                         y + 2, autoCompleteLength * SMALLCHAR_WIDTH, SMALLCHAR_HEIGHT - 2, 0, 0, 0, 0,
                                         whiteShader);
        }
    }
...
void idConsoleLocal::DrawSolidConsole(float frac)
{
    ...
    if (display != current)
    {
        ...
        {
            renderSystem->DrawSmallChar((x + 1) * SMALLCHAR_WIDTH, idMath::FtoiFast(y), '^',
                                        localConsole.charSetShader);
        }
        y -= SMALLCHAR_HEIGHT;
        rows--;
    ...
    for (i = 0; i < rows; i++, y -= SMALLCHAR_HEIGHT, row--)
    {
        ...
        for (x = 0; x < LINE_WIDTH; x++)
        {
            ...
            renderSystem->DrawSmallChar((x + 1) * SMALLCHAR_WIDTH, idMath::FtoiFast(y), text_p[x] & 0xff,
                                        localConsole.charSetShader);
        }
    }
