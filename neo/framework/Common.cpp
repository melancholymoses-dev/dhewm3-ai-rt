...
idCVar com_enableDebuggerServer("com_enableDebuggerServer", "0", CVAR_BOOL | CVAR_SYSTEM,
...
static void PrintMemInfo_f(const idCmdArgs &args)
{
    ...
    f->Printf("total(%s ) image(%s ) model(%s ) sound(%s ): %s\n", idStr::FormatNumber(mi.assetTotals).c_str(),
              idStr::FormatNumber(mi.imageAssetsTotal).c_str(), idStr::FormatNumber(mi.modelAssetsTotal).c_str(),
...
void idCommonLocal::Frame(void)
{
    try
    {
        ...
        if (com_speeds.GetBool())
        {
            ...
            lastTime = nowTime;
            Printf("frame:%i all:%3i gfr:%3i rf:%3i bk:%3i\n", com_frameNumber, com_frameMsec, time_gameFrame,
...
void idCommonLocal::Init(int argc, char **argv)
{
    ...
    if (!SDL_Init(SDL_INIT_VIDEO | SDL_INIT_JOYSTICK | SDL_INIT_GAMEPAD))
    {
        ...
        { // retry without joystick/gamepad if it failed
