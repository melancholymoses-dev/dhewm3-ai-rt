...
void idAsyncServer::LocalClientInput(void)
{
    ...
    if (idAsyncNetwork::UsercmdInputChanged(userCmds[(gameFrame - 1) & (MAX_USERCMD_BACKUP - 1)][localClientNum],
...
void idAsyncServer::DropClient(int clientNum, const char *reason)
{
    ...
    cmdSystem->BufferCommandText(
        CMD_EXEC_NOW,
        va("addChatLine \"%s^0 %s\"", sessLocal.mapSpawnData.userInfo[clientNum].GetString("ui_name"), reason));
...
void idAsyncServer::ProcessUnreliableClientMessage(int clientNum, const idBitMsg &msg)
{
    ...
    case CLIENT_UNRELIABLE_MESSAGE_USERCMD: {
        ...
        for (last = NULL, i = clientGameFrame - numUsercmds + 1; i <= clientGameFrame; i++)
        {
            ...
            if (idAsyncNetwork::UsercmdInputChanged(userCmds[(i - 1) & (MAX_USERCMD_BACKUP - 1)][clientNum],
...
void idAsyncServer::ProcessAuthMessage(const idBitMsg &msg)
{
        ...
        if (!challenges[i].connected && challenges[i].clientId == clientId)
        {
            ...
            if (!Sys_CompareNetAdrBase(client_from, challenges[i].address))
            {
                ...
                common->DPrintf("auth: matched guid '%s' for != IPs %s and %s\n", client_guid,
                                Sys_NetAdrToString(client_from), Sys_NetAdrToString(challenges[i].address));
            }
            break;
        }
    }
    ...
    {
        common->DWarning("auth: challenge 0x%x %s authState %d != CDK_WAIT", challenges[i].challenge,
                         Sys_NetAdrToString(challenges[i].address), challenges[i].authState);
        return;
    }
...
void idAsyncServer::ProcessChallengeMessage(const netadr_t from, const idBitMsg &msg)
{
    ...
    for (i = 0; i < MAX_CHALLENGES; i++)
    {
        if (!challenges[i].connected && Sys_CompareNetAdrBase(from, challenges[i].address) &&
...
void idAsyncServer::ProcessConnectMessage(const netadr_t from, const idBitMsg &msg)
{
    ...
    case CDK_WAIT:
        if (challenges[ichallenge].authReply == AUTH_NONE &&
            Min(serverTime - lastAuthTime, serverTime - challenges[ichallenge].time) > AUTHORIZE_TIMEOUT)
        {
...
void idAsyncServer::PrintLocalServerInfo(void)
{
    ...
    common->Printf("server '%s' IP = %s\nprotocol %d.%d\n", sessLocal.mapSpawnData.serverInfo.GetString("si_name"),
                   Sys_NetAdrToString(serverPort.GetAdr()), ASYNC_PROTOCOL_MAJOR, ASYNC_PROTOCOL_MINOR);
    ...
    for (i = 0; i < MAX_ASYNC_CLIENTS; i++)
    {
        ...
        common->Printf("client %2d: %s, ping = %d, rate = %d\n", i,
                       sessLocal.mapSpawnData.userInfo[i].GetString("ui_name", "Player"), client.clientPing,
...
void idAsyncServer::RunFrame(void)
{
    ...
    do
    {
        ...
        do
        {
            ...
            newPacket =
                serverPort.GetPacketBlocking(from, msgBuf, size, sizeof(msgBuf), USERCMD_MSEC - gameTimeResidual - 1);
            if (newPacket)
            {
                msg.Init(msgBuf, sizeof(msgBuf));
                msg.SetSize(size);
...
{
    sprintf(msg, "avrg out: %d B/s - max %d B/s ( over %d ms )", stats_average_sum / stats_numsamples, stats_max,
