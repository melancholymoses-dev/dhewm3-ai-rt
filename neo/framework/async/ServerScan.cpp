...
const char *l_gameTypes[] = {"Deathmatch", "Tourney", "Team DM", "Last Man", "CTF", NULL};
...
int idServerScan::InfoResponse(networkServer_t &server)
{
    ...
    {
        common->DPrintf("idServerScan::InfoResponse - ignoring response from %s, wrong challenge %d.", serv.c_str(),
                        server.challenge);
        return false;
    }
...
void idServerScan::RunFrame()
{
    ...
    // update state
    if ((!incoming_net || (incoming_useTimeout && Sys_Milliseconds() > incoming_lastTime)) &&
...
void idServerScan::ApplyFilter()
{
    ...
    listGUI->Clear();
    for (i = m_sortAscending ? 0 : m_sortedServers.Num() - 1; m_sortAscending ? i < m_sortedServers.Num() : i >= 0;
         m_sortAscending ? i++ : i--)
    {
    ...
    if (keyval)
    {
        if (gui_filter_players.GetInteger() == 1 && server.clients == atoi(keyval->GetValue()))
        {
            return true;
                 ...
                 (!server.clients || server.clients == atoi(keyval->GetValue())))
        {
            return true;
        }
    }
