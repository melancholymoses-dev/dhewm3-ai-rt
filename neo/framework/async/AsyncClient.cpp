...
void idAsyncClient::ConnectToServer(const netadr_t adr)
{
    ...
    guiNetMenu = uiManager->FindGui("guis/netmenu.gui", true, false, true);
    guiNetMenu->SetStateString("status",
                               va(common->GetLanguageDict()->GetString("#str_06749"), Sys_NetAdrToString(adr)));
...
void idAsyncClient::ConnectToServer(const char *address)
{
    ...
    if (idStr::IsNumeric(address))
    {
        ...
        {
            session->MessageBox(MSG_OK, va(common->GetLanguageDict()->GetString("#str_06733"), serverNum),
    ...
    else
    {
        ...
        {
            session->MessageBox(MSG_OK, va(common->GetLanguageDict()->GetString("#str_06734"), address),
                                common->GetLanguageDict()->GetString("#str_06735"), true);
            return;
        }
    }
...
void idAsyncClient::ListServers(void)
{
    ...
    {
        common->Printf("%3d: %s %dms (%s)\n", i, serverList[i].serverInfo.GetString("si_name"), serverList[i].ping,
...
void idAsyncClient::ProcessUnreliableServerMessage(const idBitMsg &msg)
{
    ...
    case SERVER_UNRELIABLE_MESSAGE_GAMEINIT: {
        ...
        if (pureWait)
        {
            guiNetMenu = uiManager->FindGui("guis/netmenu.gui", true, false, true);
            session->SetGUI(guiNetMenu, HandleGuiCommand);
            session->MessageBox(MSG_ABORT, common->GetLanguageDict()->GetString("#str_04317"),
                                common->GetLanguageDict()->GetString("#str_04318"), false, "pure_abort");
        }
        else
        {
            // load map
    ...
    case SERVER_UNRELIABLE_MESSAGE_SNAPSHOT: {
        ...
        game->ClientReadSnapshot(clientNum, snapshotSequence, snapshotGameFrame, snapshotGameTime,
        ...
        if (gameTime < snapshotGameTime ||
            gameTime > snapshotGameTime + idAsyncNetwork::clientMaxPrediction.GetInteger())
        {
            gameFrame = snapshotGameFrame;
            gameTime = snapshotGameTime;
            gameTimeResidual = idMath::ClampInt(-idAsyncNetwork::clientMaxPrediction.GetInteger(),
                                                idAsyncNetwork::clientMaxPrediction.GetInteger(), gameTimeResidual);
            clientPredictTime = idMath::ClampInt(-idAsyncNetwork::clientMaxPrediction.GetInteger(),
                                                 idAsyncNetwork::clientMaxPrediction.GetInteger(), clientPredictTime);
        }
        ...
        clientPrediction = idMath::ClampInt(idAsyncNetwork::clientPrediction.GetInteger(),
        ...
        case SERVER_RELIABLE_MESSAGE_DISCONNECT: {
            ...
            else
            {
                ...
                cmdSystem->BufferCommandText(
                    CMD_EXEC_NOW, va("addChatLine \"%s^0 %s\"",
...
void idAsyncClient::ProcessInfoResponseMessage(const netadr_t from, const idBitMsg &msg)
{
    ...
    if (protocol != ASYNC_PROTOCOL_VERSION)
    {
        common->Printf("server %s ignored - protocol %d.%d, expected %d.%d\n", Sys_NetAdrToString(serverInfo.adr),
                       protocol >> 16, protocol & 0xffff, ASYNC_PROTOCOL_MAJOR, ASYNC_PROTOCOL_MINOR);
        return;
    }
    ...
    for (i = msg.ReadByte(); i < MAX_ASYNC_CLIENTS; i = msg.ReadByte())
    {
        ...
        if (verbose)
        {
            common->Printf("client %2d: %s, ping = %d, rate = %d\n", i, serverInfo.nickname[serverInfo.clients],
                           serverInfo.pings[serverInfo.clients], serverInfo.rate[serverInfo.clients]);
        }
        serverInfo.clients++;
    }
    index = serverList.InfoResponse(serverInfo);

    common->Printf("%d: server %s - protocol %d.%d - %s\n", index, Sys_NetAdrToString(serverInfo.adr), protocol >> 16,
...
void idAsyncClient::ProcessPrintMessage(const netadr_t from, const idBitMsg &msg)
{
    ...
    if (opcode == SERVER_PRINT_GAMEDENY)
    {
        ...
        {
            retpass = session->MessageBox(MSG_PROMPT, common->GetLanguageDict()->GetString("#str_04321"), string, true,
                                          "passprompt_ok");
            ClearPendingPackets();
    ...
    if (authMsg == AUTHKEY_BADKEY)
    {
        ...
        if (clientState == CS_CONNECTING)
        {
            while (1)
            {
                ...
                retkey =
                    session->MessageBox(MSG_CDKEY, auth_msg, common->GetLanguageDict()->GetString("#str_04325"), true);
                if (retkey)
                {
                    ...
                    else
                    {
                        ...
                        idAsyncNetwork::BuildInvalidKeyMsg(auth_msg, valid);
                        session->MessageBox(MSG_OK, auth_msg.c_str(),
...
bool idAsyncClient::ValidatePureServerChecksums(const netadr_t from, const idBitMsg &msg)
{
    ...
    case PURE_MISSING: {
        ...
        if (idAsyncNetwork::clientDownload.GetInteger() == 0)
        {
            ...
            {
                message +=
                    va(common->GetLanguageDict()->GetString("#str_06751"), numMissingChecksums, checksums.c_str());
            }
...
void idAsyncClient::ConnectionlessMessage(const netadr_t from, const idBitMsg &msg)
{
    ...
    if (!Sys_CompareNetAdrBase(from, serverAddress) &&
        (lastRconTime + 10000 < realTime || !Sys_CompareNetAdrBase(from, lastRconAddress)))
    {
...
void idAsyncClient::RunFrame(void)
{
    ...
    do
    {
        ...
        do
        {
            ...
            newPacket = clientPort.GetPacketBlocking(from, msgBuf, size, sizeof(msgBuf),
    ...
    while (gameTimeResidual + clientPredictTime >= USERCMD_MSEC)
    {
        ...
        while (snapshotGameFrame < gameFrame)
        {
            ...
            bool lastPredictFrame =
                (snapshotGameFrame + 1 >= gameFrame && gameTimeResidual + clientPredictTime < USERCMD_MSEC);
...
void idAsyncClient::HandleDownloads(void)
{
    ...
    if (updateState == UPDATE_SENT && clientTime > updateSentTime + 2000)
    {
        ...
        {
            session->MessageBox(MSG_OK, common->GetLanguageDict()->GetString("#str_04839"),
    ...
    else if (backgroundDownload.completed)
    {
        ...
        {
            //
            if (session->MessageBox(MSG_YESNO, updateMSG, common->GetLanguageDict()->GetString("#str_04330"), true,
                                    "yes")[0])
            {
                ...
                else
                {
                    ...
                    if (backgroundDownload.url.status == DL_DONE)
                    {
                        ...
                        if (session->MessageBox(MSG_YESNO, common->GetLanguageDict()->GetString("#str_04331"),
                                                common->GetLanguageDict()->GetString("#str_04332"), true, "yes")[0])
                        {
                        ...
                        {
                            session->MessageBox(
                                MSG_OK, va(common->GetLanguageDict()->GetString("#str_04333"), fullPath.c_str()),
                    ...
                    else
                    {
                        ...
                        session->MessageBox(MSG_OK, common->GetLanguageDict()->GetString("#str_04335"),
            ...
            do
            {
                ...
                if (backgroundDownload.url.status == DL_DONE)
                {
                    ...
                    if (!checksum || checksum != dlList[0].checksum)
                    {
                        ...
                        session->MessageBox(
                            MSG_OK,
                            va(common->GetLanguageDict()->GetString("#str_07214"), checksum, dlList[0].checksum),
                ...
                else
                {
                    ...
                    // "Download failed"
                    session->MessageBox(MSG_OK, common->GetLanguageDict()->GetString("#str_07215"),
...
bool idAsyncClient::CheckTimeout(void)
{
    ...
    {
        session->StopBox();
        session->MessageBox(MSG_OK, common->GetLanguageDict()->GetString("#str_04328"),
    ...
    else if (infoType == SERVER_DL_LIST)
    {
        ...
        do
        {
            ...
            if (pakDl == SERVER_PAK_YES)
            {
                ...
                dlList.Append(entry);
                common->Printf("download %s from %s ( 0x%x )\n", entry.filename.c_str(), entry.url.c_str(),
                               entry.checksum);
            }
        ...
        if (totalDlSize == 0)
        {
            ...
            dlList.Clear();
            session->MessageBox(MSG_OK, common->GetLanguageDict()->GetString("#str_07219"),
