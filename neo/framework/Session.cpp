...
bool idSessionLocal::MaybeWaitOnCDKey(void)
{
    ...
    {
        authWaitBox = true;
        sessLocal.MessageBox(MSG_WAIT, common->GetLanguageDict()->GetString("#str_07191"), NULL, true, NULL, NULL,
                             true);
        return true;
    }
    return false;
...
static void Session_PromptKey_f(const idCmdArgs &args)
{
    ...
    do
    {
        ...
        retkey = sessLocal.MessageBox(MSG_CDKEY, prompt_msg, common->GetLanguageDict()->GetString("#str_04305"), true,
                                      NULL, NULL, true);
        if (retkey)
        {
...
idSessionLocal::idSessionLocal()
{
    guiInGame = guiMainMenu = guiIntro = guiRestartMenu = guiLoading = guiGameOver = guiActive = guiTest = guiMsg =
        guiMsgRestore = guiTakeNotes = NULL;
...
void idSessionLocal::EndAVICapture()
{
    ...
    idFile *f =
        fileSystem->OpenFileWrite(va("demos/%s/%s.roqParam", aviDemoShortName.c_str(), aviDemoShortName.c_str()));
...
#else
        const char *people[] = {"Tim",   "Kenneth",   "Robert", "Matt",  "Mal",  "Jerry",  "Steve",  "Pat",
                                "Xian",  "Ed",        "Fred",   "James", "Eric", "Andy",   "Seneca", "Patrick",
                                "Kevin", "MrElusive", "Jim",    "Brian", "John", "Adrian", "Nobody"};
#endif
...
bool idSessionLocal::SaveGame(const char *saveName, bool autosave, const char *saveFileName)
{
    ...
    const idDeclEntityDef *mapDef =
        static_cast<const idDeclEntityDef *>(declManager->FindType(DECL_MAPDEF, mapName, false));
    if (mapDef)
    {
        mapName = common->GetLanguageDict()->GetString(mapDef->dict.GetString("name", mapName));
    }
...
void idSessionLocal::Init()
{
    ...
    cmdSystem->AddCommand("testmap", Session_TestMap_f, CMD_FL_SYSTEM, "tests a map",
                          idCmdSystem::ArgCompletion_MapName);
