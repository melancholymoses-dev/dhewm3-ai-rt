    ...
    while (1)
    {
        ...
        if (!token.Icmp("distribution"))
        {
            ...
            ParseParms(src, stage->distributionParms,
                       sizeof(stage->distributionParms) / sizeof(stage->distributionParms[0]));
            continue;
        }
        ...
        { // #3878 soft particles
            common->Warning("Particle %s from %s has stage with \"softeningRadius\" attribute, "
                            "which is currently ignored (we soften all suitable particles)\n",
...
bool idDeclParticle::RebuildTextSource(void)
{
    ...
    f.WriteFloatString("\n\n/*\n"
                       ...
                       "\tTo use the particle editor, launch the game and type "
                       "'editParticles' on the console.\n"
                       "*/\n");
    ...
    if (customPathType == PPATH_STANDARD)
    {
        ...
        case PDIST_CYLINDER: { // ( sizeX sizeY sizeZ ringFraction )
            ...
            if (distributionParms[3] > 0.0f)
            {
                ...
                {
                    // if we are inside the inner reject zone, rescale to put it out into
                    // the good zone
...
void idParticleStage::ParticleColors(particleGen_t *g, idDrawVert *verts) const
{
    ...
    for (int i = 0; i < 4; i++)
    {
        float fcolor = ((entityColor) ? g->renderEnt->shaderParms[i] : color[i]) * fadeFraction +
