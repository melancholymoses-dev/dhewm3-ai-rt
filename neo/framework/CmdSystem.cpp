...
class idCmdSystemLocal : public idCmdSystem
{
  public:
    ...
    virtual void ArgCompletion_FolderExtension(const idCmdArgs &args, void (*callback)(const char *s),
                                               const char *folder, bool stripFolder, ...);
...
template <> ID_INLINE int idListSortCompare(const commandDef_t *const *a, const commandDef_t *const *b)
{
    ...
    for (prev = &commands; *prev; prev = &cmd->next)
    {
        ...
        if (idStr::Icmp(args.Argv(0), cmd->name) == 0)
        {
            ...
            *prev = cmd->next;
            ...
            if ((cmd->flags & (CMD_FL_CHEAT | CMD_FL_TOOL)) && session && session->IsMultiplayer() &&
...
============
*/
void idCmdSystemLocal::ArgCompletion_FolderExtension(const idCmdArgs &args, void (*callback)(const char *s),
                                                     const char *folder, bool stripFolder, ...)
{
    int i;
    idStr string;
