...
const char *GetLocalizedString(const char *id, const char *fallback)
{
    ...
    if (ret == nullptr || ret[0] == '\0' || (ret[0] == '#' && idStr::Cmpn(ret, STRTABLE_ID, STRTABLE_ID_LENGTH) == 0))
    {
        ret = fallback;
    }
    return ret;
...
static idCVar imgui_numBindingColumns("imgui_numBindingColumns", "3", CVAR_ARCHIVE | CVAR_SYSTEM | CVAR_INTEGER,
                                      "Number of columns with bindings in Dhewm3SettingsMenu's Bindings tab", 1, 10);
...
static const char *GetGamepadBindNowButtonName()
{                                                       // TODO: rename to confirm or sth
...
struct BindingEntry
{
    ...
    BindingEntry(const BindingEntryTemplate &bet) : command(bet.command), description(bet.description)
    {
    ...
    bool DrawAllBindingsWindow(/* in+out */ BindingEntrySelectionState &selState, bool newOpen, const ImVec2 &btnMin,
                               const ImVec2 &btnMax)
    {
        ...
        if (ImGui::Begin(menuWinTitle, &showThisMenu, menuWinFlags))
        {
            ...
            if (numBindings > 0 && ImGui::BeginTable("AllBindingsForCommand", 2, tableFlags))
            {
                ...
                for (int bnd = 0; bnd < numBindings; ++bnd)
                {
                    ...
                    if (colHasBinding)
                    {
                        ...
                        {
                            ImGui::SetItemTooltip("Unbind '%s' and bind another key to %s", keyName,
                                                  displayName.c_str());
                        }
                        ImGui::SameLine();
            ...
            {
                ImGui::SetItemTooltip("Add %s keybinding for %s", (numBindings == 0) ? "a" : "another",
                                      displayName.c_str());
            }
            ...
            if (!workRect.Contains(winRect))
            {
                ...
                ImVec2 newWinPos = ImGui::FindBestWindowPosForPopupEx(
                    ImVec2(btnMin.x, btnMax.y), winSize, &dir, workRect, r_avoid, ImGuiPopupPositionPolicy_Default);
    ...
    BindingEntrySelectionState HandleClearPopup(const char *popupName, bool newOpen)
    {
        ...
        if (ImGui::BeginPopupModal(popupName, NULL, ImGuiWindowFlags_AlwaysAutoResize))
        {
            ...
            {
                ImGui::Text("Unbind key '%s' from command %s ?", bindings[selectedBinding].keyName.c_str(),
                            displayName.c_str());
            }

            ImGui::NewLine();
            ImGui::Text("Press Enter (or %s) to confirm, or\nEscape (or %s) to cancel.", GetGamepadBindNowButtonName(),
                        GetGamepadCancelButtonNames());
            ImGui::NewLine();
    ...
    BindingEntrySelectionState HandleBindPopup(const char *popupName, bool newOpen)
    {
        ...
        if (ImGui::BeginPopupModal(popupName, NULL, ImGuiWindowFlags_AlwaysAutoResize))
        {
            if (selectedBinding < 0 || selectedBinding >= bindings.Num() || bindings[selectedBinding].keyNum == -1)
            {
                // add a binding
    ...
    BindingEntrySelectionState HandleRebindPopup(const char *popupName, bool newOpen)
    {
        ...
        if (ImGui::BeginPopupModal(popupName, NULL, ImGuiWindowFlags_AlwaysAutoResize))
        {
            ...
            {
                ImGui::Text("Key '%s' is already bound to command %s !\nBind to %s instead?", keyName,
                            rebindOtherEntry->displayName.c_str(), displayName.c_str());
            }
            else
            {
            ...
            ImGui::NewLine();
            ImGui::Text("Press Enter (or %s) to confirm,\nor Escape (or %s) to cancel.", GetGamepadBindNowButtonName(),
                        GetGamepadCancelButtonNames());
            ImGui::NewLine();
...
static void DrawBindingsMenu()
{
    ...
    { // TODO: could go into some init function
...
struct CVarOption
{
    ...
    CVarOption(const char *_name, DrawCallback drawCB) : name(_name), drawCallback(drawCB), type(OT_CUSTOM)
    {
    }
...
static CVarOption videoOptionsImmediately[] = {
    ...
    CVarOption("image_anisotropy",
               [](idCVar &cvar) {
                   ...
                   if (glConfig.maxTextureAnisotropy > 1)
                   {
                       ...
                       ImGui::SliderInt("Anisotropic Filtering", &texAni, 1, glConfig.maxTextureAnisotropy, fmtStr,
    ...
    CVarOption("r_enableDepthCapture",
               [](idCVar &cvar) {
...
static bool VideoHasResettableChanges()
{
    ...
    if (curMode != initialMode)
        return true;
    if (curMode == -1 &&
        (initialCustomVidRes[0] != r_customWidth.GetInteger() || initialCustomVidRes[1] != r_customHeight.GetInteger()))
    {
        return true;
    }
    if (r_fullscreen.GetBool() != initialFullscreen || r_fullscreenDesktop.GetBool() != initialFullscreenDesktop)
    {
        return true;
    }
...
static void DrawVideoOptionsMenu()
{
    ...
    const char *resLabel = (fullscreenChoice && fullscreenDesktop)
                               ? "Window Resolution (ignored for Fullscreen Desktop Mode!)###resMode"
                               : "Resolution###resMode";
...
static void DrawRTOptionsMenu()
{
    ...
    if (ImGui::BeginTable("##FeatureCols", 2, ImGuiTableFlags_None))
    {
        ImGui::TableNextColumn();
    ...
    if (ImGui::BeginTable("##ReflCols", 2, ImGuiTableFlags_None))
    {
        ImGui::TableNextColumn();
    ...
    RTSliderFloat("Probe Alpha (1=full history, 0=current only)", rtCVars.rtGIProbeHysteresis, 0.0f, 1.0f, "%.3f");
    ...
    if (ImGui::BeginTable("##volCols", 2, ImGuiTableFlags_None))
    {
        ...
        ImGui::TableNextColumn();
        ImGui::TextDisabled("General");
        RTCheckbox("Froxel Volumetrics", rtCVars.rtVolFroxel);
...
static void DrawAudioOptionsMenu()
{
    ...
    if (idSoundSystemLocal::useEFXReverb)
    {
        ...
        if (ImGui::SliderFloat("Strength of EFX Reverb Effects", &reverbGain, 0, 1, "%.2f",
                               ImGuiSliderFlags_AlwaysClamp))
        {
...
{
    if (data->EventChar > 0xFF)
    { // some unicode char that ISO8859-1 can't represent
        data->EventChar = 0;
        return 1;
    }
...
static void DrawOtherOptionsMenu()
{
    ...
    if (ImGui::Combo("ImGui Style", &style_idx, "dhewm3\0ImGui Default\0Userstyle\0"))
    {
        ...
        {
        case 0:
            D3::ImGuiHooks::SetImGuiStyle(D3::ImGuiHooks::Style::Dhewm3);
            break;
        case 1:
            D3::ImGuiHooks::SetImGuiStyle(D3::ImGuiHooks::Style::ImGui_Default);
            break;
        case 2:
            D3::ImGuiHooks::SetImGuiStyle(D3::ImGuiHooks::Style::User);
            break;
        }
