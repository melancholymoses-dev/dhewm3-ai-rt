...
class idFileSystemLocal : public idFileSystem
{
  public:
    ...
    virtual idFile *OpenFileReadFlags(const char *relativePath, int searchFlags, pack_t **foundInPak = NULL,
                                      bool allowCopyFiles = true, const char *gamedir = NULL);
  ...
  private:
    ...
    int GetFileList(const char *relativePath, const idStrList &extensions, idStrList &list, idHashIndex &hashIndex,
                    bool fullRelativePath, const char *gamedir = NULL);

    int GetFileListTree(const char *relativePath, const idStrList &extensions, idStrList &list, idHashIndex &hashIndex,
                        const char *gamedir = NULL);
...
idCVar idFileSystemLocal::fs_copyfiles("fs_copyfiles", "0", CVAR_SYSTEM | CVAR_INIT | CVAR_INTEGER, "", 0, 4,
...
#endif
idCVar idFileSystemLocal::fs_searchAddons("fs_searchAddons", "0", CVAR_SYSTEM | CVAR_BOOL,
...
int idFileSystemLocal::GetFileList(const char *relativePath, const idStrList &extensions, idStrList &list,
                                   idHashIndex &hashIndex, bool fullRelativePath, const char *gamedir)
{
    searchpath_t *search;
    fileInPack_t *buildBuffer;
    int i, j;
...
idFileList *idFileSystemLocal::ListFiles(const char *relativePath, const char *extension, bool sort,
                                         bool fullRelativePath, const char *gamedir)
{
...
===============
*/
int idFileSystemLocal::GetFileListTree(const char *relativePath, const idStrList &extensions, idStrList &list,
                                       idHashIndex &hashIndex, const char *gamedir)
{
    int i;
...
void idFileSystemLocal::Path_f(const idCmdArgs &args)
{
    ...
    for (sp = fileSystemLocal.searchPaths; sp; sp = sp->next)
    {
        if (sp->pack)
        {
            ...
            {
                sprintf(status, "%s (%i files - 0x%x %s", sp->pack->pakFilename.c_str(), sp->pack->numfiles,
                        sp->pack->checksum, sp->pack->referenced ? "referenced" : "not referenced");
                if (sp->pack->addon)
                {
                    status += " - addon)\n";
                }
        ...
        if (deppak)
        {
            ...
            if (!deppak->addon_search)
            {
                ...
                common->Printf("Addon pk4 %s 0x%x depends on pak %s 0x%x, will be searched\n", pak->pakFilename.c_str(),
                               pak->checksum, deppak->pakFilename.c_str(), deppak->checksum);
...
bool idFileSystemLocal::FileAllowedFromDir(const char *path)
{
    ...
    if (strstr(path, "guis/assets/splash/addon") == path && !strcmp(path + l - 4, ".tga"))
    {
        return true;
    }
...
idFile_InZip *idFileSystemLocal::ReadFileFromZip(pack_t *pak, fileInPack_t *pakFile, const char *relativePath)
{
    ...
    if (err != UNZ_OK)
    {
        common->FatalError("Couldn't get file info for %s in %s, pos %llu", relativePath, pak->pakFilename.c_str(),
                           pakFile->pos);
    }
...
===========
*/
idFile *idFileSystemLocal::OpenFileReadFlags(const char *relativePath, int searchFlags, pack_t **foundInPak,
                                             bool allowCopyFiles, const char *gamedir)
{
    searchpath_t *search;
    idStr netpath;
    pack_t *pak;
