...
================
*/
bool idAFVector::Finish(const char *fileName, const getJointTransform_t GetJointTransform, const idJointMat *frame,
                        void *model) const
{
    idMat3 axis;
    idVec3 start, end;
...
bool idDeclAF::WriteBody(idFile *f, const idDeclAF_Body &body) const
{
    ...
    if (body.inertiaScale != mat3_identity)
    {
        ...
        f->WriteFloatString("\tinertiaScale (%f %f %f %f %f %f %f %f %f)\n", ic[0][0], ic[0][1], ic[0][2], ic[1][0],
                            ic[1][1], ic[1][2], ic[2][0], ic[2][1], ic[2][2]);
    }
...
bool idDeclAF::WriteSettings(idFile *f) const
{
    ...
    f->WriteFloatString("\tfriction %f, %f, %f, %f\n", defaultLinearFriction, defaultAngularFriction,
                        defaultContactFriction, defaultConstraintFriction);
    f->WriteFloatString("\tsuspendSpeed %f, %f, %f, %f\n", suspendVelocity[0], suspendVelocity[1],
                        suspendAcceleration[0], suspendAcceleration[1]);
...
bool idDeclAF::RebuildTextSource(void)
{
    ...
    f.WriteFloatString("\n\n/*\n"
                       ...
                       "\tDo not edit directly but launch the game and type "
                       "'editAFs' on the console.\n"
                       "*/\n");
    ...
    while (src.ReadToken(&token))
    {
        ...
        if (!token.Icmp("model"))
        {
            ...
            if (!token.Icmp("box"))
            {
                ...
                if (!src.ExpectTokenString("(") || !body->v1.Parse(src) || !src.ExpectTokenString(",") ||
                    !body->v2.Parse(src) || !src.ExpectTokenString(")"))
                {
                    return false;
                }
            }
