...
static void loadMod(const idStr &modName)
{
    ...
    static const char *d3xpMods[] = {// TODO: if there are more mods that need d3xp as base
            ...
            if (!vcmd.Icmp("eax"))
            {
                if (cvarSystem->GetCVarBool("s_useEAXReverb"))
                {
                    ...
                    case -1:
                        ...
                        // disabled
                        MessageBox(MSG_OK, common->GetLanguageDict()->GetString("#str_07233"),
                                   common->GetLanguageDict()->GetString("#str_07231"), true);
                        break;
                    case 0:
                        cvarSystem->SetCVarBool("s_useEAXReverb", false);
                        // not available
                        MessageBox(MSG_OK, common->GetLanguageDict()->GetString("#str_07232"),
                                   common->GetLanguageDict()->GetString("#str_07231"), true);
                        break;
                    }
                }
...
=================
*/
const char *idSessionLocal::MessageBox(msgBoxType_t type, const char *message, const char *title, bool wait,
                                       const char *fire_yes, const char *fire_no, bool network)
{
    ...
    if (wait)
    {
        ...
        else if (type == MSG_CDKEY)
        {
            if (msgRetIndex == 0)
            {
                ...
                sprintf(msgFireBack[0], "%1s;%16s;%2s;%1s;%16s;%2s", guiMsg->State().GetString("visible_cdchk"),
                        guiMsg->State().GetString("str_cdkey"), guiMsg->State().GetString("str_cdchk"),
                        guiMsg->State().GetString("visible_xpchk"), guiMsg->State().GetString("str_xpkey"),
...
=================
*/
void idSessionLocal::DownloadProgressBox(backgroundDownload_t *bgl, const char *title, int progress_start,
                                         int progress_end)
{
            ...
            else if (bgl->url.dltotal != dltotal || bgl->url.dlnow != dlnow)
            {
                ...
                if (dltotal)
                {
                    guiMsg->SetStateString(
                        "progress", va("%d", progress_start + dlnow * (progress_end - progress_start) / dltotal));
                }
                else
                {
    ...
    if (idStr::Icmp(menuCommand, "note") == 0 && mapSpawned)
    {
        ...
        if (extended)
        {
            ...
            const idDecl *mapDecl =
                declManager->FindType(DECL_ENTITYDEF, mapSpawnData.serverInfo.GetString("si_map"), false);
        ...
        for (i = 0; i < fileList.Num(); i++)
        {
            ...
            UpdateScreen();
            renderSystem->TakeScreenshot(renderSystem->GetScreenWidth(), renderSystem->GetScreenHeight(), shotName, 1,
                                         NULL);
        }
        noteNumber++;
...
void idSessionLocal::SetCDKeyGuiVars(void)
{
    ...
    guiMainMenu->SetStateString("str_d3key_state",
                                common->GetLanguageDict()->GetString(va("#str_071%d", 86 + cdkey_state)));
    guiMainMenu->SetStateString("str_xpkey_state",
