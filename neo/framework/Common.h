...
extern bool com_debuggerSupported; // only set to true when the updateDebugger function
...
class idCommon
{
  public:
    ...
    /* Some Mods (like Ruiner and DarkMod when it still was a mod) used
     * "SourceHook" to override Doom3 Methods to call their own code before the
     * original method was executed.. this is super ugly and probably not super
     * portable either.
     *
     * So let's offer something that's slightly less ugly: A function pointer
     * based interface to provide similar (but known!) hacks. For example, Ruiner
     * used SourceHook to intercept idCmdSystem::BufferCommandText() and recreate
     * some cooked rendering data in case reloadImages or vid_restart was
     * executed. Now, instead of doing ugly hacks with SourceHook, Ruiner can just
     * call common->SetCallback( idCommon::CB_ReloadImages,
     *                        (idCommon::FunctionPointer)functionToCall,
     *                        (void*)argForFunctionToCall );
     *
     * (the Mod needs to check if SetCallback() returned true; if it didn't the
     * used version of dhewm3 doesn't support the given CallBackType and the Mod
     * must either error out or handle the case that the callback doesn't work)
     *
     * Of course this means that for every new SourceHook hack a Mod (that's
     * ported to dhewm3) uses, a corresponding entry must be added to enum
     * CallbackType and it must be handled, which implies that the Mod will only
     * properly work with the latest dhewm3 git code or the next release.. I guess
     * most mods don't need this hack though, so I think it's feasible.
     *
     * Note that this allows adding new types of callbacks without breaking the
     * API and ABI between dhewm3 and the Game DLLs; the alternative would be
     * something like idCommon::RegisterReloadImagesCallback(), and maybe other
     * similar methods later, which would break the ABI and API each time and all
     * Mods would have to be adjusted, even if they don't even need that
     * functionality (because they never needed SourceHook or similar).
     *
     * Similar to SetCallback() I've also added GetAdditionalFunction() to get a
     * function pointer from dhewm3 that Mods can call (and that's not exported
     * via the normal interface classes). Right now it's only used for a Doom3
     * Demo specific hack only relevant for base.dll (not for Mods)
     */

    typedef void *(*FunctionPointer)(void *); // needs to be cast to/from real type!
    enum CallbackType
    {
        // called on reloadImages and vid_restart commands (before anything "real"
        // happens)
        // expecting callback to be like void cb(void* userarg, const idCmdArgs&
        // cmdArgs)
    ...
    enum FunctionType
    {
        ...
        FT_IsDemo = 1,
        // the function's signature is bool fn(idInterpreter,idProgram,int) with
        // arguments:
