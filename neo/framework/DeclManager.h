...
template <class type> ID_INLINE idDecl *idDeclAllocator(void)
{
    return new type;
}
...
template <declType_t type> ID_INLINE void idListDecls_f(const idCmdArgs &args)
{
    declManager->ListType(args, type);
}

template <declType_t type> ID_INLINE void idPrintDecls_f(const idCmdArgs &args)
{
