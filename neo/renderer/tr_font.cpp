...
============
*/
glyphInfo_t *RE_ConstructGlyphInfo(unsigned char *imageOut, int *xOut, int *yOut, int *maxHeight, FT_Face face,
...
bool idRenderSystemLocal::RegisterFont(const char *fontName, fontInfoEx_t &font)
{
    ...
    */
    ...
    for (fontCount = 0; fontCount < 3; fontCount++)
    {
        ...
        {
            common->Warning("RegisterFont: couldn't find font: '%s' (len=%d, expected=%d)", name, len,
                            (int)FILESIZE_fontInfo_t);
            return false;
        }
