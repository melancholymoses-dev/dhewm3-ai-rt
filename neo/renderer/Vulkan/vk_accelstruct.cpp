...
static void AllocASBuffer( VkDeviceSize size, VkBufferUsageFlags extraUsage,
...
vkBLAS_t *VK_RT_BuildBLAS( const srfTriangles_t *tri ) {
...

	{
		...
		VkMemoryRequirements mr; vkGetBufferMemoryRequirements( vk.device, vertBuf, &mr );
		...
		VkMemoryAllocateInfo ai = {}; ai.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO; ai.pNext = &fi; ai.allocationSize = mr.size; ai.memoryTypeIndex = VK_FindMemoryType( mr.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT );
		...
		ai.allocationSize = mr.size; ai.memoryTypeIndex = VK_FindMemoryType( mr.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT );
	...
	// Upload vertex/index data using staging
	{
		...
		VkBufferCreateInfo bi = {}; bi.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO; bi.size = vertexDataSize; bi.usage = VK_BUFFER_USAGE_TRANSFER_SRC_BIT; bi.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
		VK_CHECK( vkCreateBuffer( vk.device, &bi, NULL, &stagingV ) );
		VkMemoryRequirements mr; vkGetBufferMemoryRequirements( vk.device, stagingV, &mr );
		VkMemoryAllocateInfo ai = {}; ai.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO; ai.allocationSize = mr.size; ai.memoryTypeIndex = VK_FindMemoryType( mr.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT );
		VK_CHECK( vkAllocateMemory( vk.device, &ai, NULL, &stagingVMem ) );
		VK_CHECK( vkBindBufferMemory( vk.device, stagingV, stagingVMem, 0 ) );
		void *ptr; VK_CHECK( vkMapMemory( vk.device, stagingVMem, 0, vertexDataSize, 0, &ptr ) );
		memcpy( ptr, verts, (size_t)vertexDataSize ); vkUnmapMemory( vk.device, stagingVMem );
		VkCommandBuffer cmd = VK_BeginSingleTimeCommands();
		VkBufferCopy region = { 0, 0, vertexDataSize }; vkCmdCopyBuffer( cmd, stagingV, vertBuf, 1, &region );
		VK_EndSingleTimeCommands( cmd );
		vkDestroyBuffer( vk.device, stagingV, NULL ); vkFreeMemory( vk.device, stagingVMem, NULL );
		...
		ai.allocationSize = mr.size; ai.memoryTypeIndex = VK_FindMemoryType( mr.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT );
		...
		memcpy( ptr, indexes, (size_t)indexDataSize ); vkUnmapMemory( vk.device, stagingIMem );
		cmd = VK_BeginSingleTimeCommands();
		region = { 0, 0, indexDataSize }; vkCmdCopyBuffer( cmd, stagingI, idxBuf, 1, &region );
		VK_EndSingleTimeCommands( cmd );
		vkDestroyBuffer( vk.device, stagingI, NULL ); vkFreeMemory( vk.device, stagingIMem, NULL );
	}
	...
	AllocASBuffer( sizeInfo.buildScratchSize,
...
void VK_RT_RebuildTLAS( VkCommandBuffer cmd, const viewDef_t *viewDef ) {
	...
	for ( const viewEntity_t *vEntity = viewDef->viewEntitys;
	      vEntity != NULL && instanceCount < 4096;
	      vEntity = vEntity->next )
	{
		...
		if ( !surf || !surf->geometry || surf->geometry->numVerts == 0 ) continue;
		...
		if ( ent->dynamicModel ) needRebuild = true;
		...
		if ( !ent->blas || !ent->blas->isValid ) continue;
		...
		inst.transform.matrix[0][0] = m[0];  inst.transform.matrix[0][1] = m[4];  inst.transform.matrix[0][2] = m[8];  inst.transform.matrix[0][3] = m[12];
		inst.transform.matrix[1][0] = m[1];  inst.transform.matrix[1][1] = m[5];  inst.transform.matrix[1][2] = m[9];  inst.transform.matrix[1][3] = m[13];
		inst.transform.matrix[2][0] = m[2];  inst.transform.matrix[2][1] = m[6];  inst.transform.matrix[2][2] = m[10]; inst.transform.matrix[2][3] = m[14];
	...
	// Instance buffer must be host-visible so we can fill it each frame
	{
		...
		VkMemoryRequirements mr; vkGetBufferMemoryRequirements( vk.device, tlas.instanceBuffer, &mr );
		VkMemoryAllocateFlagsInfo fi = {}; fi.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_FLAGS_INFO; fi.flags = VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_BIT;
		VkMemoryAllocateInfo ai = {}; ai.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO; ai.pNext = &fi; ai.allocationSize = mr.size; ai.memoryTypeIndex = VK_FindMemoryType( mr.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT );
	...
	AllocASBuffer( sizeInfo.buildScratchSize, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, &tlas.scratchBuffer, &tlas.scratchMemory );
...
void VK_RT_Shutdown( void ) {
	...
	for ( int i = 0; i < VK_MAX_FRAMES_IN_FLIGHT; i++ ) {
		...
		if ( sm.view   != VK_NULL_HANDLE ) vkDestroyImageView( vk.device, sm.view,   NULL );
		if ( sm.image  != VK_NULL_HANDLE ) vkDestroyImage(     vk.device, sm.image,  NULL );
		if ( sm.memory != VK_NULL_HANDLE ) vkFreeMemory(       vk.device, sm.memory, NULL );
	}
...
void VK_RT_ResizeShadowMask( uint32_t width, uint32_t height ) {
	...
	for ( int i = 0; i < VK_MAX_FRAMES_IN_FLIGHT; i++ ) {
		vkShadowMask_t &sm = vkRT.shadowMask[i];
		if ( sm.view   != VK_NULL_HANDLE ) { vkDestroyImageView( vk.device, sm.view,   NULL ); sm.view   = VK_NULL_HANDLE; }
		if ( sm.image  != VK_NULL_HANDLE ) { vkDestroyImage(     vk.device, sm.image,  NULL ); sm.image  = VK_NULL_HANDLE; }
		if ( sm.memory != VK_NULL_HANDLE ) { vkFreeMemory(       vk.device, sm.memory, NULL ); sm.memory = VK_NULL_HANDLE; }
		...
		VkCommandBuffer cmd = VK_BeginSingleTimeCommands();
		VK_TransitionImageLayout( cmd, sm.image,
