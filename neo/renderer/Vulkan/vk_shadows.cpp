...
extern void VK_CreateBuffer( VkDeviceSize size,
...
static void VK_RT_InitShadowPipeline( void ) {
	...
	VK_CHECK( vkCreatePipelineLayout( vk.device, &plInfo, NULL,
	                                  &vkRT.shadowPipelineLayout ) );
	...
	VK_CHECK( vkCreateRayTracingPipelinesKHR( vk.device, VK_NULL_HANDLE, VK_NULL_HANDLE,
	                                          1, &rtPipeInfo, NULL, &vkRT.shadowPipeline ) );
	...
	VK_CreateBuffer( sbtSize,
	               ...
	               | VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
	                 ...
	                 &vkRT.sbtBuffer, &vkRT.sbtMemory );

	// Retrieve handles
	uint8_t *handles = (uint8_t *)alloca( 3 * handleSize );
	VK_CHECK( vkGetRayTracingShaderGroupHandlesKHR( vk.device, vkRT.shadowPipeline,
	                                                0, 3, 3 * handleSize, handles ) );
...
void VK_RT_DispatchShadowRays( VkCommandBuffer cmd, const viewDef_t *viewDef ) {
	if ( !vkRT.isInitialized || !vkRT.tlas.isValid ) return;
	if ( !r_rtShadows.GetBool() ) return;

	uint32_t frameIdx = vk.currentFrame;
	vkShadowMask_t &sm = vkRT.shadowMask[frameIdx];

	if ( sm.image == VK_NULL_HANDLE ) return;
	      ...
	      vLight = vLight->next )
	{
		...
		VK_CreateBuffer( sizeof(ShadowParamsUBO),
		...
		vkCmdBindDescriptorSets( cmd, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR,
		                         vkRT.shadowPipelineLayout, 0, 1, &ds, 0, NULL );
