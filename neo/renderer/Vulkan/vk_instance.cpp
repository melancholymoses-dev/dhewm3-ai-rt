...
static bool VK_FindQueueFamilies( VkPhysicalDevice physDev, uint32_t *outGraphics, uint32_t *outPresent ) {
	...
	*outPresent  = UINT32_MAX;
	...
	for ( uint32_t i = 0; i < count; i++ ) {
		...
		if ( *outGraphics != UINT32_MAX && *outPresent != UINT32_MAX ) break;
	}
...
static int VK_ScoreDevice( VkPhysicalDevice physDev, bool *outRTSupport ) {
	...
	uint32_t gfx, pres;
	if ( !VK_FindQueueFamilies( physDev, &gfx, &pres ) ) return -1;
	if ( !VK_CheckDeviceExtensionSupport( physDev, deviceExtensions, numDeviceExtensions ) ) return -1;
