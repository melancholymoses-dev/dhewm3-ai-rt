...
static VkSamplerAddressMode MapRepeat( textureRepeat_t r ) {
	switch ( r ) {
		case TR_CLAMP:               return VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE;
		case TR_CLAMP_TO_ZERO:       return VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_BORDER;
		case TR_CLAMP_TO_ZERO_ALPHA: return VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_BORDER;
...
void VK_Image_Init( void ) {
	...
	bi.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO; bi.size = 4;
	bi.usage = VK_BUFFER_USAGE_TRANSFER_SRC_BIT; bi.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
	VK_CHECK( vkCreateBuffer( vk.device, &bi, NULL, &stagingBuf ) );
	VkMemoryRequirements smr; vkGetBufferMemoryRequirements( vk.device, stagingBuf, &smr );
	VkMemoryAllocateInfo sai = {}; sai.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
	sai.allocationSize = smr.size;
	sai.memoryTypeIndex = VK_FindMemoryType( smr.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT );
	VK_CHECK( vkAllocateMemory( vk.device, &sai, NULL, &stagingMem ) );
	VK_CHECK( vkBindBufferMemory( vk.device, stagingBuf, stagingMem, 0 ) );
	void *ptr; VK_CHECK( vkMapMemory( vk.device, stagingMem, 0, 4, 0, &ptr ) );
	memcpy( ptr, white, 4 ); vkUnmapMemory( vk.device, stagingMem );

	VkCommandBuffer cmd = VK_BeginSingleTimeCommands();
	VK_TransitionImageLayout( cmd, s_fallback.image,
	                          VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
	                          VK_IMAGE_ASPECT_COLOR_BIT );
	VkBufferImageCopy rgn = {}; rgn.imageSubresource = { VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, 1 };
	rgn.imageExtent = { 1, 1, 1 };
	vkCmdCopyBufferToImage( cmd, stagingBuf, s_fallback.image,
	...
	vci.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO; vci.image = s_fallback.image;
	vci.viewType = VK_IMAGE_VIEW_TYPE_2D; vci.format = fmt;
...
void VK_Image_Upload( idImage *img, const byte *pic, int width, int height ) {
	if ( !vk.isInitialized || !pic || width <= 0 || height <= 0 ) return;
	...
	for ( uint32_t i = 1; i < mipLevels; i++ ) {
		...
		vkCmdBlitImage( cmd,
		                vkd->image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
		                vkd->image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
...
bool VK_Image_GetDescriptorInfo( idImage *img, VkDescriptorImageInfo *out ) {
	if ( !img || !img->vkData ) return false;
	vkImageData_t *vkd = img->vkData;
