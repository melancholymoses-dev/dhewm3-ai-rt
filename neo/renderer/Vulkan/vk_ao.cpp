...
dhewm3 Vulkan ray tracing - ambient occlusion ray pipeline and dispatch.
...
static void VK_RT_InitAOPipeline(void)
{
    ...
    VK_CreateBuffer(sbtSize,
                    ...
                    &vkRT.sbtAOBuffer, &vkRT.sbtAOMemory);
...
void VK_RT_DispatchAO(VkCommandBuffer cmd, const viewDef_t *viewDef)
{
    ...
    vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR, vkRT.aoPipelineLayout,
                            0, 1, &vkRT.aoDescSets[frameIdx], 1, &uboOff);

    if (r_vkLogRT.GetInteger() >= 1)
        common->Printf("VK RT AO: dispatch %ux%u samples=%d radius=%.1f\n",
                       ao.width, ao.height, ubo.numSamples, ubo.aoRadius);
