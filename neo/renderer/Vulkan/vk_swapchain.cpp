...
static void VK_CreateImage( uint32_t width, uint32_t height, VkFormat format,
...
void VK_CreateSwapchain( int width, int height ) {
	...
	for ( uint32_t i = 0; i < vk.swapchainImageCount; i++ ) {
		vk.swapchainImageViews[i] = VK_CreateImageView( vk.swapchainImages[i],
		                                                  vk.swapchainFormat,
		                                                  VK_IMAGE_ASPECT_COLOR_BIT );
	}

	// Create depth buffer
	VK_CreateImage( vk.swapchainExtent.width, vk.swapchainExtent.height,
	                vk.depthFormat,
