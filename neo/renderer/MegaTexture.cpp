...
idCVar idMegaTexture::r_megaTextureLevel("r_megaTextureLevel", "0", CVAR_RENDERER | CVAR_INTEGER,
                                         "draw only a specific level");
idCVar idMegaTexture::r_showMegaTexture("r_showMegaTexture", "0", CVAR_RENDERER | CVAR_BOOL,
                                        "display all the level images");
idCVar idMegaTexture::r_showMegaTextureLabels("r_showMegaTextureLabels", "0", CVAR_RENDERER | CVAR_BOOL,
                                              "draw colored blocks in each tile");
idCVar idMegaTexture::r_skipMegaTexture("r_skipMegaTexture", "0", CVAR_RENDERER | CVAR_INTEGER,
...
static byte colors[8][4] = {{0, 0, 0, 255},   {255, 0, 0, 255},   {0, 255, 0, 255},   {255, 255, 0, 255},
...
void idMegaTexture::SetViewOrigin(const idVec3 viewOrigin)
{
    ...
    for (int i = 0; i < 2; i++)
    {
        texCenter[i] = viewOrigin[0] * localViewToTextureCenter[i][0] + viewOrigin[1] * localViewToTextureCenter[i][1] +
                       viewOrigin[2] * localViewToTextureCenter[i][2] + localViewToTextureCenter[i][3];
    }
...
void idTextureLevel::UpdateTile(int localX, int localY, int globalX, int globalY)
{
    ...
    while (1)
    {
        qglTexSubImage2D(GL_TEXTURE_2D, level, localX * size, localY * size, size, size, GL_RGBA, GL_UNSIGNED_BYTE,
                         data);
        size >>= 1;
        level++;
                    ...
                    for (int xx = 0; xx < 2; xx++)
                    {
                        ...
                        for (int yyy = 0; yyy < TILE_SIZE / 2; yyy++)
                        {
                            for (int xxx = 0; xxx < TILE_SIZE / 2; xxx++)
                            {
                                ...
                                byte *out =
                                    &newBlock[(((TILE_SIZE / 2 * yy) + yyy) * TILE_SIZE + (TILE_SIZE / 2 * xx) + xxx) *
                                              4];
...
void idMegaTexture::GenerateMegaPreview(const char *fileName)
{
    ...
    for (int y = 0; y < height; y++)
    {
        for (int x = 0; x < width; x++)
        {
            ...
            {
                memcpy(pic + ((y * tileSize + yy) * width * tileSize + x * tileSize) * 4, oldBlock + yy * tileSize * 4,
                       tileSize * 4);
            }
        }
    }
...
void idMegaTexture::MakeMegaTexture_f(const idCmdArgs &args)
{
    ...
    common->Printf("Writing %i x %i size %i tiles to %s.\n", mtHeader.tilesWide, mtHeader.tilesHigh, mtHeader.tileSize,
                   outName.c_str());
