...
void idImage::UploadCompressedNormalMap(int width, int height, const byte *rgba, int mipLevel)
{
    ...
    {
        qglTexImage2D(GL_TEXTURE_2D, mipLevel, GL_COLOR_INDEX8_EXT, width, height, 0, GL_COLOR_INDEX, GL_UNSIGNED_BYTE,
                      normals);
    }
}
                                     ...
                                     textureDepth_t minimumDepth) const
{
    ...
    const bool useBC7compression =
        glConfig.bptcTextureCompressionAvailable && globalImages->image_useCompression.GetInteger() == 2;
    ...
    if (minimumDepth == TD_BUMP)
    {
            ...
            globalImages->image_useNormalCompression.GetInteger() == 1)
        {
            ...
            return GL_COLOR_INDEX8_EXT;
                 ...
                 globalImages->image_useNormalCompression.GetInteger() && glConfig.textureCompressionAvailable)
        {
...
#ifdef DHEWM3_VULKAN
    // Upload to Vulkan using the RGBA8 mip-0 data before the mip loop frees
    // scaledBuffer. VK_Image_Upload generates the full mip chain on the GPU via
    // vkCmdBlitImage.
...
==================
*/
void idImage::Generate3DImage(const byte *pic, int width, int height, int picDepth, textureFilter_t filterParm,
                              bool allowDownSizeParm, textureRepeat_t repeatParm, textureDepth_t minDepthParm)
{
    ...
    qglTexImage3D(GL_TEXTURE_3D, 0, internalFormat, scaled_width, scaled_height, scaled_depth, 0, GL_RGBA,
    ...
    while (scaled_width > 1 || scaled_height > 1 || scaled_depth > 1)
    {
        ...
        // upload the mip map
        qglTexImage3D(GL_TEXTURE_3D, miplevel, internalFormat, scaled_width, scaled_height, scaled_depth, 0, GL_RGBA,
                      GL_UNSIGNED_BYTE, scaledBuffer);
    }
    R_StaticFree(scaledBuffer);
...
====================
*/
void idImage::GenerateCubeImage(const byte *pic[6], int size, textureFilter_t filterParm, bool allowDownSizeParm,
                                textureDepth_t depthParm)
{
    ...
    while (scaled_width > 1)
    {
        for (i = 0; i < 6; i++)
        {
            ...
            qglTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X_EXT + i, miplevel, internalFormat, scaled_width / 2,
                          scaled_height / 2, 0, GL_RGBA, GL_UNSIGNED_BYTE, shrunk[i]);
...
bool idImage::CheckPrecompressedImage(bool fullLoad)
{
    ...
    { // +4 for the magic 'DDS ' fourcc at the beginning
        fileSystem->CloseFile(f);
        return false;
    }
    ...
    if (glConfig.bptcTextureCompressionAvailable && globalImages->image_usePrecompressedTextures.GetInteger() == 2)
    {
...
void idImage::UploadPrecompressedImage(byte *data, int len)
{
    ...
    for (int i = 0; i < numMipmaps; i++)
    {
        ...
        else
        {
            ...
            {
                qglTexImage2D(GL_TEXTURE_2D, i - skipMip, internalFormat, uw, uh, 0, externalFormat, GL_UNSIGNED_BYTE,
                              imagedata);
            }
        }
        lastUW = uw;
...
void idImage::UploadScratch(const byte *data, int cols, int rows)
{
    ...
    if (rows == cols * 6)
    {
        ...
        if (cols != uploadWidth || rows != uploadHeight)
        {
            ...
            for (i = 0; i < 6; i++)
            {
                qglTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X_EXT + i, 0, GL_RGB8, cols, rows, 0, GL_RGBA,
                              GL_UNSIGNED_BYTE, data + cols * rows * 4 * i);
            }
        }
        else
        {
