...
static const char *interactionVertSrc =
"#version 330 core\n"
...
static const char *shadowVertSrc =
"#version 330 core\n"
...
static const char *shadowFragSrc =
"#version 330 core\n"
"out vec4 fragColor;\n"
"void main() {\n"
...
static const char *depthVertSrc =
"#version 330 core\n"
"layout(location = 0) in vec3 in_Position;\n"
"layout(location = 8) in vec2 in_TexCoord;\n"
"uniform mat4 u_ModelViewProjection;\n"
...
static const char *depthFragSrc =
"#version 330 core\n"
...
static GLuint R_GLSL_LinkProgram( GLuint vert, GLuint frag, const char *name,
                                   const char **attribNames, const GLuint *attribLocs, int numAttribs ) {
...
void R_GLSL_Init( void ) {
	...
	#define LOAD_GLPROC(var, name) \
		...
		if (!var) { common->Printf("R_GLSL: missing %s\n", name); return; }
	...
	// --- Compile the interaction program ---
	{
		static const char *attribNames[] = {
			"in_Position", "in_Color", "in_TexCoord",
		...
		if ( vert && frag ) {
			glslProgs.interaction.progId = R_GLSL_LinkProgram( vert, frag, "interaction",
			                                                    attribNames, attribLocs, numAttribs );
	...
	// --- Compile the shadow volume program ---
	{
		...
		if ( vert && frag ) {
			glslProgs.shadow.progId = R_GLSL_LinkProgram( vert, frag, "shadow",
			                                               attribNames, attribLocs, 1 );
			glslProgs.shadow.isValid = (glslProgs.shadow.progId != 0);
		}

		if ( glslProgs.shadow.isValid ) {
			glslProgs.shadowUniforms.u_LightOrigin =
				qglGetUniformLocation( glslProgs.shadow.progId, "u_LightOrigin" );
	...
	// --- Compile the depth pre-pass program ---
	{
		...
		if ( vert && frag ) {
			glslProgs.depth.progId = R_GLSL_LinkProgram( vert, frag, "depth",
			                                              attribNames, attribLocs, 2 );
		...
		if ( glslProgs.depth.isValid ) {
			...
			glslProgs.depthUniforms.u_DiffuseMap =
				qglGetUniformLocation( glslProgs.depth.progId, "u_DiffuseMap" );
			glslProgs.depthUniforms.u_AlphaTest =
				qglGetUniformLocation( glslProgs.depth.progId, "u_AlphaTest" );
	...
	common->Printf( "R_GLSL: initialized (interaction=%s, shadow=%s, depth=%s)\n",
	                glslProgs.interaction.isValid ? "OK" : "FAIL",
	                glslProgs.shadow.isValid      ? "OK" : "FAIL",
...
void R_GLSL_Shutdown( void ) {
	if ( !glslInitialized ) return;
	if ( !qglDeleteProgram ) return;

	if ( glslProgs.interaction.progId ) qglDeleteProgram( glslProgs.interaction.progId );
	if ( glslProgs.shadow.progId )      qglDeleteProgram( glslProgs.shadow.progId );
	if ( glslProgs.depth.progId )       qglDeleteProgram( glslProgs.depth.progId );
...
static void RB_GLSL_CreateDrawInteractions( const drawSurf_t *surf ) {
	...
	// Unbind textures
	qglActiveTextureARB( GL_TEXTURE6_ARB ); qglBindTexture( GL_TEXTURE_2D, 0 );
	qglActiveTextureARB( GL_TEXTURE5_ARB ); qglBindTexture( GL_TEXTURE_2D, 0 );
	qglActiveTextureARB( GL_TEXTURE4_ARB ); qglBindTexture( GL_TEXTURE_2D, 0 );
	qglActiveTextureARB( GL_TEXTURE3_ARB ); qglBindTexture( GL_TEXTURE_2D, 0 );
	qglActiveTextureARB( GL_TEXTURE2_ARB ); qglBindTexture( GL_TEXTURE_2D, 0 );
	qglActiveTextureARB( GL_TEXTURE1_ARB ); qglBindTexture( GL_TEXTURE_2D, 0 );
	qglActiveTextureARB( GL_TEXTURE0_ARB );
...
static void RB_GLSL_StencilShadowPass( const drawSurf_t *drawSurfs ) {
	if ( !drawSurfs ) return;
	if ( !glslProgs.shadow.isValid ) return;
