...
const char *imageFilter[] = {"GL_LINEAR_MIPMAP_NEAREST",  "GL_LINEAR_MIPMAP_LINEAR",  "GL_NEAREST", "GL_LINEAR",
                             "GL_NEAREST_MIPMAP_NEAREST", "GL_NEAREST_MIPMAP_LINEAR", NULL};

idCVar idImageManager::image_filter("image_filter", imageFilter[1], CVAR_RENDERER | CVAR_ARCHIVE,
                                    "changes texture filtering on mipmapped images", imageFilter,
...
idCVar idImageManager::image_lodbias("image_lodbias", "0", CVAR_RENDERER | CVAR_ARCHIVE,
                                     "change lod bias on mipmapped images");
idCVar idImageManager::image_downSize("image_downSize", "0", CVAR_RENDERER | CVAR_ARCHIVE,
...
idCVar idImageManager::image_preload("image_preload", "1", CVAR_RENDERER | CVAR_BOOL | CVAR_ARCHIVE,
...
idCVar idImageManager::image_useCompression("image_useCompression", "1", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_INTEGER,
                                            "Compress textures on load so they use less VRAM. 1 = compress with "
                                            "S3TC/DXT when uploading 2 = compress with BPTC when uploading (if "
                                            "available) "
                                            "0 = upload uncompressed (unless image_usePrecompressedTextures is 1 and "
                                            "it's loaded from a precompressed .dds file)");
...
idCVar idImageManager::image_useNormalCompression(
    "image_useNormalCompression", "2", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_INTEGER,
...
idCVar idImageManager::image_usePrecompressedTextures(
    "image_usePrecompressedTextures", "1", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_INTEGER,
    "1 = use .dds files if present 2 = only use .dds files if they contain "
    "BPTC (BC7) textures (those have higher quality than S3TC/DXT) 0 = use "
    "uncompressed textures");
idCVar idImageManager::image_writePrecompressedTextures("image_writePrecompressedTextures", "0",
                                                        CVAR_RENDERER | CVAR_BOOL, "write .dds files if necessary");
...
idCVar idImageManager::image_cacheMinK("image_cacheMinK", "200", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_INTEGER,
                                       "maximum KB of precompressed files to read at specification time");
idCVar idImageManager::image_cacheMegs("image_cacheMegs", "20", CVAR_RENDERER | CVAR_ARCHIVE,
                                       "maximum MB set aside for temporary loading of full-sized precompressed "
                                       "images");
idCVar idImageManager::image_useCache("image_useCache", "0", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_BOOL,
...
idCVar idImageManager::image_downSizeSpecular("image_downSizeSpecular", "0", CVAR_RENDERER | CVAR_ARCHIVE,
                                              "controls specular downsampling");
idCVar idImageManager::image_downSizeBump("image_downSizeBump", "0", CVAR_RENDERER | CVAR_ARCHIVE,
                                          "controls normal map downsampling");
idCVar idImageManager::image_downSizeSpecularLimit("image_downSizeSpecularLimit", "64", CVAR_RENDERER | CVAR_ARCHIVE,
                                                   "controls specular downsampled limit");
idCVar idImageManager::image_downSizeBumpLimit("image_downSizeBumpLimit", "128", CVAR_RENDERER | CVAR_ARCHIVE,
...
idCVar idImageManager::image_downSizeLimit("image_downSizeLimit", "256", CVAR_RENDERER | CVAR_ARCHIVE,
...
const imageClassificate_t IC_Info[] = {{"models/characters", "Characters", IC_NPC, 512, 512},
...
static void R_BorderClampImage(idImage *image)
{
    ...
    for (int i = 0; i < BORDER_CLAMP_SIZE; i++)
    {
        data[i][0][0] = data[i][0][1] = data[i][0][2] = data[i][0][3] =
            ...
            data[i][BORDER_CLAMP_SIZE - 1][0] = data[i][BORDER_CLAMP_SIZE - 1][1] = data[i][BORDER_CLAMP_SIZE - 1][2] =
                data[i][BORDER_CLAMP_SIZE - 1][3] =
                    ...
                    data[0][i][0] = data[0][i][1] = data[0][i][2] = data[0][i][3] =
                        ...
                        data[BORDER_CLAMP_SIZE - 1][i][0] = data[BORDER_CLAMP_SIZE - 1][i][1] =
                            data[BORDER_CLAMP_SIZE - 1][i][2] = data[BORDER_CLAMP_SIZE - 1][i][3] = 0;
    }

    image->GenerateImage((byte *)data, BORDER_CLAMP_SIZE, BORDER_CLAMP_SIZE, TF_LINEAR /* TF_NEAREST */, false,
                         TR_CLAMP_TO_BORDER, TD_DEFAULT);
...
void idImageManager::ChangeTextureFilter(void)
{
    ...
    idImage *glt;
    const char *string;
    static const filterName_t textureFilters[] = {{"GL_LINEAR_MIPMAP_NEAREST", GL_LINEAR_MIPMAP_NEAREST, GL_LINEAR},
...
void R_ReloadImages_f(const idCmdArgs &args)
{
    ...
    all = false;
    checkPrecompressed = false; // if we are doing this as a vid_restart, look for
                                // precompressed like normal
...
void R_ListImages_f(const idCmdArgs &args)
{
    ...
    if (failed)
    {
        common->Printf("usage: listImages [ sorted | partial | unloaded | cached | uncached | "
    ...
    for (i = 0; i < globalImages->images.Num(); i++)
    {
        ...
        if (uncompressedOnly)
        {
            if ((image->internalFormat >= GL_COMPRESSED_RGB_S3TC_DXT1_EXT &&
                ...
                image->internalFormat == GL_COMPRESSED_RGBA_BPTC_UNORM_ARB)
            {
                continue;
            }
        }
    ...
    if (byClassification)
    {
        ...
        for (i = 0; i < IC_COUNT; i++)
        {
            ...
            for (j = 0; j < classifications[i].Num(); j++)
            {
                ...
                if (overSized)
                {
                    if (sortedArray[classifications[i][j]].image->uploadWidth > IC_Info[i].maxWidth &&
                        sortedArray[classifications[i][j]].image->uploadHeight > IC_Info[i].maxHeight)
                    {
            ...
            common->Printf(" Classification %s contains %i images using %5.1f megabytes\n", IC_Info[i].desc,
                           classifications[i].Num(), partialSize / (1024 * 1024.0));
                                       ...
                                       textureRepeat_t repeat, textureDepth_t depth, cubeFiles_t cubeMap)
{
    ...
    if (image->ShouldImageBePartialCached())
    {
        ...
        if (image_preload.GetBool() && !insideLevelLoad)
        {
            ...
            declManager->MediaPrint("%ix%i %s\n", image->partialImage->uploadWidth, image->partialImage->uploadHeight,
                                    image->imgName.c_str());
        }
        else
        {
...
void R_CombineCubeImages_f(const idCmdArgs &args)
{
    if (args.Argc() != 2)
    {
        ...
        common->Printf(" combines basename[1-6][0001-9999].tga to "
...
void idImage::StartBackgroundImageLoad()
{
    ...
    {
        common->Warning("idImageManager::StartBackgroundImageLoad: %s wasn't a "
                        "precompressed file",
                        imgName.c_str());
        return;
    }
    ...
    int totalSize = 0;
    for (idImage *check = globalImages->cacheLRU.cacheUsageNext; check != &globalImages->cacheLRU;
...
void idImageManager::Init()
{
    ...
    currentDepthImage = ImageFromFunction("_currentDepth",
                                          R_DepthImage); // #3877. Allow shaders to access scene depth

    cmdSystem->AddCommand("reloadImages", R_ReloadImages_f, CMD_FL_RENDERER, "reloads images");
    cmdSystem->AddCommand("listImages", R_ListImages_f, CMD_FL_RENDERER, "lists images");
    cmdSystem->AddCommand("combineCubeImages", R_CombineCubeImages_f, CMD_FL_RENDERER,
