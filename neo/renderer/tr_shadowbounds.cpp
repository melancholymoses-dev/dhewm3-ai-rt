...
template <class T, int N> struct MyArray
{
    MyArray() : s(0)
    {
    }
...
{
    return idMat4(m[0], m[4], m[8], m[12], m[1], m[5], m[9], m[13], m[2], m[6], m[10], m[14], m[3], m[7], m[11], m[15]);
}
...
void world_to_hclip(const viewDef_t *viewDef, const idVec4 &global, idVec4 &clip)
{
    ...
    for (i = 0; i < 4; i++)
    {
        clip[i] = view[0] * viewDef->projectionMatrix[i + 0 * 4] + view[1] * viewDef->projectionMatrix[i + 1 * 4] +
                  view[2] * viewDef->projectionMatrix[i + 2 * 4] + view[3] * viewDef->projectionMatrix[i + 3 * 4];
    }
}
                                       ...
                                       const viewDef_t *viewDef)
{
    ...
    idVec4 lightpos =
        idVec4(lightDef->globalLightOrigin.x, lightDef->globalLightOrigin.y, lightDef->globalLightOrigin.z, 1.0f);
