...
static void R_FlareDeform(drawSurf_t *surf)
{
    ...
    // find the plane
    if (!plane.FromPoints(tri->verts[tri->indexes[0]].xyz, tri->verts[tri->indexes[1]].xyz,
...
#if 1
    static glIndex_t triIndexes[18 * 3] = {0,  4,  5, 0,  5, 6,  0,  6, 7, 0,  7, 1,  1,  7, 8,  1,  8, 9,
        ...
        for (int stageNum = 0; stageNum < particleSystem->stages.Num(); stageNum++)
        {
            ...
            for (int index = 0; index < totalParticles; index++)
            {
                ...
                if (useArea)
                {
                    ...
                    pointTri =
                        idBinSearch_LessEqual<float>(sourceTriAreas, numSourceTris, g.random.RandomFloat() * totalArea);
                }
