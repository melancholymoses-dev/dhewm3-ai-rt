...
void RB_DrawElementsWithCounters(const srfTriangles_t *tri)
{
    ...
    {
        qglDrawElements(GL_TRIANGLES, r_singleTriangle.GetBool() ? 3 : tri->numIndexes, GL_INDEX_TYPE,
    ...
    else
    {
        ...
        qglDrawElements(GL_TRIANGLES, r_singleTriangle.GetBool() ? 3 : tri->numIndexes, GL_INDEX_TYPE, tri->indexes);
    }
}
...
void RB_DrawShadowElementsWithCounters(const srfTriangles_t *tri, int numIndexes)
{
    ...
    if (tri->indexCache && r_useIndexBuffers.GetBool())
    {
        qglDrawElements(GL_TRIANGLES, r_singleTriangle.GetBool() ? 3 : numIndexes, GL_INDEX_TYPE,
                        (int *)vertexCache.Position(tri->indexCache));
...
void RB_BindStageTexture(const float *shaderRegisters, const textureStage_t *texture, const drawSurf_t *surf)
{
    ...
    if (texture->texgen == TG_DIFFUSE_CUBE)
    {
        qglTexCoordPointer(3, GL_FLOAT, sizeof(idDrawVert),
                           ((idDrawVert *)vertexCache.Position(surf->geo->ambientCache))->normal.ToFloatPtr());
    }
    ...
    if (texture->texgen == TG_REFLECT_CUBE)
    {
        ...
        qglNormalPointer(GL_FLOAT, sizeof(idDrawVert),
                         ((idDrawVert *)vertexCache.Position(surf->geo->ambientCache))->normal.ToFloatPtr());
...
void RB_BeginDrawingView(void)
{
    ...
    qglScissor(tr.viewportOffset[0] + viewDef->viewport.x1 + viewDef->scissor.x1,
               ...
               viewDef->scissor.x2 + 1 - viewDef->scissor.x1, viewDef->scissor.y2 + 1 - viewDef->scissor.y1);
...
==================
*/
void R_SetDrawInteraction(const shaderStage_t *surfaceStage, const float *surfaceRegs, idImage **image,
    ...
    for (int lightStageNum = 0; lightStageNum < lightShader->GetNumStages(); lightStageNum++)
    {
        ...
        for (int surfaceStageNum = 0; surfaceStageNum < surfaceShader->GetNumStages(); surfaceStageNum++)
        {
            ...
            case SL_DIFFUSE: {
                ...
                R_SetDrawInteraction(surfaceStage, surfaceRegs, &inter.diffuseImage, inter.diffuseMatrix,
            ...
            case SL_SPECULAR: {
                ...
                {
                    R_SetDrawInteraction(surfaceStage, surfaceRegs, &inter.specularImage, inter.specularMatrix,
