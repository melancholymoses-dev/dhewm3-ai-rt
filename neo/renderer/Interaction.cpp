...
================
*/
void R_CalcInteractionFacing(const idRenderEntityLocal *ent, const srfTriangles_t *tri, const idRenderLightLocal *light,
...
=====================
*/
void R_CalcInteractionCullBits(const idRenderEntityLocal *ent, const srfTriangles_t *tri,
...
====================
*/
static srfTriangles_t *R_CreateLightTris(const idRenderEntityLocal *ent, const srfTriangles_t *tri,
        ...
        for (faceNum = i = 0; i < tri->numIndexes; i += 3, faceNum++)
        {
            ...
            if (r_usePreciseTriangleInteractions.GetBool())
            {
                ...
                if (cullBits[i1] && cullBits[i2] && cullBits[i3])
                {
                    ...
                    if (!R_ClipTriangleToLight(tri->verts[i1].xyz, tri->verts[i2].xyz, tri->verts[i3].xyz, cull,
...
idScreenRect idInteraction::CalcInteractionScissorRectangle(const idFrustum &viewFrustum)
{
    ...
    if (lightDef->parms.pointLight)
    {
        viewFrustum.ClippedProjectionBounds(
            frustum, idBox(lightDef->parms.origin, lightDef->parms.lightRadius, lightDef->parms.axis),
            projectionBounds);
    }
    else
    {
...
bool idInteraction::CullInteractionByViewFrustum(const idFrustum &viewFrustum)
{
    ...
    if (r_showInteractionFrustums.GetInteger())
    {
        ...
        tr.viewDef->renderWorld->DebugFrustum(colors[lightDef->index & 7], frustum,
...
======================
*/
static bool R_PotentiallyInsideInfiniteShadow(const srfTriangles_t *occluder, const idVec3 &localView,
        ...
        if (!lightScissorsEmpty && sint->ambientTris && sint->ambientTris->ambientViewCount == tr.viewCount)
        {
            ...
            if (lightTris)
            {
                ...
                if (!R_CullLocalBox(lightTris->bounds, vEntity->modelMatrix, 5, tr.viewDef->frustum))
                {
                    ...
                    {
                        R_LinkLightSurf(&vLight->translucentInteractions, lightTris, vEntity, lightDef, shader,
                                        lightScissor, false);
                    }
                    else if (!lightDef->parms.noShadows && sint->shader->TestMaterialFlag(MF_NOSELFSHADOW))
                    {
                        R_LinkLightSurf(&vLight->localInteractions, lightTris, vEntity, lightDef, shader, lightScissor,
                                        false);
                    }
                    else
                    {
                        R_LinkLightSurf(&vLight->globalInteractions, lightTris, vEntity, lightDef, shader, lightScissor,
                                        false);
                    }
                }
            }
