...
bool rvRollupPanel::Create(DWORD dwStyle, const RECT &rect, HWND parent, unsigned int id)
{
    ...
    mWindow = CreateWindowEx(WS_EX_TOOLWINDOW, "ROLLUP_PANEL", "", dwStyle | WS_CLIPSIBLINGS, rect.left, rect.top,
                             rect.right - rect.left, rect.bottom - rect.top, parent, NULL, win32.hInstance, this);
...
int rvRollupPanel::InsertItem(const char *caption, HWND dialog, bool autoDestroy, int index)
{
    ...
    HWND groupbox = CreateWindow("BUTTON", "", WS_CHILD | BS_GROUPBOX, r.left, r.top, r.right - r.left,
                                 r.bottom - r.top, mWindow, 0, win32.hInstance, NULL);

    // Create the expand button
    HWND button = CreateWindow("BUTTON", caption, WS_CHILD | BS_AUTOCHECKBOX | BS_PUSHLIKE | BS_FLAT, r.left, r.top,
                               r.right - r.left, r.bottom - r.top, mWindow, 0, win32.hInstance, NULL);
...
void rvRollupPanel::RecallLayout(void)
{
    ...
    for (i = 0; i < mItems.Num(); i++)
    {
        ...
        if (item->mExpanded && item->mEnable)
        {
            ...
            SetWindowPos(
#endif
                           item->mGroupBox, 0, 2, posy, (r.right - r.left) - 3 - RP_SCROLLBARWIDTH,
            ...
            SetWindowPos(
                           ...
                           (r.right - r.left) - RP_SCROLLBARWIDTH - (RP_GRPBOXINDENT * 2), ir.bottom - ir.top,
                           SWP_NOZORDER | SWP_SHOWWINDOW);
            ...
            SetWindowPos(
                           ...
                           (r.right - r.left) - RP_SCROLLBARWIDTH - (RP_GRPBOXINDENT * 2), RP_PGBUTTONHEIGHT,
                           SWP_NOZORDER | SWP_SHOWWINDOW);
        ...
        else
        {
            ...
            SetWindowPos(
#endif
                           item->mGroupBox, 0, 2, posy, (r.right - r.left) - 3 - RP_SCROLLBARWIDTH, 16,
                           SWP_NOZORDER | SWP_SHOWWINDOW);
...
int rvRollupPanel::HandleCommand(WPARAM wParam, LPARAM lParam)
{
    ...
    else if (LOWORD(wParam) >= RP_IDM_STARTITEMS && LOWORD(wParam) < RP_IDM_STARTITEMS + GetItemCount())
    {
