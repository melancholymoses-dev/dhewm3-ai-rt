...
static ColorTableEntry _crColors[] = {
    ...
    {RGB(0x7F, 0xFF, 0xD4)}, {RGB(0x7D, 0x9E, 0xC0)}, {RGB(0x80, 0x00, 0x80)}, {RGB(0x7F, 0x7F, 0x7F)},
...
CPropTreeItemColor::CPropTreeItemColor()
    : m_cColor(0), m_cPrevColor(0), m_nSpot(-1), m_bButton(FALSE), m_bInDialog(FALSE)
{
}
...
void CPropTreeItemColor::OnActivate(int activateType, CPoint point)
{
    ...
    if (!IsWindow(m_hWnd))
    {
        ...
        pszClassName =
            AfxRegisterWndClass(CS_VREDRAW | CS_HREDRAW, LoadCursor(NULL, IDC_ARROW), (HBRUSH)(COLOR_BTNFACE + 1));
