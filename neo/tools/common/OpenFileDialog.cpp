...
void rvOpenFileDialog::HandleCommandOK(void)
{
    ...
    else if (item.iImage == 2)
    {
        ...
        if (mFlags & OFD_MUSTEXIST)
        {
            ...
            {
                MessageBox(mWnd,
                           va("%s\nFile not found.\nPlease verify the correct file name was given", mFilename.c_str()),
...
void rvOpenFileDialog::HandleInitDialog(void)
{
    ...
    mBackBitmap = (HBITMAP)LoadImage(mInstance, MAKEINTRESOURCE(IDB_TOOLS_BACK), IMAGE_BITMAP, 16, 16,
