...
bool rvRegistryOptions::Save(void)
{
    ...
    for (i = 0; i < mValues.GetNumKeyVals(); i++)
    {
        ...
        assert(key);
        RegSetValueEx(hKey, key->GetKey().c_str(), 0, REG_SZ, (BYTE *)key->GetValue().c_str(),
                      key->GetValue().Length());
    }
...
void rvRegistryOptions::SetWindowPlacement(const char *name, HWND hwnd)
{
    ...
    out = va("%d %d %d %d %d %d %d %d %d %d", wp.flags, wp.ptMaxPosition.x, wp.ptMaxPosition.y, wp.ptMinPosition.x,
             wp.ptMinPosition.y, wp.rcNormalPosition.left, wp.rcNormalPosition.top, wp.rcNormalPosition.right,
...
bool rvRegistryOptions::GetWindowPlacement(const char *name, HWND hwnd)
{
    ...
    sscanf(key->GetValue().c_str(), "%d %d %d %d %d %d %d %d %d %d", &wp.flags, &wp.ptMaxPosition.x,
           &wp.ptMaxPosition.y, &wp.ptMinPosition.x, &wp.ptMinPosition.y, &wp.rcNormalPosition.left,
           &wp.rcNormalPosition.top, &wp.rcNormalPosition.right, &wp.rcNormalPosition.bottom, &wp.showCmd);
