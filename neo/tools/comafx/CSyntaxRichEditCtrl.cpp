...
void CSyntaxRichEditCtrl::Init(void)
{
    ...
    autoCompleteListBox.Create(WS_DLGFRAME | WS_VISIBLE | WS_VSCROLL | LBS_SORT | LBS_NOTIFY, rect, this,
...
================
*/
void CSyntaxRichEditCtrl::SetColor(int startCharIndex, int endCharIndex, COLORREF foreColor, COLORREF backColor,
                                   bool bold)
{
    tom::ITextRange *range;
    tom::ITextFont *font;
...
void CSyntaxRichEditCtrl::HighlightSyntax(int startCharIndex, int endCharIndex)
{
    ...
    for (charIndex = startCharIndex; charIndex <= endCharIndex; charIndex++)
    {
        ...
        case CT_NAME: {
            ...
            if (keyWordIndex != -1)
            {
                SetColor(syntaxStart, syntaxStart + keyWordLength, keyWordColors[keyWordIndex], DEFAULT_BACK_COLOR,
                         false);
            }
            break;
        }
