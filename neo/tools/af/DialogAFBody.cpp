...
DialogAFBody::DialogAFBody(CWnd *pParent /*=NULL*/)
    : CDialog(DialogAFBody::IDD, pParent), constraintDlg(NULL), numJoints(0), cm_length(0), cm_height(0), cm_width(0),
      cm_density(0), cm_numSides(3), cm_origin_x(0), cm_origin_y(0), cm_origin_z(0), cm_angles_pitch(0),
      cm_angles_yaw(0), cm_angles_roll(0), m_selfCollision(false), m_linearFriction(0), m_angularFriction(0),
      m_contactFriction(0), file(NULL), body(NULL)
{
...
void DialogAFBody::InitCollisionModelType(void)
{
    ...
    if (type == TRM_BONE)
    {
        ...
        CheckRadioButton(IDC_RADIO_ORIGIN_COORDINATES, IDC_RADIO_ORIGIN_JOINT, IDC_RADIO_ORIGIN_BONECENTER);
    }
    else
    {
        showBone = SW_HIDE;
...
void DialogAFBody::LoadBody(const char *name)
{
    ...
    if (body->frictionDirection.ToVec3() != vec3_origin)
    {
        ...
        file.WriteFloatString("%f %f %f", body->frictionDirection.ToVec3().x, body->frictionDirection.ToVec3().y,
    ...
    if (body->contactMotorDirection.ToVec3() != vec3_origin)
    {
        ...
        file.WriteFloatString("%f %f %f", body->contactMotorDirection.ToVec3().x,
                              body->contactMotorDirection.ToVec3().y, body->contactMotorDirection.ToVec3().z);
...
void DialogAFBody::SaveBody(void)
{
    ...
    if (str.GetLength() != 0)
    {
        ...
        sscanf(str, "%f %f %f", &body->frictionDirection.ToVec3().x, &body->frictionDirection.ToVec3().y,
               &body->frictionDirection.ToVec3().z);
    }
    ...
    if (str.GetLength() != 0)
    {
        ...
        sscanf(str, "%f %f %f", &body->contactMotorDirection.ToVec3().x, &body->contactMotorDirection.ToVec3().y,
