...
================
*/
DialogAF::DialogAF(CWnd *pParent /*=NULL*/) : CDialog(DialogAF::IDD, pParent), file(NULL)
{
    wndTabs = NULL;
    wndTabDisplay = NULL;
}
...
void DialogAF::OnBnClickedButtonAfNew()
{
    ...
    CFileDialog dlgSave(FALSE, "map", NULL, OFN_OVERWRITEPROMPT, "AF Files (*.af)|*.af|All Files (*.*)|*.*||",
                        AfxGetMainWnd());
...
void DialogAF::OnBnClickedButtonAfDelete()
{
    ...
    if (i != CB_ERR)
    {
        if (MessageBox("Are you sure you want to delete the articulated figure file ?", "Delete Articulated Figure",
...
void DialogAF::OnBnClickedButtonAfSave()
{
    ...
    if (file)
    {
        ...
        {
            MessageBox("Saving the file failed. Make sure the file is not read-only.", "Delete Articulated Figure",
                       MB_OK);
        }
    }
}
