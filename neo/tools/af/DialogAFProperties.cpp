...
toolTip_t DialogAFProperties::toolTips[] = {
    ...
    {IDC_EDIT_LINEARVELOCITY, "do not suspend simulation if the linear "
                              "velocity is higher than this value"},
    {IDC_EDIT_ANGULARVELOCITY, "do not suspend simulation if the angular "
                               "velocity is higher than this value"},
    {IDC_EDIT_LINEARACCELERATION, "do not suspend simulation if the linear "
                                  "acceleration is higher than this value"},
    {IDC_EDIT_ANGULARACCELERATION, "do not suspend simulation if the angular "
                                   "acceleration is higher than this value"},
...
DialogAFProperties::DialogAFProperties(CWnd *pParent /*=NULL*/)
    : CDialog(DialogAFProperties::IDD, pParent), m_selfCollision(false), m_linearFriction(0), m_angularFriction(0),
      m_contactFriction(0), m_constraintFriction(0), m_totalMass(0), m_suspendLinearVelocity(0),
      m_suspendAngularVelocity(0), m_suspendLinearAcceleration(0), m_suspendAngularAcceleration(0), m_noMoveTime(0),
      m_minMoveTime(0), m_maxMoveTime(0), m_linearTolerance(0), m_angularTolerance(0), file(NULL)
{
...
void DialogAFProperties::OnBnClickedCheckSelfcollision()
{
    ...
    if (file && file->bodies.Num() &&
        MessageBox("Apply to all bodies ?", "Self Collision", MB_YESNO | MB_ICONQUESTION) == IDYES)
    {
...
void DialogAFProperties::OnEnChangeEditContents()
{
    if (EditControlEnterHit(&m_editContents))
    {
        UpdateFile();
        if (file && file->bodies.Num() &&
            MessageBox("Apply to all bodies ?", "Contents", MB_YESNO | MB_ICONQUESTION) == IDYES)
        {
            for (int i = 0; i < file->bodies.Num(); i++)
            {
...
void DialogAFProperties::OnEnChangeEditClipmask()
{
    if (EditControlEnterHit(&m_editClipMask))
    {
        UpdateFile();
        if (file && file->bodies.Num() &&
            MessageBox("Apply to all bodies ?", "Clip Mask", MB_YESNO | MB_ICONQUESTION) == IDYES)
        {
            for (int i = 0; i < file->bodies.Num(); i++)
            {
...
void DialogAFProperties::OnEnChangeEditLinearfriction()
{
    if (EditControlEnterHit((CEdit *)GetDlgItem(IDC_EDIT_LINEARFRICTION)))
    {
        UpdateFile();
        if (file && file->bodies.Num() &&
            MessageBox("Apply to all bodies ?", "Linear Friction", MB_YESNO | MB_ICONQUESTION) == IDYES)
        {
            for (int i = 0; i < file->bodies.Num(); i++)
            {
...
void DialogAFProperties::OnEnChangeEditAngularfriction()
{
    if (EditControlEnterHit((CEdit *)GetDlgItem(IDC_EDIT_ANGULARFRICTION)))
    {
        UpdateFile();
        if (file && file->bodies.Num() &&
            MessageBox("Apply to all bodies ?", "Angular Friction", MB_YESNO | MB_ICONQUESTION) == IDYES)
        {
            for (int i = 0; i < file->bodies.Num(); i++)
            {
...
void DialogAFProperties::OnEnChangeEditContactfriction()
{
    if (EditControlEnterHit((CEdit *)GetDlgItem(IDC_EDIT_CONTACTFRICTION)))
    {
        UpdateFile();
        if (file && file->bodies.Num() &&
            MessageBox("Apply to all bodies ?", "Contact Friction", MB_YESNO | MB_ICONQUESTION) == IDYES)
        {
            for (int i = 0; i < file->bodies.Num(); i++)
            {
...
void DialogAFProperties::OnEnChangeEditConstraintfriction()
{
    if (EditControlEnterHit((CEdit *)GetDlgItem(IDC_EDIT_CONSTRAINTFRICTION)))
    {
        ...
        if (file && file->constraints.Num() &&
            MessageBox("Apply to all constraints ?", "Constraint Friction", MB_YESNO | MB_ICONQUESTION) == IDYES)
        {
