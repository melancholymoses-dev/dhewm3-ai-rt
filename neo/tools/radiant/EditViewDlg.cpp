...
CEditViewDlg::CEditViewDlg(CWnd *pParent /*=NULL*/) : CDialog(CEditViewDlg::IDD, pParent)
{
    findDlg = NULL;
}
...
void CEditViewDlg::OnSize(UINT nType, int cx, int cy)
{
    ...
    editInfo.SetWindowPos(NULL, 4, 8 + crect.Height(), rect.Width() - 8, rect.Height() - crect.Height() * 2 - 16,
                          SWP_SHOWWINDOW);
    ...
    wnd->SetWindowPos(NULL, 8 + crect.Width(), rect.Height() - 3 - crect.Height(), crect.Width() + 8,
                      crect.Height() - 3, SWP_SHOWWINDOW);
    wnd = GetDlgItem(IDC_STATIC_LINE);
    wnd->SetWindowPos(NULL, 30 + crect.Width() * 2, rect.Height() - crect.Height(), crect.Width() * 2, crect.Height(),
                      SWP_SHOWWINDOW);
    wnd = GetDlgItem(IDC_EDIT_LINE);
    wnd->SetWindowPos(NULL, 40 + crect.Width() * 3, rect.Height() - crect.Height(), crect.Width() + 8, crect.Height(),
                      SWP_SHOWWINDOW);
}
...
void CEditViewDlg::UpdateEditPreview()
{
    if (GetSafeHwnd() && editInfo.GetSafeHwnd())
    {
        ...
        SetWindowText(
            va("Editing %s in file <%s>", (mode == MATERIALS) ? matName.c_str() : fileName.c_str(), fileName.c_str()));
        editInfo.SetFocus();
    }
}
...
BOOL CEditViewDlg::PreTranslateMessage(MSG *pMsg)
{
    ...
    if (pMsg->message == WM_KEYDOWN && (pMsg->wParam == 's' || pMsg->wParam == 'S') &&
    ...
    if (pMsg->message == WM_KEYDOWN && (pMsg->wParam == 'o' || pMsg->wParam == 'O') &&
        GetAsyncKeyState(VK_CONTROL) & 0x8000)
    {
