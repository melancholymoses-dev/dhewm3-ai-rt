...
void CTabsDlg::DockWindow(int ID, bool dock)
{
    ...
    if (!dock)
    {
        ...
        info->m_Container.CreateEx(WS_EX_TOOLWINDOW, classname, info->m_Title,
    ...
    else
    {
        ...
        info->m_TabControlIndex =
            m_Tabs.InsertItem(TCIF_TEXT | TCIF_IMAGE | TCIF_PARAM, 0, info->m_Title, info->m_ImageID, (LPARAM)info);
...
void CTabsDlg::AddDockedWindow(CWnd *wnd, int ID, int imageID, const CString &title, bool dock,
