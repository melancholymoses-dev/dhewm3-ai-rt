...
CEntityDlg::CEntityDlg(CWnd *pParent /*=NULL*/) : CDialog(CEntityDlg::IDD, pParent)
{
    editEntity = NULL;
...
void CEntityDlg::OnSize(UINT nType, int cx, int cy)
{
    ...
    btnCreate.SetWindowPos(NULL, rect.Width() - crect2.Width() - 4, top, crect2.Width(), crect.Height(),
                           SWP_SHOWWINDOW);
    top += crect.Height() + s4;
    ...
    btnModel.SetWindowPos(NULL, rect.right - s4 - crect.Width(), top + s8, crect.Width(), crect.Height(),
                          SWP_SHOWWINDOW);
    btnSound.SetWindowPos(NULL, rect.right - s4 - crect.Width(), top + s12 + crect.Height(), crect.Width(),
                          crect.Height(), SWP_SHOWWINDOW);
    btnGui.SetWindowPos(NULL, rect.right - s4 - crect.Width(), top + s16 + crect.Height() * 2, crect.Width(),
                        crect.Height(), SWP_SHOWWINDOW);
    btnParticle.SetWindowPos(NULL, rect.right - s8 - (crect.Width() * 2), top + s16 + crect.Height() * 2, crect.Width(),
                             crect.Height(), SWP_SHOWWINDOW);
    btnSkin.SetWindowPos(NULL, rect.right - s8 - (crect.Width() * 2), top + s12 + crect.Height(), crect.Width(),
                         crect.Height(), SWP_SHOWWINDOW);
    btnCurve.SetWindowPos(NULL, rect.right - s8 - (crect.Width() * 2), top + s8, crect.Width(), crect.Height(),
                          SWP_SHOWWINDOW);
    ...
    btnStopAnim.SetWindowPos(NULL, rightAnimAreaBorder - crect.Width(), top + 8, crect.Width(), crect.Height(),
                             SWP_SHOWWINDOW);
    ...
    staticFrame.SetWindowPos(NULL, rightAnimAreaBorder - crect.Width(), top + (34 * scaling_factor), crect.Width(),
                             crect.Height(), SWP_SHOWWINDOW);

    left = rightAnimAreaBorder - crect.Width() - s4;

    slFrameSlider.GetWindowRect(crect);
    slFrameSlider.SetWindowPos(NULL, left - crect.Width(), top + (32 * scaling_factor), crect.Width(), crect.Height(),
                               SWP_SHOWWINDOW);
...
static int TabOrder[] = {IDC_COMBO_CLASS, IDC_BUTTON_CREATE,
                         // IDC_EDIT_INFO,
                         IDC_LIST_KEYVAL, IDC_EDIT_KEY, IDC_EDIT_VAL, IDC_BUTTON_BROWSE, IDC_E_135, IDC_E_90, IDC_E_45,
                         IDC_E_180, IDC_E_0, IDC_E_225, IDC_E_270, IDC_E_315, IDC_E_UP, IDC_E_DOWN, IDC_BUTTON_MODEL,
                         IDC_BUTTON_SOUND, IDC_BUTTON_GUI, IDC_ENTITY_ANIMATIONS};
    ...
    else
    {
        ...
        if (isModel && !(editEntity->eclass->nShowFlags & ECLASS_WORLDSPAWN))
        {
            ...
            if (dynamic_cast<idRenderModelPrt *>(selected_brushes.next->modelHandle) ||
                dynamic_cast<idRenderModelLiquid *>(selected_brushes.next->modelHandle))
            {
                bo.Zero();
                bo.ExpandSelf(12.0f);
            }
