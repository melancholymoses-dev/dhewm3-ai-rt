...
void HandlePopup(CWnd *pWindow, unsigned int uId)
{
    ...
    mnuPopup.GetSubMenu(0)->TrackPopupMenu(TPM_LEFTALIGN | TPM_LEFTBUTTON | TPM_RIGHTBUTTON, ptMouse.x, ptMouse.y,
                                           pWindow);
...
void CMainFrame::SetButtonMenuStates()
{
    ...
    if (pMenu)
    {
        ...
        pMenu->CheckMenuItem(ID_TOGGLE_ROTATELOCK,
        ...
        pMenu->CheckMenuItem(ID_VIEW_OPENGLLIGHTING,
        ...
        {
            m_wndToolBar.GetToolBarCtrl().CheckButton(ID_VIEW_CUBICCLIPPING,
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
    ...
    m_wndToolBar.GetToolBarCtrl().CheckButton(ID_SOUND_SHOWSELECTEDSOUNDVOLUMES,
    ...
    if (radiant_entityMode.GetBool())
    {
        g_qeglobals.d_savedinfo.exclude |=
            (EXCLUDE_PATHS | EXCLUDE_CLIP | EXCLUDE_CAULK | EXCLUDE_VISPORTALS | EXCLUDE_NODRAW | EXCLUDE_TRIGGERS);
    }
...
void CMainFrame::LoadCommandMap()
{
    ...
    for (int i = 0; i < g_nCommandCount; i++)
    {
        ...
        if (nLen > 0)
        {
            ...
            strBuff.MakeUpper();
            if (nLen == 1)
            { // most often case.. deal with first
    ...
    for (int i = 0; i < g_nCommandCount; i++)
    {
        ...
        { // find a match?
            ...
            if (g_Commands[i].m_nModifiers & RAD_PRESS)
            {
                ...
                { // no modifiers make sure none of those keys are pressed
                    if (GetAsyncKeyState(VK_MENU) & 0x8000)
                    {
                        bGo = false;
                    }
...
void RunBsp(const char *command)
{
    ...
    if (idStr::Icmpn(command, "bspext", strlen("runbsp")) == 0)
    {
        ...
        {
            idStr::snPrintf(system, sizeof(system), "%s %s +set r_fullscreen 0 +dmap editorOutput %s %s +quit", buff,
                            paths.c_str(), command + strlen("bspext"), in);
        }
        else
        {
            idStr::snPrintf(system, sizeof(system), "%s %s +set r_fullscreen 0 +dmap editorOutput %s +quit", buff,
                            paths.c_str(), in);
        }
...
void CMainFrame::OnViewShowblocks()
{
    ...
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_SHOWBLOCKS,
...
void CMainFrame::OnViewShowcoordinates()
{
    ...
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_SHOWCOORDINATES,
...
void CMainFrame::OnViewShownames()
{
    ...
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_SHOWNAMES,
...
void CMainFrame::OnMiscSelectentitycolor()
{
    ...
    if (DoColor(COLOR_ENTITY))
    {
        ...
        sprintf(buffer, "%f %f %f", g_qeglobals.d_savedinfo.colors[COLOR_ENTITY][0],
                g_qeglobals.d_savedinfo.colors[COLOR_ENTITY][1], g_qeglobals.d_savedinfo.colors[COLOR_ENTITY][2]);
...
{
    CEntKeyFindReplace FindReplace(&strFindKey, &strFindValue, &strReplaceKey, &strReplaceValue,
                                   &gbWholeStringMatchOnly, &gbSelectAllMatchingEnts);
...
void CMainFrame::SetGridStatus()
{
    ...
    strStatus.Format("G:%1.2f T:%1.2f R:%i C:%i L:%c%c", g_qeglobals.d_gridsize,
                     g_qeglobals.d_savedinfo.m_nTextureTweak, g_PrefsDlg.m_nRotation, g_PrefsDlg.m_nCubicScale, c1, c2);
...
void CMainFrame::NudgeSelection(int nDirection, float fAmount)
{
    ...
    else if (ActiveXY()->ScaleMode())
    {
        ...
        Select_Scale((g_nScaleHow & SCALE_X) ? v[0] : 1.0f, (g_nScaleHow & SCALE_Y) ? v[1] : 1.0f,
...
void CMainFrame::OnSelectByBoundingBrush()
{
    ...
    m_wndToolBar.GetToolBarCtrl().CheckButton(ID_SELECT_BYBOUNDINGBRUSH,
...
void CMainFrame::SetEntityCheck()
{
    ...
    if (pMenu)
    {
        pMenu->CheckMenuItem(ID_VIEW_ENTITIESAS_WIREFRAME,
                             MF_BYCOMMAND | (g_PrefsDlg.m_nEntityShowState == ENTITY_WIRE) ? MF_CHECKED : MF_UNCHECKED);
        pMenu->CheckMenuItem(ID_VIEW_ENTITIESAS_SKINNED,
                             MF_BYCOMMAND | (g_PrefsDlg.m_nEntityShowState == ENTITY_SKINNED) ? MF_CHECKED
                                                                                              : MF_UNCHECKED);
    }
}
...
void CMainFrame::OnViewOpengllighting()
{
    ...
    g_PrefsDlg.SavePrefs();
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_OPENGLLIGHTING,
                  MF_BYCOMMAND | (g_PrefsDlg.m_bGLLighting) ? MF_CHECKED : MF_UNCHECKED);
    Sys_UpdateWindows(W_XY | W_CAMERA);
}
...
{
    CFileDialog dlgFile(FALSE, "pfb", NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, "Prefab files (*.pfb)|*.pfb||",
                        this);
    char CurPath[1024];
...
{
    CFileDialog dlgFile(TRUE, "pfb", NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, "Prefab files (*.pfb)|*.pfb||",
                        this);
    char CurPath[1024];
    ::GetCurrentDirectory(1024, CurPath);
    dlgFile.m_ofn.lpstrInitialDir = CurPath;
...
void CMainFrame::OnSelectionCombine()
{
    ...
    if (e1->eclass->nShowFlags & ECLASS_LIGHT)
    {
        ...
        sprintf(str, "%g %g %g %g %g %g %g %g %g", mat[0][0], mat[0][1], mat[0][2], mat[1][0], mat[1][1], mat[1][2],
        ...
        if (b->owner != e1)
        {
            if (e1->eclass->nShowFlags & ECLASS_LIGHT)
            {
                ...
                if (GetMatrixForKey(b->owner, "rotation", mat))
                {
                    ...
                    sprintf(str, "%g %g %g %g %g %g %g %g %g", mat[0][0], mat[0][1], mat[0][2], mat[1][0], mat[1][1],
...
void CMainFrame::OnViewRendermode()
{
    ...
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_RENDERMODE,
...
void CMainFrame::OnViewRealtimerebuild()
{
    ...
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_REALTIMEREBUILD,
...
void CMainFrame::OnViewRenderentityoutlines()
{
    ...
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_RENDERENTITYOUTLINES,
...
void CMainFrame::OnViewMaterialanimation()
{
    ...
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_MATERIALANIMATION,
                  MF_BYCOMMAND | (m_pCamWnd->GetAnimationMode()) ? MF_CHECKED : MF_UNCHECKED);
    Sys_UpdateWindows(W_ALL);
}
...
void CMainFrame::OnViewRenderselection()
{
    ...
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_RENDERSELECTION,
...
void CMainFrame::OnViewRendersound()
{
    ...
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_RENDERSOUND,
