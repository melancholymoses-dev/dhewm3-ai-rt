...
bool UpdateActiveDragPoint(const idVec3 &move)
{
    if (activeDrag)
    {
        ...
        if (activeDrag->nType == LIGHT_TARGET)
        {
            ...
            target += (activeDrag->pBrush->trackLightOrigin) ? activeDrag->pBrush->owner->lightOrigin
        ...
        else if (activeDrag->nType == LIGHT_START)
        {
            ...
            start += (activeDrag->pBrush->trackLightOrigin) ? activeDrag->pBrush->owner->lightOrigin
                                                            : activeDrag->pBrush->owner->origin;
        ...
        else if (activeDrag->nType == LIGHT_END)
        {
            ...
            end += (activeDrag->pBrush->trackLightOrigin) ? activeDrag->pBrush->owner->lightOrigin
                                                          : activeDrag->pBrush->owner->origin;
...
void CXYWnd::OnRButtonDown(UINT nFlags, CPoint point)
{
    ...
    if (g_PrefsDlg.m_nMouseButtons == 3)
    { // 3 button mouse
        if ((GetAsyncKeyState(VK_CONTROL) & 0x8000))
        {
            if (ClipMode())
...
void CXYWnd::OnMouseMove(UINT nFlags, CPoint point)
{
    ...
    if (g_PrefsDlg.m_bChaseMouse == TRUE && (point.x < 0 || point.y < 0 || point.x > m_nWidth || point.y > m_nHeight) &&
        GetCapture() == this)
    {
...
void CXYWnd::OnPaint()
{
    ...
    if (bPaint)
    {
        ...
        if (PathMode())
        {
            ...
            for (n = 0; n < g_nPathCount; n++)
            {
                qglRasterPos3f(g_PathPoints[n].m_ptClip[0] + 2, g_PathPoints[n].m_ptClip[1] + 2,
...
void CXYWnd::OnEntityCreate(unsigned int nID)
{
    if (m_mnuDrop.GetSafeHmenu())
    {
        ...
        if (strItem.CompareNoCase("Add to...") == 0 || strItem.CompareNoCase("Remove") == 0 ||
            strItem.CompareNoCase("Name...") == 0 || strItem.CompareNoCase("New group...") == 0)
        {
...
bool MergeMenu(CMenu *pMenuDestination, const CMenu *pMenuAdd, bool bTopLevel /*=false*/)
{
    ...
    for (int iLoop = 0; iLoop < iMenuAddItemCount; iLoop++)
    {
        ...
        else
        {
            ...
            if (pMenuDestination->InsertMenu(iInsertPosDefault, MF_BYPOSITION | MF_POPUP | MF_ENABLED,
        ...
        for (e = eclass; e; e = e->next)
        {
            ...
            if (n_ > 0)
            {
                ...
                else
                {
                    if (pChild)
                    {
                        pMakeEntityPop->AppendMenu(MF_POPUP, reinterpret_cast<UINT_PTR>(pChild->GetSafeHmenu()),
                                                   strActive);
...
void GLColoredBoxWithLabel(float x, float y, float size, idVec4 color, const char *text, idVec4 textColor, float xofs,
                           float yofs, float lineSize)
{
...
void CXYWnd::XY_Draw()
{
    ...
    qglClearColor(g_qeglobals.d_savedinfo.colors[COLOR_GRIDBACK][0], g_qeglobals.d_savedinfo.colors[COLOR_GRIDBACK][1],
    ...
    for (brush = active_brushes.next; brush != &active_brushes; brush = brush->next)
    {
        ...
        else if (brush->mins[nDim1] > maxs[0] || brush->mins[nDim2] > maxs[1] || brush->maxs[nDim1] < mins[0] ||
    ...
    qglPushMatrix();
    qglTranslatef(g_qeglobals.d_select_translate[0], g_qeglobals.d_select_translate[1],
    ...
    if (g_qeglobals.d_select_mode == sel_area)
    {
        ...
        qglBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
        qglColor4f(0.0, 0.0, 1.0, 0.25);
        qglRectf(g_qeglobals.d_vAreaTL[nDim1], g_qeglobals.d_vAreaTL[nDim2], g_qeglobals.d_vAreaBR[nDim1],
...
void CXYWnd::Paste()
{
    ...
    if (g_brClipboard.next != &g_brClipboard || g_enClipboard.next != &g_enClipboard)
    {
        ...
        for (entity_t *pEntity = g_enClipboard.next; pEntity != NULL && pEntity != &g_enClipboard;
