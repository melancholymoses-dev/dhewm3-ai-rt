...
void Select_AutoCaulk()
{
    ...
    for (brush_t *pSelectedBrush = selected_brushes.next; pSelectedBrush != &selected_brushes; pSelectedBrush = next)
    {
        ...
        {
            brush_t *snext;
            for (brush_t *pScannedBrush = (iBrushListToScan ? active_brushes.next : selected_brushes.next);
                 pScannedBrush != (iBrushListToScan ? &active_brushes : &selected_brushes); pScannedBrush = snext)
            {
                        ...
                        */
                        {
                            ...
                            if (v3ZeroTest == v3Zero)
                            {
                                ...
                                {
                                    fTotalDist +=
                                        fabs(DotProduct(pSelectedFace->plane.Normal(), (*pSelectedWinding)[0]) -
                                ...
                                {
                                    AddPointToBounds((*pScannedWinding)[iPoint].ToVec3(), v3ScannedBoundsMins,
                                                     v3ScannedBoundsMaxs);
                                }
