...
{
    return (double)a[0] * (double)b[1] * (double)c[2] + (double)b[0] * (double)c[1] * (double)a[2] +
           (double)c[0] * (double)a[1] * (double)b[2] - (double)c[0] * (double)b[1] * (double)a[2] -
           (double)a[1] * (double)b[0] * (double)c[2] - (double)a[0] * (double)b[2] * (double)c[1];
}
...
void EmitBrushPrimitTextureCoordinates(face_t *f, idWinding *w, patchMesh_t *patch)
{
    ...
    if (w)
    {
        for (i = 0; i < w->GetNumPoints(); i++)
        {
            ...
            (*w)[i][3] = f->brushprimit_texdef.coords[0][0] * x + f->brushprimit_texdef.coords[0][1] * y +
                         f->brushprimit_texdef.coords[0][2];
            (*w)[i][4] = f->brushprimit_texdef.coords[1][0] * x + f->brushprimit_texdef.coords[1][1] * y +
        ...
        for (i = 0; i < patch->width; i++)
        {
            for (j = 0; j < patch->height; j++)
            {
                ...
                patch->ctrl(i, j).st.x = f->brushprimit_texdef.coords[0][0] * x +
                                         f->brushprimit_texdef.coords[0][1] * y + f->brushprimit_texdef.coords[0][2];
                patch->ctrl(i, j).st.y = f->brushprimit_texdef.coords[1][0] * x +
                                         f->brushprimit_texdef.coords[1][1] * y + f->brushprimit_texdef.coords[1][2];
            }
        }
    }
}
...
void ConvertTexMatWithQTexture(float texMat1[2][3], const idMaterial *qtex1, float texMat2[2][3],
                               const idMaterial *qtex2, float sScale = 1.0, float tScale = 1.0)
{
    float s1, s2;
    s1 = (qtex1 ? static_cast<float>(qtex1->GetEditorImage()->uploadWidth) : 2.0f) /
         (qtex2 ? static_cast<float>(qtex2->GetEditorImage()->uploadWidth) : 2.0f);
    s2 = (qtex1 ? static_cast<float>(qtex1->GetEditorImage()->uploadHeight) : 2.0f) /
         (qtex2 ? static_cast<float>(qtex2->GetEditorImage()->uploadHeight) : 2.0f);
    s1 *= sScale;
    s2 *= tScale;
...
/*
 ...
 =======================================================================================================================
 */
void ConvertTexMatWithQTexture(brushprimit_texdef_t *texMat1, const idMaterial *qtex1, brushprimit_texdef_t *texMat2,
                               const idMaterial *qtex2, float sScale, float tScale)
{
...
void Face_FitTexture_BrushPrimit(face_t *f, idVec3 mins, idVec3 maxs, float height, float width)
{
    ...
    Mf[0][0] =
        N.coords[0][0] * f->brushprimit_texdef.coords[0][0] + N.coords[0][1] * f->brushprimit_texdef.coords[1][0];
    Mf[0][1] =
        N.coords[0][0] * f->brushprimit_texdef.coords[0][1] + N.coords[0][1] * f->brushprimit_texdef.coords[1][1];
    Mf[0][2] = N.coords[0][0] * f->brushprimit_texdef.coords[0][2] +
               N.coords[0][1] * f->brushprimit_texdef.coords[1][2] + N.coords[0][2];
    Mf[1][0] =
        N.coords[1][0] * f->brushprimit_texdef.coords[0][0] + N.coords[1][1] * f->brushprimit_texdef.coords[1][0];
    Mf[1][1] =
        N.coords[1][0] * f->brushprimit_texdef.coords[0][1] + N.coords[1][1] * f->brushprimit_texdef.coords[1][1];
    Mf[1][2] = N.coords[1][0] * f->brushprimit_texdef.coords[0][2] +
               N.coords[1][1] * f->brushprimit_texdef.coords[1][2] + N.coords[1][2];
...
void Face_GetScale_BrushPrimit(face_t *face, float *s, float *t, float *rot)
{
    ...
    *t = idMath::Sqrt(aux[0][1] * aux[0][1] + aux[1][1] * aux[1][1]);
    ...
    {
        *rot = RAD2DEG(atan2(face->brushprimit_texdef.coords[1][0] / (*s) ? (*s) : 1.0f,
...
void Face_SetAxialScale_BrushPrimit(face_t *face, bool y)
{
    ...
    if (g_bAxialMode)
    {
        if (g_axialAnchor >= 0 && g_axialAnchor < face->face_winding->GetNumPoints() && g_axialDest >= 0 &&
            g_axialDest < face->face_winding->GetNumPoints() && g_axialAnchor != g_axialDest)
        {
