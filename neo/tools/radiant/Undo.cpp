...
void Undo_Undo(void)
{
    ...
    for (pEntity = entities.next; pEntity != NULL && pEntity != &entities; pEntity = pNextEntity)
    {
        ...
        if (pEntity->undoId == undo->id)
        {
            ...
            for (pUndoEntity = undo->entitylist.next; pUndoEntity != NULL && pUndoEntity != &undo->entitylist;
    ...
    for (pEntity = undo->entitylist.next; pEntity != NULL && pEntity != &undo->entitylist;
...
void Undo_Redo(void)
{
    ...
    for (pEntity = entities.next; pEntity != NULL && pEntity != &entities; pEntity = pNextEntity)
    {
        ...
        if (pEntity->redoId == redo->id)
        {
            ...
            for (pRedoEntity = redo->entitylist.next; pRedoEntity != NULL && pRedoEntity != &redo->entitylist;
    ...
    // add the undo entities back into the entity list
    for (pEntity = redo->entitylist.next; pEntity != NULL && pEntity != &redo->entitylist;
