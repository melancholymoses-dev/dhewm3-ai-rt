...
void SetKeyMat3(entity_t *ent, const char *key, idMat3 m)
{
    ...
    sprintf(str, "%g %g %g %g %g %g %g %g %g", m[0][0], m[0][1], m[0][2], m[1][0], m[1][1], m[1][2], m[2][0], m[2][1],
            m[2][2]);
...
bool GetMatrixForKey(entity_t *ent, const char *key, idMat3 &mat)
{
    ...
    if (k && strlen(k) > 0)
    {
        sscanf(k, "%f %f %f %f %f %f %f %f %f ", &mat[0][0], &mat[0][1], &mat[0][2], &mat[1][0], &mat[1][1], &mat[1][2],
...
const EpairFixup FloatFixups[] = {{"origin", 1},      {"rotation", 2},     {"_color", 1},    {"falloff", 0},
                                  {"light", 0},       {"light_target", 1}, {"light_up", 1},  {"light_right", 1},
                                  {"light_start", 1}, {"light_center", 1}, {"light_end", 1}, {"light_radius", 1},
                                  {"light_origin", 1}};
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        for (int j = 0; j < FixupCount; j++)
        {
            if (kv->GetKey().Icmp(FloatFixups[j].name) == 0)
            {
                ...
                {
                    idMat3 mat;
                    sscanf(kv->GetValue().c_str(), "%f %f %f %f %f %f %f %f %f ", &mat[0][0], &mat[0][1], &mat[0][2],
                           &mat[1][0], &mat[1][1], &mat[1][2], &mat[2][0], &mat[2][1], &mat[2][2]);
                    sprintf(val, "%g %g %g %g %g %g %g %g %g", mat[0][0], mat[0][1], mat[0][2], mat[1][0], mat[1][1],
...
void Entity_UpdateSoundEmitter(entity_t *ent)
{
    ...
    if (g_pParentWnd->GetCamera()->GetSoundMode() && ent->brushes.onext != &ent->brushes &&
