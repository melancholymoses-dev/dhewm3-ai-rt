...
bool Map_SaveFile(const char *filename, bool use_region, bool autosave)
{
    ...
    {
        CFileDialog dlgSave(FALSE, "map", NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, "Map Files (*.map)|*.map||",
                            AfxGetMainWnd());
...
void Map_RegionXY(void)
{
    ...
    region_mins[0] = g_pParentWnd->GetXYWnd()->GetOrigin()[0] -
                     0.5 * g_pParentWnd->GetXYWnd()->Width() / g_pParentWnd->GetXYWnd()->Scale();
    region_maxs[0] = g_pParentWnd->GetXYWnd()->GetOrigin()[0] +
                     0.5 * g_pParentWnd->GetXYWnd()->Width() / g_pParentWnd->GetXYWnd()->Scale();
    region_mins[1] = g_pParentWnd->GetXYWnd()->GetOrigin()[1] -
                     0.5 * g_pParentWnd->GetXYWnd()->Height() / g_pParentWnd->GetXYWnd()->Scale();
...
bool WriteFileString(FILE *fp, char *string, ...)
{
    ...
    while (*string)
    {
        ...
        case '%':
            string++;
            while ((*string >= '0' && *string <= '9') || *string == '.' || *string == '-' || *string == '+' ||
                   *string == '#')
            {
                string++;
            }
