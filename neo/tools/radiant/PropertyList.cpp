...
void CPropertyList::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
    ...
    if (nIndex != (UINT)-1)
    {
        ...
        dc.DrawText(pItem->m_propName, CRect(rect2.left + s3, rect2.top + s3, rect2.right - s3, rect2.bottom + s3),
                    DT_LEFT | DT_SINGLELINE);

        // write the initial property value in the second rectangle
        dc.DrawText(pItem->m_curValue, CRect(rect.left + s3, rect.top + s3, rect.right + s3, rect.bottom + s3),
                    DT_LEFT | (pItem->m_nItemType == PIT_VAR) ? DT_WORDBREAK : DT_SINGLELINE);
    }
    dc.Detach();
}
...
void CPropertyList::OnSelchange()
{
    ...
    if (pItem->m_nItemType == PIT_COMBO)
    {
        ...
        {
            rect.bottom += (s3 * 100);
            m_cmbBox.Create(CBS_DROPDOWNLIST | WS_VSCROLL | WS_VISIBLE | WS_CHILD | WS_BORDER, rect, this,
                            IDC_PROPCMBBOX);
