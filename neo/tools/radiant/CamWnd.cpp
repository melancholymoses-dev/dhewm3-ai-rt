...
void CCamWnd::OnPaint()
{
    ...
    else
    {
        ...
        if (g_bClipMode)
        {
            ...
            {
                g_pSplitList = ((g_pParentWnd->ActiveXY()->GetViewType() == XZ) ? !g_bSwitch : g_bSwitch)
                                   ? &g_brBackSplits
                                   : &g_brFrontSplits;
            }
        }
...
void CCamWnd::Cam_Draw()
{
    ...
    setGLMode(m_Camera.draw_mode);
    qglDisable(GL_LIGHTING);
    qglColor4f(g_qeglobals.d_savedinfo.colors[COLOR_SELBRUSHES][0], g_qeglobals.d_savedinfo.colors[COLOR_SELBRUSHES][1],
...
{
#if USE_COLLADA
    CFileDialog dlgFile(FALSE, "lwo, ase, dae", NULL, 0, "(*.lwo)|*.lwo|(*.ase)|*.ase|(*.ma)|*.ma|(*.dae)|*.dae||",
                        g_pParentWnd);
#else
