...
const int EditEnableID[] = {IDC_BUTTON_XDN,         IDC_BUTTON_XUP,    IDC_BUTTON_YUP,
...
CDialogParticleEditor::CDialogParticleEditor(CWnd *pParent /*=NULL*/)
    : CDialog(CDialogParticleEditor::IDD, pParent), matName(_T("")), animFrames(_T("")), animRate(_T("")),
      color(_T("")), fadeColor(_T("")), fadeIn(_T("")), fadeOut(_T("")), fadeFraction(_T("")), count(_T("")),
      time(_T("")), timeOffset(_T("")), deadTime(_T("")), gravity(_T("")), bunching(_T("")), offset(_T("")),
      xSize(_T("")), ySize(_T("")), zSize(_T("")), ringOffset(_T("")), directionParm(_T("")), direction(0),
      orientation(0), distribution(0), viewOrigin(_T("")), speedFrom(_T("")), speedTo(_T("")), rotationFrom(_T("")),
      rotationTo(_T("")), sizeFrom(_T("")), sizeTo(_T("")), aspectFrom(_T("")), aspectTo(_T("")), customPath(_T("")),
      customParms(_T("")), trails(_T("")), trailTime(_T("")), worldGravity(TRUE), entityColor(TRUE),
      randomDistribution(TRUE), initialAngle(_T("")), boundsExpansion(_T("")), customDesc(_T("")), particleMode(FALSE)
{
...
void CDialogParticleEditor::OnBnClickedButtonSaveAs()
{
    ...
    if (dlg.DoModal() == IDOK)
    {
        ...
        CFileDialog dlgSave(TRUE, "prt", NULL, OFN_CREATEPROMPT, "Particle Files (*.prt)|*.prt||All Files (*.*)|*.*||",
                            AfxGetMainWnd());
        if (dlgSave.DoModal() == IDOK)
        {
            ...
            idDeclParticle *decl =
                dynamic_cast<idDeclParticle *>(declManager->CreateNewDecl(DECL_PARTICLE, dlg.m_strName, fileName));
            if (decl)
            {
...
void CDialogParticleEditor::DlgVarsToCurStage()
{
    ...
    sscanf(customParms, "%f %f %f %f %f %f %f %f", &ps->customPathParms[0], &ps->customPathParms[1],
           &ps->customPathParms[2], &ps->customPathParms[3], &ps->customPathParms[4], &ps->customPathParms[5],
...
void CDialogParticleEditor::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar *pScrollBar)
{
    ...
    // DG: from SteelStorm2:
    // Something funky is going on with the RTTI.  The dynamic_cast even to a
    // CRangeSlider* was not happening correctly.  Whatever is getting to this
    // callback from the CRangeSlider must not be one of the basic slider types or
    // even a CRangeSlider.  What is weird is the objects coming in have the same
    // addresses as the various CRangeSliders on the dlg, so they should cast
    // correctly.  Turns out it does not matter because once the addresses matche
    // up this code just uses the dlg member reference so all I need is for the
    // address check to go through correctly.
