...
bool rvDebuggerServer::ProcessMessages(void)
{
    ...
    while (mPort.GetPacket(adrFrom, buffer, msgSize, MAX_MSGLEN))
    {
        ...
        case DBMSG_STEPOVER:
            ...
            {
                mBreakStepOverFunc2 = ((idGameEditExt *)gameEdit)
                                          ->GetInterpreterCallStackFunction(mBreakInterpreter, mBreakStepOverDepth - 1);
            }
            else
            {
                mBreakStepOverFunc2 = NULL;
...
void rvDebuggerServer::HandleInspectCallstack(idBitMsg *msg)
{
    ...
    ((idGameEditExt *)gameEdit)
        ->MSG_WriteInterpreterInfo(&msgOut, mBreakInterpreter, mBreakProgram, mBreakInstructionPointer);
...
void rvDebuggerServer::HandleInspectThreads(idBitMsg *msg)
{
    ...
    {
        ((idGameEditExt *)gameEdit)
            ->MSG_WriteThreadInfo(&msgOut, ((idGameEditExt *)gameEdit)->GetThreadByIndex(i), mBreakInterpreter);
    }
...
void rvDebuggerServer::CheckBreakpoints(idInterpreter *interpreter, idProgram *program, int instructionPointer)
{
    ...
    if (mBreakStepOver)
    {
        ...
        if (((idGameEditExt *)gameEdit)
                ->CheckForBreakPointHit(interpreter, mBreakStepOverFunc1, mBreakStepOverFunc2, mBreakStepOverDepth))
        {
            Break(interpreter, program, instructionPointer);
            return;
        }
