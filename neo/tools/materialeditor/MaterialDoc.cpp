    ...
    if (orig.Icmp(value))
    {
        ...
        if (addUndo)
        {
            ...
            AttributeMaterialModifierString *mod =
                new AttributeMaterialModifierString(manager, name, stage, attribName, value, orig);
...
void MaterialDoc::SetAttributeBool(int stage, const char *attribName, bool value, bool addUndo)
{
    ...
    if (orig != value)
    {
        ...
        {
            // Create a new Modifier for this change so we can undo and redo later
            AttributeMaterialModifierBool *mod =
                new AttributeMaterialModifierBool(manager, name, stage, attribName, value, orig);
            manager->AddMaterialUndoModifier(mod);
        }
...
void MaterialDoc::Save()
{
    ...
    {
        MessageBox(GetMaterialEditorWindow(), va("Unable to save '%s'. It may be read-only", name.c_str()),
