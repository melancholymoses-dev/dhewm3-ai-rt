...
void idGLDrawableView::UpdateLights(void)
{
    ...
    for (i = 0; i < viewLights.Num(); i++)
    {
        ...
        vLight->renderLight.lightRadius[0] = vLight->renderLight.lightRadius[1] = vLight->renderLight.lightRadius[2] =
            vLight->radius;
...
void idGLDrawableView::drawLights(renderView_t *refdef)
{
    ...
    for (i = 0; i < viewLights.Num(); i++)
    {
        ...
        session->rw->DrawText(va("%d", i + 1), vLight->renderLight.origin + idVec3(0, 0, 5), 0.25f, idVec4(1, 1, 0, 1),
