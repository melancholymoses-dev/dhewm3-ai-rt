...
int StageView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
    ...
    SetToggleIcons(MAKEINTRESOURCE(IDI_ME_DISABLED_ICON), MAKEINTRESOURCE(IDI_ME_ON_ICON),
...
void StageView::OnDeleteStage()
{
    ...
    if (pos)
    {
        ...
        {
            int result =
                MessageBox("Are you sure you want to delete this stage?", "Delete?", MB_ICONQUESTION | MB_YESNO);
            if (result == IDYES)
            {
...
void StageView::OnPaste()
{
    if (materialDocManager->IsCopyStage())
    {
        ...
        {
            if (MessageBox(va("Do you want to replace '%s' stage?", name.c_str()), "Replace?",
