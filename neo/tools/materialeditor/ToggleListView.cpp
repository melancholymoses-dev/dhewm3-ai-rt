...
void ToggleListView::SetToggleIcons(LPCSTR disabled, LPCSTR on, LPCSTR off)
{
    ...
    {
        offIcon =
            (HICON)LoadImage(AfxGetInstanceHandle(), off, IMAGE_ICON, 16, 16, LR_DEFAULTCOLOR | LR_LOADMAP3DCOLORS);
    }
    else
    {
        offIcon = NULL;
    ...
    if (disabled)
    {
        disabledIcon = (HICON)LoadImage(AfxGetInstanceHandle(), disabled, IMAGE_ICON, 16, 16,
