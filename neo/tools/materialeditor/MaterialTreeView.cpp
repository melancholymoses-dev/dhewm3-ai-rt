...
void MaterialTreeView::DeleteFolder(HTREEITEM item, bool addUndo)
{
    ...
    if (addUndo)
    {
        DeleteMaterialFolderModifier *mod = new DeleteMaterialFolderModifier(
            materialDocManager, tree.GetItemText(item), this, tree.GetParentItem(item), &affectedMaterials);
...
int MaterialTreeView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
    ...
    lpCreateStruct->style |=
        TVS_HASLINES | TVS_HASBUTTONS | TVS_LINESATROOT | TVS_EDITLABELS | TVS_SHOWSELALWAYS | TVS_INFOTIP;
    ...
    if (pTVDispInfo->item.pszText)
    {
        ...
        else if (type == TYPE_MATERIAL_FOLDER)
        {
            ...
            RenameMaterialFolderModifier *mod =
                new RenameMaterialFolderModifier(materialDocManager, pTVDispInfo->item.pszText, this,
...
void MaterialTreeView::OnDeleteMaterial()
{
    ...
    {
        int result =
            MessageBox("Are you sure you want to delete this material?", "Delete?", MB_ICONQUESTION | MB_YESNO);
        if (result == IDYES)
        {
...
void MaterialTreeView::OnPaste()
{
    ...
    if (item && materialDocManager->IsCopyMaterial() && itemType >= TYPE_FILE)
    {
        ...
        // Generate the name
        if (itemType == TYPE_MATERIAL)
        { // Backup one if a file is selected
            item = tree.GetParentItem(item);
