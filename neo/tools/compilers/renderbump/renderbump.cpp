...
static triHash_t *CreateTriHash(const srfTriangles_t *highMesh)
{
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        if (hash->binSize[i] <= 0)
        {
            common->FatalError("CreateTriHash: bad bounds: (%f %f %f) to (%f %f %f)", bounds[0][0], bounds[0][1],
...
static float TraceToMeshFace(const srfTriangles_t *highMesh, int faceNum, float minDist, float maxDist,
                             const idVec3 &point, const idVec3 &normal, idVec3 &sampledNormal, byte sampledColor[4])
{
    int j;
    float dist;
    const idVec3 *v[3];
...
================
*/
static bool SampleHighMesh(const renderBump_t *rb, const idVec3 &point, const idVec3 &direction, idVec3 &sampledNormal,
                           byte sampledColor[4])
{
    idVec3 p;
    binLink_t *bl;
...
void RenderBump_f(const idCmdArgs &args)
{
    ...
    for (i = 0; i < lowPoly->NumSurfaces(); i++)
    {
        ...
        for (j = 0; j < numRenderBumps; j++)
        {
            ...
            if (idStr::Icmp(rb->highName, opt.highName) || rb->width != opt.width || rb->height != opt.height ||
                rb->antiAlias != opt.antiAlias || rb->traceFrac != opt.traceFrac)
            {
