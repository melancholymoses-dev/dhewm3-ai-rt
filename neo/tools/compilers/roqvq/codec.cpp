...
void codec::Segment(int *alist, float *flist, int numElements, float rmse)
{
    ...
    if (doopen)
    {
        ...
        for (onf = 0; onf < 256; onf++)
        {
            ...
            for (x = 0; x < 4; x++)
            {
                fy = RMULT * (float)(codebook2[onf][numc + 0]) + GMULT * (float)(codebook2[onf][numc + 1]) +
...
void codec::FvqData(byte *bitmap, int size, int realx, int realy, quadcel *pquad, bool clamp)
{
    ...
    if (((realx - xxMean) + searchX) < 0 || (((realx - xxMean) - searchX) + depthx + size) > xLen ||
        ((realy - yyMean) + searchY) < 0 || (((realy - yyMean) - searchY) + depthy + size) > yLen)
    {
        pquad->snr[FCC] = 9999;
        return;
    }
    ...
    {
        for (sY = (((realy - yyMean) - searchY) + depthy); sY <= ((realy - yyMean) + searchY) && breakHigh;
             sY += depthy)
        {
            temp1 = xLen * sY + sX;
...
int codec::AddQuad(quadcel *pquad, int lownum)
{
    ...
    if (lownum != -1)
    {
        ...
        if (pquad[lownum + nx * 0 + 1].status == MOT && pquad[lownum + nx * 1 + 1].status == MOT &&
            pquad[lownum + nx * 2 + 1].status == MOT && pquad[lownum + nx * 3 + 1].status == MOT && nsize == 4)
        {
            newsnr = 9999;
        ...
        {
            theRoQ->MarkQuadx(pquad[lownum].xat, pquad[lownum].yat, nsize * 2, pquad[lownum].rsnr,
...
void codec::SparseEncode(void)
{
    ...
    if (theRoQ->IsQuiet() == false)
    {
        common->Printf("sparseEncode: rmse of quad0 is %f, size is %d (meant to be %d)\n", sRMSE,
                       GetCurrentQuadOutputSize(qStatus), fsize);
    }
...
void codec::VQ(const int numEntries, const int dimension, const unsigned char *vectors, float *import,
               VQDATA **codebook, const bool optimize)
{
