...
static void FragmentSilQuad(silQuad_t quad, silPlane_t *silPlane, shadowOptEdge_t *startEdge, shadowOptEdge_t *skipEdge)
{
    ...
    for (shadowOptEdge_t *check = startEdge; check; check = check->nextEdge)
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            if ((d1 > EDGE_PLANE_EPSILON && d3 > EDGE_PLANE_EPSILON && d2 < -EDGE_PLANE_EPSILON &&
                 d4 < -EDGE_PLANE_EPSILON) ||
                (d2 > EDGE_PLANE_EPSILON && d4 > EDGE_PLANE_EPSILON && d1 < -EDGE_PLANE_EPSILON &&
                 d3 < -EDGE_PLANE_EPSILON))
            {
                float f = d1 / (d1 - d2);
                float f2 = d3 / (d3 - d4);
                f = f2;
...
static int FindUniqueVert(idVec3 &v)
{
    ...
    for (k = 0; k < numUniqued; k++)
    {
        ...
        if (fabs(v[0] - check[0]) < UNIQUE_EPSILON && fabs(v[1] - check[1]) < UNIQUE_EPSILON &&
...
void CleanupOptimizedShadowTris(srfTriangles_t *tri)
{
    ...
    for (int i = 0; i < numSilIndexes; i += 6)
    {
        ...
        for (j = i + 6; j < numSilIndexes; j += 6)
        {
            ...
            // quad indexes
            if (tri->indexes[i + 0] == tri->indexes[j + 1] && tri->indexes[i + 1] == tri->indexes[j + 0] &&
                tri->indexes[i + 2] == tri->indexes[j + 3] && tri->indexes[i + 3] == tri->indexes[j + 5] &&
