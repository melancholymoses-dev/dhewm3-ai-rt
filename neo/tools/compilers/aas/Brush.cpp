    ...
    if (mid)
    {
        ...
        else if (mid->IsHuge())
        {
            ...
            common->Warning("brush %d on entity %d is unbounded"
                            ...
                            primitiveNum, entityNum, bounds[0][0], bounds[0][1], bounds[0][2], bounds[1][0],
                            bounds[1][1], bounds[1][2], bounds[1][0] - bounds[0][0], bounds[1][1] - bounds[0][1],
    ...
    for (i = 0; i < sides.Num(); i++)
    {
        ...
        else
        {
            ...
            else if (maxFrontWinding[i] > -BRUSH_EPSILON)
            {
                ...
                side = side->Copy();
                side->winding =
                    side->winding->Clip(idPlane(plane.Normal(), (plane.Dist() - (BRUSH_EPSILON + 0.02f))), 0.01f, true);
                assert(side->winding);
            ...
            else if (maxBackWinding[i] < BRUSH_EPSILON)
            {
                // favor an overconstrained brush
                side = side->Copy();
                side->winding = side->winding->Clip(idPlane(-plane.Normal(), -(plane.Dist() + (BRUSH_EPSILON + 0.02f))),
                                                    0.01f, true);
                assert(side->winding);
...
============
*/
void idBrushList::Split(const idPlane &plane, int planeNum, idBrushList &frontList, idBrushList &backList,
...
void idBrushList::CreatePlaneList(idPlaneSet &planeList) const
{
    ...
    for (b = Head(); b; b = b->Next())
    {
        for (i = 0; i < b->GetNumSides(); i++)
        {
            side = b->GetSide(i);
            side->SetPlaneNum(
                planeList.FindPlane(side->GetPlane(), BRUSH_PLANE_NORMAL_EPSILON, BRUSH_PLANE_DIST_EPSILON));
        }
    }
}
...
void idBrushMap::WriteBrush(const idBrush *brush)
{
    ...
    for (i = 0; i < brush->GetNumSides(); i++)
    {
        ...
        fp->WriteFloatString(" ( %f %f %f %f ) ", side->GetPlane()[0], side->GetPlane()[1], side->GetPlane()[2],
                             -side->GetPlane().Dist());
