...
{
    fp->WriteFloatString("\t\t%d %d (%f %f %f) (%f %f %f) %d %d", (int)reach->travelType, (int)reach->toAreaNum,
                         reach->start.x, reach->start.y, reach->start.z, reach->end.x, reach->end.y, reach->end.z,
...
bool idAASSettings::WriteToFile(idFile *fp) const
{
    ...
    for (i = 0; i < numBoundingBoxes; i++)
    {
        fp->WriteFloatString("\t\t(%f %f %f)-(%f %f %f)\n", boundingBoxes[i][0].x, boundingBoxes[i][0].y,
                             boundingBoxes[i][0].z, boundingBoxes[i][1].x, boundingBoxes[i][1].y,
                             boundingBoxes[i][1].z);
    }
    fp->WriteFloatString("\t}\n");
...
bool idAASSettings::ValidEntity(const char *classname) const
{
    ...
    if (playerFlood)
    {
        if (!strcmp(classname, "info_player_start") || !strcmp(classname, "info_player_deathmatch") ||
    ...
    const idDeclEntityDef *decl =
        static_cast<const idDeclEntityDef *>(declManager->FindType(DECL_ENTITYDEF, classname, false));
...
bool idAASFileLocal::Write(const idStr &fileName, unsigned int mapFileCRC)
{
    ...
    {
        aasFile->WriteFloatString("\t%d ( %f %f %f %f )\n", i, planeList[i].Normal().x, planeList[i].Normal().y,
    ...
    for (i = 0; i < areas.Num(); i++)
    {
        ...
        aasFile->WriteFloatString("\t%d ( %d %d %d %d %d %d ) %d {\n", i, areas[i].flags, areas[i].contents,
                                  areas[i].firstFace, areas[i].numFaces, areas[i].cluster, areas[i].clusterAreaNum,
                                  num);
    ...
    {
        aasFile->WriteFloatString("\t%d ( %d %d %d )\n", i, nodes[i].planeNum, nodes[i].children[0],
                                  nodes[i].children[1]);
    }
    aasFile->WriteFloatString("}\n");
