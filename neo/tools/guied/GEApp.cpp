...
bool rvGEApp::Initialize(void)
{
    ...
    mMDIFrame =
        CreateWindow("QUAKE4_GUIEDITOR_CLASS", "Quake IV GUI Editor", WS_OVERLAPPEDWINDOW | WS_CLIPSIBLINGS,
                     CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, mInstance, (LPVOID)this);
    ...
    mMDIClient = CreateWindow("MDICLIENT", NULL, WS_CHILDWINDOW | WS_CLIPSIBLINGS | WS_CLIPCHILDREN, 0, 0, 1000, 1000,
                              mMDIFrame, NULL, mInstance, &ccs);
...
bool rvGEApp::TranslateAccelerator(LPMSG msg)
{
    ...
    if (focus == mMDIClient || focus == mMDIFrame || focus == GetNavigator().GetWindow())
    {
...
void rvGEApp::HandleCommandSave(rvGEWorkspace *workspace, const char *filename)
{
    ...
    if (workspace->IsNew() || filename == NULL)
    {
        ...
        {
            if (IDNO == MessageBox(va("File '%s' already exists. Do you want to replace it?", realFilename.c_str()),
...
int rvGEApp::HandleCommand(WPARAM wParam, LPARAM lParam)
{
    ...
    case ID_GUIED_SOURCECONTROL_UNDOCHECKOUT:
        assert(workspace);
        if (IDYES ==
            MessageBox(va("Are you sure you want to undo the checkout of the file '%s'?", workspace->GetFilename()),
                       MB_YESNO | MB_ICONQUESTION))
        {
    ...
    for (nPos = 0; nPos < cMenuItems; nPos++)
    {
        ...
        case ID_GUIED_EDIT_COPY:
            EnableMenuItem(hmenu, nPos,
                           MF_BYPOSITION | (workspace->GetSelectionMgr().Num() > 0 ? MF_ENABLED : MF_GRAYED));
            break;
        ...
        case ID_GUIED_ITEM_SCRIPTS:
            EnableMenuItem(hmenu, nPos,
                           MF_BYPOSITION | (workspace->GetSelectionMgr().Num() == 1 ? MF_ENABLED : MF_GRAYED));
            break;
        ...
        case ID_GUIED_SOURCECONTROL_UNDOCHECKOUT:
            EnableMenuItem(
                hmenu, nPos,
                MF_BYPOSITION |
                    ((workspace->GetSourceControlState() == rvGEWorkspace::SCS_CHECKEDOUT) ? MF_ENABLED : MF_GRAYED));
            break;
        ...
        case ID_GUIED_SOURCECONTROL_CHECKOUT:
            EnableMenuItem(
                hmenu, nPos,
                MF_BYPOSITION |
                    ((workspace->GetSourceControlState() == rvGEWorkspace::SCS_CHECKEDIN) ? MF_ENABLED : MF_GRAYED));
            break;
...
bool rvGEApp::NewFile(void)
{
    ...
    if (workspace->NewFile())
    {
        ...
        child = CreateMDIWindow("QUAKE4_GUIEDITOR_CHILD_CLASS", "Untitled.gui",
                                ...
                                CW_USEDEFAULT, CW_USEDEFAULT, 640, 480, mMDIClient, mInstance, (LONG_PTR)workspace);
