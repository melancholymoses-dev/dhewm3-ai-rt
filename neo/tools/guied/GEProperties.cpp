...
bool rvGEProperties::Create(HWND parent, bool visible)
{
    ...
    mWnd = CreateWindowEx(WS_EX_TOOLWINDOW, "GUIEDITOR_PROPERTIES_CLASS", "Properties",
    ...
    if (!gApp.GetOptions().GetWindowPlacement("properties", mWnd))
    {
        ...
        SetWindowPos(mWnd, NULL, rParent.right - 10 - (rClient.right - rClient.left),
                     rParent.bottom - 10 - (rClient.bottom - rClient.top), 0, 0, SWP_NOZORDER | SWP_NOSIZE);
    }
