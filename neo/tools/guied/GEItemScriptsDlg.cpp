...
INT_PTR CALLBACK GEItescriptsDlg_WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_SIZE: {
        ...
        MoveWindow(GetDlgItem(hwnd, IDC_GUIED_SCRIPT), rClient.left, rClient.top, rClient.right - rClient.left,
...
bool GEItemScriptsDlg_DoModal(HWND parent, idWindow *window)
{
    ...
    if (DialogBoxParam(gApp.GetInstance(), MAKEINTRESOURCE(IDD_GUIED_SCRIPTS), parent, GEItescriptsDlg_WndProc,
                       (LPARAM)window))
    {
        return true;
    }
