...
static INT_PTR CALLBACK GEOptionsDlg_GeneralProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        ...
        CheckDlgButton(hwnd, IDC_GUIED_IGNOREDESKTOP,
                       gApp.GetOptions().GetIgnoreDesktopSelect() ? BST_CHECKED : BST_UNCHECKED);
        break;
    ...
    case WM_NOTIFY:
        ...
        {
        case PSN_APPLY:
            gApp.GetOptions().SetLastOptionsPage(
                PropSheet_HwndToIndex(GetParent(hwnd), PropSheet_GetCurrentPageHwnd(GetParent(hwnd))));
...
static INT_PTR CALLBACK GEOptionsDlg_GridProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        ...
        ColorButton_SetColor(GetDlgItem(hwnd, IDC_GUIED_GRIDCOLOR),
                             RGB(gApp.GetOptions().GetGridColor()[0] * 255, gApp.GetOptions().GetGridColor()[1] * 255,
    ...
    case WM_COMMAND:
        ...
        case IDC_GUIED_GRIDCOLOR: {
            ...
            col.hInstance = NULL;
            col.Flags = CC_RGBINIT;
            col.rgbResult = RGB(gApp.GetOptions().GetGridColor()[0] * 255, gApp.GetOptions().GetGridColor()[1] * 255,
                                gApp.GetOptions().GetGridColor()[2] * 255);
            if (ChooseColor(&col))
            {
