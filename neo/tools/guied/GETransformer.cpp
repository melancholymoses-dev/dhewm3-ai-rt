...
bool rvGETransformer::Create(HWND parent, bool visible)
{
    ...
    mWnd = CreateWindowEx(WS_EX_TOOLWINDOW, "GUIEDITOR_TRANSFORMER_CLASS", "Transformer",
    ...
    if (!gApp.GetOptions().GetWindowPlacement("transformer", mWnd))
    {
        ...
        SetWindowPos(mWnd, NULL, rParent.right - 10 - (rTrans.right - rTrans.left),
                     rParent.bottom - 10 - (rTrans.bottom - rTrans.top), 0, 0, SWP_NOZORDER | SWP_NOSIZE);
    }
...
LRESULT CALLBACK rvGETransformer::WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE: {
        ...
        trans->mWnd = hWnd;
        trans->mDlg =
            CreateDialogParam(gApp.GetInstance(), MAKEINTRESOURCE(IDD_GUIED_TRANSFORMER), hWnd, DlgProc, (LPARAM)trans);
...
INT_PTR CALLBACK rvGETransformer::DlgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        else if (HIWORD(wParam) == EN_KILLFOCUS && trans && trans->mWorkspace)
        {
            ...
            // The transformer coords are relative to the botto most selected window's
            // parent so adjust the rect accordingly
            if (trans->mRelative)
            {
