...
void rvGEWorkspace::Render(HDC hdc)
{
    ...
    qglColor3f(mApplication->GetOptions().GetWorkspaceColor()[0], mApplication->GetOptions().GetWorkspaceColor()[1],
...
void rvGEWorkspace::UpdateTitle(void)
{
    ...
    SetWindowText(mWnd, va("%s%s (%d%%)", idStr(mFilename).StripPath().c_str(), mModified ? "*" : "",
                           (int)(g_ZoomScales[mZoom] * 100)));
...
void rvGEWorkspace::HandleMessage(UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CLOSE: {
        ...
        if (IsModified())
        {
            if (IDYES == gApp.MessageBox(va("Save changes to the document \"%s\" before closing?", GetFilename()),
...
int rvGEWorkspace::HandleRButtonDown(WPARAM wParam, LPARAM lParam)
{
    ...
    for (i = 0; i < mSelectMenu.Num(); i++)
    {
        ...
        AppendMenu(popup, MF_STRING | MF_ENABLED | (wrapper->IsSelected() ? MF_CHECKED : 0), ID_GUIED_SELECT_FIRST + i,
...
int rvGEWorkspace::HandleMouseMove(WPARAM wParam, LPARAM lParam)
{
    ...
    {
        cursor.x = (float)(((int)cursor.x + gApp.GetOptions().GetGridWidth() / 2) / gApp.GetOptions().GetGridWidth() *
                           gApp.GetOptions().GetGridWidth());
        cursor.y = (float)(((int)cursor.y + gApp.GetOptions().GetGridWidth() / 2) / gApp.GetOptions().GetGridWidth() *
                           gApp.GetOptions().GetGridWidth());
    }
    ...
    case rvGESelectionMgr::HT_SIZE_TOPLEFT:
        AddModifierSize("Size", cursor.x - mDragPoint.x, cursor.y - mDragPoint.y, 0, 0,
                        mApplication->GetOptions().GetGridSnap());
        break;

    case rvGESelectionMgr::HT_SIZE_TOPRIGHT:
        AddModifierSize("Size", 0, cursor.y - mDragPoint.y, cursor.x - mDragPoint.x, 0,
                        mApplication->GetOptions().GetGridSnap());
        break;

    case rvGESelectionMgr::HT_SIZE_BOTTOMLEFT:
        AddModifierSize("Size", cursor.x - mDragPoint.x, 0, 0, cursor.y - mDragPoint.y,
                        mApplication->GetOptions().GetGridSnap());
        break;

    case rvGESelectionMgr::HT_SIZE_BOTTOMRIGHT:
        AddModifierSize("Size", 0, 0, cursor.x - mDragPoint.x, cursor.y - mDragPoint.y,
                        mApplication->GetOptions().GetGridSnap());
        break;
    }
...
idWindow *rvGEWorkspace::NewWindow(idDict *state, rvGEWindowWrapper::EWindowType type)
{
    ...
    idParser src(winTemplate, winTemplate.Length(), "",
                 LEXFL_ALLOWMULTICHARLITERALS | LEXFL_NOSTRINGCONCAT | LEXFL_ALLOWBACKSLASHSTRINGCONCAT);
    window->Parse(&src);
