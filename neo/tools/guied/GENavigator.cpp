...
bool rvGENavigator::Create(HWND parent, bool visible)
{
    ...
    mVisibleIcon = (HICON)LoadImage(win32.hInstance, MAKEINTRESOURCE(IDI_GUIED_NAV_VISIBLE), IMAGE_ICON, 16, 16,
                                    LR_DEFAULTCOLOR | LR_LOADMAP3DCOLORS);
    mScriptsIcon =
        (HICON)LoadImage(win32.hInstance, MAKEINTRESOURCE(IDI_GUIED_NAV_SCRIPTS), IMAGE_ICON, 16, 16, LR_DEFAULTCOLOR);
    mScriptsLightIcon = (HICON)LoadImage(win32.hInstance, MAKEINTRESOURCE(IDI_GUIED_NAV_SCRIPTSHI), IMAGE_ICON, 16, 16,
                                         LR_DEFAULTCOLOR);
    mVisibleIconDisabled = (HICON)LoadImage(win32.hInstance, MAKEINTRESOURCE(IDI_GUIED_NAV_VISIBLEDISABLED), IMAGE_ICON,
                                            16, 16, LR_DEFAULTCOLOR | LR_LOADMAP3DCOLORS);
    mExpandIcon = (HICON)LoadImage(win32.hInstance, MAKEINTRESOURCE(IDI_GUIED_NAV_EXPAND), IMAGE_ICON, 16, 16,
                                   LR_DEFAULTCOLOR | LR_LOADMAP3DCOLORS);
    mCollapseIcon = (HICON)LoadImage(win32.hInstance, MAKEINTRESOURCE(IDI_GUIED_NAV_COLLAPSE), IMAGE_ICON, 16, 16,
                                     LR_DEFAULTCOLOR | LR_LOADMAP3DCOLORS);

    mWnd = CreateWindowEx(WS_EX_TOOLWINDOW, "GUIEDITOR_NAVIGATOR_CLASS", "Navigator",
                          WS_SYSMENU | WS_THICKFRAME | WS_CAPTION | WS_POPUP | WS_OVERLAPPED | WS_BORDER |
    ...
    if (!gApp.GetOptions().GetWindowPlacement("navigator", mWnd))
    {
        ...
        SetWindowPos(mWnd, NULL, rParent.right - 10 - (rNav.right - rNav.left),
                     rParent.bottom - 10 - (rNav.bottom - rNav.top), 0, 0, SWP_NOZORDER | SWP_NOSIZE);
    }
...
LRESULT CALLBACK rvGENavigator::WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_DRAWITEM: {
        ...
        if (window)
        {
            ...
            if (!wrapper->IsHidden())
            {
                DrawIconEx(dis->hDC, rDraw.left, rDraw.top, disabled ? nav->mVisibleIconDisabled : nav->mVisibleIcon,
            ...
            if (wrapper->CanHaveChildren() && window->GetChildCount())
            {
                ...
                {
                    DrawIconEx(dis->hDC, rDraw.left + offset, rDraw.top + 3, nav->mCollapseIcon, 16, 16, 0, NULL,
                               DI_NORMAL);
                }
                else
                {
                    DrawIconEx(dis->hDC, rDraw.left + offset, rDraw.top + 3, nav->mExpandIcon, 16, 16, 0, NULL,
                               DI_NORMAL);
                }
            }
            ...
            {
                DrawIconEx(dis->hDC, dis->rcItem.right - 16, (dis->rcItem.bottom + dis->rcItem.top) / 2 - 6,
                           (dis->itemState & ODS_SELECTED) ? nav->mScriptsLightIcon : nav->mScriptsIcon, 13, 13, 0,
                           NULL, DI_NORMAL);
            }
        }
    ...
    case WM_CREATE: {
        ...
        nav->mTree = CreateWindowEx(0, "SysListView32", "",
    ...
    case WM_NOTIFY: {
        ...
        case NM_RCLICK: {
            ...
            if (index != -1)
            {
                ...
                TrackPopupMenu(menu, TPM_RIGHTBUTTON | TPM_LEFTALIGN, info.pt.x, info.pt.y, 0, gApp.GetMDIFrame(),
                               NULL);
                DestroyMenu(menu);
            }
