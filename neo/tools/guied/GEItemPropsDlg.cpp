...
void rvGEItemPropsImagePage::UpdateCheckedStates(void)
{
    ...
    state =
        IsDlgButtonChecked(mPage, IDC_GUIED_USEBACKCOLOR) && IsWindowEnabled(GetDlgItem(mPage, IDC_GUIED_USEBACKCOLOR));
...
bool rvGEItemPropsImagePage::SetActive(void)
{
    ...
    CheckDlgButton(mPage, IDC_GUIED_USEMATERIAL,
                   idStr(mDict->GetString("background", "")).StripQuotes().Length() ? BST_CHECKED : BST_UNCHECKED);
    CheckDlgButton(mPage, IDC_GUIED_USEBACKCOLOR,
                   idStr(mDict->GetString("backcolor", "")).StripQuotes().Length() ? BST_CHECKED : BST_UNCHECKED);

    CheckRadioButton(mPage, IDC_GUIED_USEBORDERCOLOR, IDC_GUIED_USEBORDERMATERIAL,
                     idStr(mDict->GetString("borderShader", "")).Length() ? IDC_GUIED_USEBORDERMATERIAL
...
bool rvGEItemPropsImagePage::KillActive(void)
{
    ...
    if (IsDlgButtonChecked(mPage, IDC_GUIED_USEBACKCOLOR))
    {
        if (IsWindowEnabled(GetDlgItem(mPage, IDC_GUIED_ITEMBACKCOLOR)))
        {
            ...
            mDict->Set("backcolor",
                       StringFromVec4(idVec4((float)GetRValue(color) / 255.0f, (float)GetGValue(color) / 255.0f,
                                             (float)GetBValue(color) / 255.0f, (float)GetRValue(alpha) / 255.0f)));
        }
    }
    else
    {
...
bool rvGEItemPropsTextPage::SetActive(void)
{
    ...
    CheckDlgButton(mPage, IDC_GUIED_USETEXT,
                   idStr(mDict->GetString("text", "")).Length() ? BST_CHECKED : BST_UNCHECKED);
    CheckDlgButton(mPage, IDC_GUIED_ITEMTEXTNOWRAP,
                   atoi(idStr(mDict->GetString("nowrap", "0")).StripQuotes()) ? BST_CHECKED : BST_UNCHECKED);
...
int rvGEItemPropsKeysPage::HandleMessage(UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        case IDC_GUIED_ADDKEY: {
            ...
            while (1)
            {
                if (DialogBoxParam(gApp.GetInstance(), MAKEINTRESOURCE(IDD_GUIED_ITEMKEY), mPage,
                                   ModifyItemKeyDlg_WndProc, (LPARAM)key))
                {
                    ...
                    if (!mWrapper->VerfiyStateKey(key->GetKey(), finalValue))
                    {
                        ...
                        if (!mWrapper->VerfiyStateKey(key->GetKey(), finalValue))
                        {
                            gApp.MessageBox(va("Invalid property value '%s' for property '%s'", key->GetValue().c_str(),
                                               key->GetKey().c_str()),
                                            MB_OK | MB_ICONERROR);
                            continue;
                        }
                ...
                while (1)
                {
                                       ...
                                       ModifyItemKeyDlg_WndProc, (LPARAM)key))
                    {
                        ...
                        if (!mWrapper->VerfiyStateKey(key->GetKey(), finalValue))
                        {
                            ...
                            if (!mWrapper->VerfiyStateKey(key->GetKey(), finalValue))
                            {
                                gApp.MessageBox(va("Invalid property value '%s' for property '%s'",
...
bool rvGEItemPropsGeneralPage::SetActive(void)
{
    ...
    CheckDlgButton(mPage, IDC_GUIED_ITEMVISIBLE,
                   atol(idStr(mDict->GetString("visible", "1")).StripQuotes()) ? BST_CHECKED : BST_UNCHECKED);
    ...
    CheckDlgButton(mPage, IDC_GUIED_ITEMNOTIME,
                   atol(idStr(mDict->GetString("notime", "0")).StripQuotes()) ? BST_CHECKED : BST_UNCHECKED);
    ...
    CheckDlgButton(mPage, IDC_GUIED_ITEMNOEVENTS,
                   atol(idStr(mDict->GetString("noevents", "0")).StripQuotes()) ? BST_CHECKED : BST_UNCHECKED);
    ...
    CheckDlgButton(mPage, IDC_GUIED_ITEMNOCLIP,
                   atol(idStr(mDict->GetString("noclip", "0")).StripQuotes()) ? BST_CHECKED : BST_UNCHECKED);
    ...
    CheckDlgButton(mPage, IDC_GUIED_ITEMNOCURSOR,
                   atol(idStr(mDict->GetString("nocursor", "0")).StripQuotes()) ? BST_CHECKED : BST_UNCHECKED);
