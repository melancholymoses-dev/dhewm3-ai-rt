...
bool idExportModel::WriteMesh(const char *filename, idExportOptions &options)
{
    ...
    for (i = 0; i < jointList.Num(); i++)
    {
        ...
        WriteFloatString(file, "\t\"%s\"\t%d ( %f %f %f ) ( %f %f %f )\t\t// %s\n", joint->name.c_str(), parentNum,
                         joint->bindpos.x, joint->bindpos.y, joint->bindpos.z, bindQuat[0], bindQuat[1], bindQuat[2],
                         parentName);
    }
    ...
    for (i = 0; i < meshes.Num(); i++)
    {
        ...
        {
            WriteFloatString(file, "\tvert %d ( %f %f ) %d %d\n", j, mesh->verts[j].texCoords[0],
                             mesh->verts[j].texCoords[1], mesh->verts[j].startweight, mesh->verts[j].numWeights);
        }

        WriteFloatString(file, "\n\tnumtris %d\n", mesh->tris.Num());
        for (j = 0; j < mesh->tris.Num(); j++)
        {
            WriteFloatString(file, "\ttri %d %d %d %d\n", j, mesh->tris[j].indexes[2], mesh->tris[j].indexes[1],
                             mesh->tris[j].indexes[0]);
        }
        ...
        for (j = 0; j < mesh->weights.Num(); j++)
        {
            ...
            WriteFloatString(file, "\tweight %d %d %f ( %f %f %f )\n", j, weight->joint->exportNum, weight->jointWeight,
...
bool idExportModel::WriteAnim(const char *filename, idExportOptions &options)
{
    ...
    for (i = 0; i < jointList.Num(); i++)
    {
        ...
        if (parent)
        {
            WriteFloatString(file, "\t\"%s\"\t%d %d %d\t// %s", joint->name.c_str(), parent->exportNum, joint->animBits,
                             joint->firstComponent, parent->name.c_str());
        }
        else
        {
            WriteFloatString(file, "\t\"%s\"\t-1 %d %d\t//", joint->name.c_str(), joint->animBits,
                             joint->firstComponent);
        }
    ...
    {
        WriteFloatString(file, "\t( %f %f %f ) ( %f %f %f )\n", bounds[i][0].x, bounds[i][0].y, bounds[i][0].z,
                         bounds[i][1].x, bounds[i][1].y, bounds[i][1].z);
    }
    WriteFloatString(file, "}\n");
    ...
    for (i = 0; i < jointList.Num(); i++)
    {
        ...
        WriteFloatString(file, "\t( %f %f %f ) ( %f %f %f )\n", joint->baseFrame.t[0], joint->baseFrame.t[1],
                         joint->baseFrame.t[2], joint->baseFrame.q[0], joint->baseFrame.q[1], joint->baseFrame.q[2]);
    }
    WriteFloatString(file, "}\n");
...
bool idExportModel::WriteCamera(const char *filename, idExportOptions &options)
{
    ...
    {
        WriteFloatString(file, "\t( %f %f %f ) ( %f %f %f ) %f\n", frame->t.x, frame->t.y, frame->t.z, frame->q[0],
                         frame->q[1], frame->q[2], frame->fov);
    }
    WriteFloatString(file, "}\n");
...
bool idMayaExport::RemapParents(idList<idNamePair> &remapjoints)
{
    ...
    for (i = 0; i < remapjoints.Num(); i++)
    {
        ...
        if (!parent)
        {
            ...
            MayaError("Couldn't find joint '%s' to be new parent for '%s'\n", remapjoints[i].to.c_str(),
                      remapjoints[i].from.c_str());
        }

        if (parent->exportNode.ParentedBy(joint->exportNode))
        {
            MayaError("Joint '%s' is a child of joint '%s' and can't become the parent.", joint->name.c_str(),
                      parent->name.c_str());
        }
        joint->exportNode.ParentTo(parent->exportNode);
    }
...
void idMayaExport::CreateMesh(float scale)
{
    ...
    for (; !iter.isDone(); iter.next())
    {
        ...
        if (!status)
        {
            MayaError("Mesh '%s': Error getting influence objects (%s)", mesh->name.c_str(),
        ...
        for (size_t ii = 0; ii < nGeoms; ++ii)
        {
            ...
            if (!status)
            {
                MayaError("Mesh '%s': Error getting geometry index (%s)", mesh->name.c_str(),
                          status.errorString().asChar());
            }
            ...
            if (!status)
            {
                MayaError("Mesh '%s': Error getting geometry path (%s)", mesh->name.c_str(),
                          status.errorString().asChar());
            }
            ...
            for (/* nothing */; !gIter.isDone(); gIter.next())
            {
                ...
                if (!status)
                {
                    MayaError("Mesh '%s': Error getting component (%s)", mesh->name.c_str(),
                              status.errorString().asChar());
                }
                    ...
                    if (w > options.jointThreshold)
                    {
                        ...
                        if (!options.ignoreScale)
                        {
                            weight.joint->bindmat.ProjectVector(
                                vert->pos - (weight.joint->bindpos * weight.joint->invscale), weight.offset);
                ...
                if (!vert->numWeights)
                {
                    ...
                    {
                        MayaError("Error on mesh '%s': Vertex %d doesn't have any joint weights.", mesh->name.c_str(),
                                  num);
                    }
                }
                else if (!totalweight)
...
void idMayaExport::CreateAnimation(idMat3 &align)
{
    ...
    if (options.cycleStart > options.startframe)
    {
        ...
        for (i = 0; i < model.numFrames; i++)
        {
            ...
            memcpy(&model.jointFrames[model.joints.Num() * i], &copyFrames[model.joints.Num() * frameNum],
