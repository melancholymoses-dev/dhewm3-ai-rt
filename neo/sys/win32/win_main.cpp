...
typedef BOOL(WINAPI *PWGLUSEFONTOUTLINES)(HDC, DWORD, DWORD, DWORD, FLOAT, FLOAT, int, LPGLYPHMETRICSFLOAT);
typedef BOOL(WINAPI *PWGLDESCRIBELAYERPLANE)(HDC, int, int, UINT, LPLAYERPLANEDESCRIPTOR);
typedef int(WINAPI *PWGLSETLAYERPALETTEENTRIES)(HDC, int, int, int, CONST COLORREF *);
typedef int(WINAPI *PWGLGETLAYERPALETTEENTRIES)(HDC, int, int, int, COLORREF *);
 ...
 */
extern "C"
{ // DG: I need this in SDL_win32_main.c
...
uintptr_t Sys_DLL_Load(const char *dllName)
{
    ...
    else
    {
        ...
        FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL,
                      e, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPTSTR)&msgBuf, 0, NULL);
...
void Sys_DLL_Unload(uintptr_t dllHandle)
{
    ...
    if (FreeLibrary((HINSTANCE)dllHandle) == 0)
    {
        ...
        LPVOID lpMsgBuf;
        FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER, NULL, lastError,
...
static HRESULT(STDAPICALLTYPE *D3_GetDpiForMonitor)(HMONITOR hmonitor, D3_MONITOR_DPI_TYPE dpiType, UINT *dpiX,
                                                    UINT *dpiY) = NULL;
...
static void setHighDPIMode(void)
{
    ...
    if (shcoreDLL)
    {
        SetProcessDpiAwareness =
            (HRESULT(WINAPI *)(D3_PROCESS_DPI_AWARENESS))GetProcAddress(shcoreDLL, "SetProcessDpiAwareness");
    }
    ...
    if (shcoreDLL)
    {
        D3_GetDpiForMonitor = (HRESULT(STDAPICALLTYPE *)(HMONITOR, D3_MONITOR_DPI_TYPE, UINT *, UINT *))GetProcAddress(
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrev, LPSTR szCmdLine, int sw)
{
    ...
    for (i = 0; i < argc; ++i)
    {
        ...
        if (!ansiSize)
        { // uhoh?
            SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Fatal Error", "Error processing command line arguments",
                                     NULL);
            return -1;
        }

        argv[i] = (char *)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY,
                                    ansiSize); // this size includes the null-terminator character.
        if (!argv[i])
        {
            return OutOfMemory();
        }

        if (WideCharToMultiByte(CP_ACP, 0, argvw[i], -1, argv[i], ansiSize, NULL, NULL) == 0)
        { // failed? uhoh!
            SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Fatal Error", "Error processing command line arguments",
                                     NULL);
            return -1;
