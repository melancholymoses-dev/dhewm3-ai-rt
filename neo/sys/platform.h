...
// dhewm3 now uses a 8MB stack, set with a linker flag in CMakeLists.txt (/STACK:8388608 for MSVC, -Wl,--stack,8388608
// for mingw) Linux has a 8MB stack by default, and so does macOS, at least for the main thread. Anyway, a 1MB limit for
// _alloca() should be safe even when using it multiple times in the same function or callstack. If there's a risk of
// bigger stack allocations, Mem_MallocA() should be used instead.
...
// NOTE: Do *not* use __builtin_alloca_with_align(), unlike regular alloca it frees at end of block instead of end of
// function !
#define _alloca16(x)                                                                                                   \
    ((void *)((assert((x) < ID_MAX_ALLOCA_SIZE)), ((((uintptr_t)__builtin_alloca((x) + 15)) + 15) & ~15)))
...
#ifdef _M_X64
// this matches AMD64 and ARM64EC (but not regular ARM64), but they're supposed to be binary-compatible somehow, so
// whatever
...
// function !
#define _alloca16(x)                                                                                                   \
    (({ assert((x) < ID_MAX_ALLOCA_SIZE); }), ((void *)((((uintptr_t)__builtin_alloca((x) + 15)) + 15) & ~15)))
#define _alloca(x) (({ assert((x) < ID_MAX_ALLOCA_SIZE); }), __builtin_alloca((x)))
#else
