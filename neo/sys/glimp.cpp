...
#include "doom_icon.h" // contains the struct d3_icon
    ...
    SDL_Surface *icon = SDL_CreateSurfaceFrom(d3_icon.width, d3_icon.height, SDL_PIXELFORMAT_RGBA32,
                                              (void *)d3_icon.pixel_data, d3_icon.bytes_per_pixel * d3_icon.width);
...
#include "doom_icon.h" // contains the struct d3_icon
    ...
    SDL_Surface *icon =
        SDL_CreateRGBSurfaceFrom((void *)d3_icon.pixel_data, d3_icon.width, d3_icon.height, d3_icon.bytes_per_pixel * 8,
                                 d3_icon.bytes_per_pixel * d3_icon.width, rmask, gmask, bmask, amask);
    ...
    try_again:
        ...
        // try to put the window on the display the mousecursor currently is on
        {
            ...
            for (int j = 0; j < numDisplays; ++j)
            {
                ...
                {
                    common->Printf(" %d: %dx%d at (%d, %d) to (%d, %d)\n", j, rect.w, rect.h, rect.x, rect.y,
                                   rect.x + rect.w, rect.y + rect.h);
#endif
                    if (!found && x >= rect.x && x < rect.x + rect.w && y >= rect.y && y < rect.y + rect.h)
                    {
                        selectedDisplay = j;
                        found = true;
                    }
...
#else  // SDL2
        window = SDL_CreateWindow(ENGINE_VERSION, SDL_WINDOWPOS_UNDEFINED_DISPLAY(selectedDisplay),
                                  SDL_WINDOWPOS_UNDEFINED_DISPLAY(selectedDisplay), parms.width, parms.height, flags);
...
#error "dhewm3 only supports the tools with SDL2, not SDL1!"
      ...
      // Get the HWND for later use.
        ...
        if (SDL_GetWindowWMInfo(window, &sdlinfo) && sdlinfo.subsystem == SDL_SYSWM_WINDOWS)
        {
            ...
            win32.wglGetPixelFormatAttribivARB = (BOOL(WINAPI *)(
                HDC, int, int, UINT, const int *, int *))SDL_GL_GetProcAddress("wglGetPixelFormatAttribivARB");
            win32.wglChoosePixelFormatARB = (BOOL(WINAPI *)(HDC, const int *, const FLOAT *, UINT, int *piFormats,
            ...
            if (win32.wglGetPixelFormatAttribivARB != NULL && win32.wglChoosePixelFormatARB != NULL)
            {
                ...
                enum
                {
                    NUM_ATTRIBUTES = sizeof(queryAttributes) / sizeof(queryAttributes[0])
                };
                int queryResults[NUM_ATTRIBUTES] = {};

                win32.wglGetPixelFormatAttribivARB(win32.hDC, pfIdx, PFD_MAIN_PLANE, NUM_ATTRIBUTES, queryAttributes,
                                                   queryResults);
        ...
        common->Printf("Requested %d color bits per chan, %d alpha %d depth, %d stencil\n", channelcolorbits,
                       talphabits, tdepthbits, tstencilbits);
...
bool GLimp_SetScreenParms(glimpParms_t parms)
{
    ...
    { // we want some kind of fullscreen mode
