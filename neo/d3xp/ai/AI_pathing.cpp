...
============
*/
void GetPointOutsideObstacles(const obstacle_t *obstacles, const int numObstacles, idVec2 &point, int *obstacle,
                              int *edgeNum)
{
...
============
*/
bool GetFirstBlockingObstacle(const obstacle_t *obstacles, int numObstacles, int skipObstacle, const idVec2 &startPos,
                              const idVec2 &delta, float &blockingScale, int &blockingObstacle, int &blockingEdgeNum)
{
    int i, edgeNums[2];
                 ...
                 idBounds &clipBounds)
{
    ...
    if (ai_showObstacleAvoidance.GetBool())
    {
        for (i = 0; i < numObstacles; i++)
        {
            ...
            for (j = 0; j < obstacle.winding.GetNumPoints(); j++)
            {
                gameRenderWorld->DebugArrow(colorGreen, silVerts[j],
...
============
*/
pathNode_t *BuildPathTree(const obstacle_t *obstacles, int numObstacles, const idBounds &clipBounds,
                          const idVec2 &startPos, const idVec2 &seekPos, obstaclePath_t &path)
{
    ...
    for (node = pathNodeQueue.Get(); node && pathNodeAllocator.GetAllocCount() < MAX_PATH_NODES;
         node = pathNodeQueue.Get())
    {
        ...
        if (GetFirstBlockingObstacle(obstacles, numObstacles, node->obstacle, node->pos, node->delta, blockingScale,
                                     blockingObstacle, blockingEdgeNum))
        {
...
============
*/
int OptimizePath(const pathNode_t *root, const pathNode_t *leafNode, const obstacle_t *obstacles, int numObstacles,
            ...
            for (i = 0; i < numObstacles; i++)
            {
                ...
                if (obstacles[i].winding.RayIntersection(curPos, curDelta, scale1, scale2, edgeNums))
                {
                    if (scale1 >= 0.0f && scale1 <= 1.0f &&
                        (i != nextNode->obstacle || scale1 * curLength < curLength - 0.5f))
                    {
                        break;
                    }
                    if (scale2 >= 0.0f && scale2 <= 1.0f &&
                        (i != nextNode->obstacle || scale2 * curLength < curLength - 0.5f))
                    {
                        break;
                    }
                }
...
============
*/
bool FindOptimalPath(const pathNode_t *root, const obstacle_t *obstacles, int numObstacles, const float height,
                     const idVec3 &curDir, idVec3 &seekPos)
{
...
bool idAI::FindPathAroundObstacles(const idPhysics *physics, const idAAS *aas, const idEntity *ignore,
                                   const idVec3 &startPos, const idVec3 &seekPos, obstaclePath_t &path)
{
    ...
    areaNum =
        aas->PointReachableAreaNum(path.startPosOutsideObstacles, bounds, (AREA_REACHABLE_WALK | AREA_REACHABLE_FLY));
    aas->PushPointIntoAreaNum(areaNum, path.startPosOutsideObstacles);

    // get all the nearby obstacles
    numObstacles = GetObstacles(physics, aas, ignore, areaNum, path.startPosOutsideObstacles,
    ...
    // build a path tree
    root = BuildPathTree(obstacles, numObstacles, clipBounds, path.startPosOutsideObstacles.ToVec2(),
...
============
*/
bool PathTrace(const idEntity *ent, const idAAS *aas, const idVec3 &start, const idVec3 &end, int stopEvent,
               struct pathTrace_s &trace, predictedPath_t &path)
{
    trace_t clipTrace;
    aasTrace_t aasTrace;
...
bool idAI::PredictPath(const idEntity *ent, const idAAS *aas, const idVec3 &start, const idVec3 &velocity,
                       int totalTime, int frameTime, int stopEvent, predictedPath_t &path)
{
...
=====================
*/
bool idAI::TestTrajectory(const idVec3 &start, const idVec3 &end, float zVel, float gravity, float time,
                          float max_height, const idClipModel *clip, int clipmask, const idEntity *ignore,
                          const idEntity *targetEntity, int drawtime)
{
    ...
    if (drawtime)
    {
        if (clip)
        {
            gameRenderWorld->DebugBounds(result ? colorGreen : colorYellow, clip->GetBounds().Expand(1.0f),
                                         trace.endpos, drawtime);
        }
        else
        {
...
=====================
*/
bool idAI::PredictTrajectory(const idVec3 &firePos, const idVec3 &target, float projectileSpeed,
                             const idVec3 &projGravity, const idClipModel *clip, int clipmask, float max_height,
                             const idEntity *ignore, const idEntity *targetEntity, int drawtime, idVec3 &aimDir)
{
    int n, i, j;
    ...
    if (projectileSpeed <= 0.0f || projGravity == vec3_origin)
    {
        ...
        if (drawtime)
        {
            ...
            idBounds bnds(trace.endpos);
            bnds.ExpandSelf(1.0f);
            gameRenderWorld->DebugBounds((trace.fraction >= 1.0f || (gameLocal.GetTraceEntity(trace) == targetEntity))
                                             ? colorGreen
                                             : colorYellow,
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (TestTrajectory(firePos, target, zVel, projGravity[2], ballistics[i].time, firePos.z + max_height, clip,
