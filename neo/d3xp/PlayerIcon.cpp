...
#ifdef CTF
                                          ,
                                          "mtr_icon_redteam", "mtr_icon_blueteam"
#endif
};
...
void idPlayerIcon::Draw(idPlayer *player, const idVec3 &origin)
{
    ...
    else if (g_CTFArrows.GetBool() && gameLocal.mpGame.IsGametypeFlagBased() && gameLocal.GetLocalPlayer() &&
             player->team == gameLocal.GetLocalPlayer()->team && !player->IsHidden() && !player->AI_DEAD)
    {
...
===============
*/
bool idPlayerIcon::CreateIcon(idPlayer *player, playerIconType_t type, const char *mtr, const idVec3 &origin,
                              const idMat3 &axis)
{
    assert(type != ICON_NONE);
