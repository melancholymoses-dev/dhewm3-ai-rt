...
void idProjectile::Restore(idRestoreGame *savefile)
{
    ...
    if (smokeFly != NULL)
    {
        ...
        gameLocal.smokeParticles->EmitSmoke(smokeFly, gameLocal.time, gameLocal.random.RandomFloat(),
                                            GetPhysics()->GetOrigin(), GetPhysics()->GetAxis(), timeGroup /*_D3XP*/);
    }
...
void idProjectile::Create(idEntity *owner, const idVec3 &start, const idVec3 &dir)
{
    ...
    if (*(const char *)shaderName)
    {
        ...
        renderLight.lightRadius[0] = renderLight.lightRadius[1] = renderLight.lightRadius[2] =
            spawnArgs.GetFloat("light_radius");
...
=================
*/
void idProjectile::Launch(const idVec3 &start, const idVec3 &dir, const idVec3 &pushVelocity, const float timeSinceFire,
...
#endif
        if (!gameLocal.smokeParticles->EmitSmoke(smokeFly, smokeFlyTime, gameLocal.random.RandomFloat(),
...
bool idProjectile::Collide(const trace_t &collision, const idVec3 &velocity)
{
    ...
    if (gameLocal.isMultiplayer && ent->IsType(idDoor::Type) && !static_cast<idDoor *>(ent)->IsOpen() &&
        !ent->spawnArgs.GetBool("no_touch"))
    {
        ent->ProcessEvent(&EV_Activate, this);
    }

    if (ent->IsType(idActor::Type) || (ent->IsType(idAFAttachment::Type) &&
                                       static_cast<const idAFAttachment *>(ent)->GetBody()->IsType(idActor::Type)))
    {
...
=================
*/
void idProjectile::DefaultDamageEffect(idEntity *soundEnt, const idDict &projectileDef, const trace_t &collision,
...
void idProjectile::AddDefaultDamageEffect(const trace_t &collision, const idVec3 &velocity)
{
    ...
    if (gameLocal.isServer && fl.networkSync)
    {
        ...
        msg.WriteInt((collision.c.material != NULL)
                         ? gameLocal.ServerRemapDecl(-1, DECL_MATERIAL, collision.c.material->Index())
                         : -1);
...
void idProjectile::Event_RadiusDamage(idEntity *ignore)
{
    ...
    {
        gameLocal.RadiusDamage(physicsObj.GetOrigin(), this, owner.GetEntity(), ignore, this, splash_damage,
                               damagePower);
    }
}
...
=================
*/
bool idProjectile::ClientPredictionCollide(idEntity *soundEnt, const idDict &projectileDef, const trace_t &collision,
                                           const idVec3 &velocity, bool addDamageEffect)
{
    idEntity *ent;
...
bool idProjectile::ClientReceiveEvent(int event, int time, const idBitMsg &msg)
{
    ...
    case EVENT_DAMAGE_EFFECT: {
        ...
        collision.c.material =
            (index != -1) ? static_cast<const idMaterial *>(declManager->DeclByIndex(DECL_MATERIAL, index)) : NULL;
...
=================
*/
void idGuidedProjectile::Launch(const idVec3 &start, const idVec3 &dir, const idVec3 &pushVelocity,
                                const float timeSinceFire, const float launchPower, float dmgPower)
{
...
void idSoulCubeMissile::KillTarget(const idVec3 &dir)
{
    ...
    if (enemy.GetEntity() && enemy.GetEntity()->IsType(idActor::Type))
    {
        ...
        if ((act->health > 0) && ownerEnt && ownerEnt->IsType(idPlayer::Type) && (ownerEnt->health > 0) &&
...
void idSoulCubeMissile::Think(void)
{
    ...
    if (state == LAUNCHED)
    {
        if (killPhase)
        {
            ...
            {
                if (!gameLocal.smokeParticles->EmitSmoke(smokeKill, smokeKillTime, gameLocal.random.CRandomFloat(),
...
=================
*/
void idSoulCubeMissile::Launch(const idVec3 &start, const idVec3 &dir, const idVec3 &pushVelocity,
                               const float timeSinceFire, const float launchPower, float dmgPower)
{
    idVec3 newStart;
    idVec3 offs;
...
void idBFGProjectile::Spawn(void)
{
    ...
    if (temp && *temp)
    {
        ...
        secondModel.shaderParms[SHADERPARM_RED] = secondModel.shaderParms[SHADERPARM_GREEN] =
            secondModel.shaderParms[SHADERPARM_BLUE] = secondModel.shaderParms[SHADERPARM_ALPHA] = 1.0f;
...
void idBFGProjectile::Think(void)
{
    if (state == LAUNCHED)
    {
        ...
        for (int i = 0; i < beamTargets.Num(); i++)
        {
            ...
            idPlayer *player = (beamTargets[i].target.GetEntity()->IsType(idPlayer::Type))
...
#else
                if (damageFreq && *(const char *)damageFreq && beamTargets[i].target.GetEntity() &&
                    beamTargets[i].target.GetEntity()->CanDamage(GetPhysics()->GetOrigin(), org))
                {
#endif
                    org = beamTargets[i].target.GetEntity()->GetPhysics()->GetOrigin() - GetPhysics()->GetOrigin();
                    org.Normalize();
                    beamTargets[i].target.GetEntity()->Damage(this, owner.GetEntity(), org, damageFreq,
...
=================
*/
void idBFGProjectile::Launch(const idVec3 &start, const idVec3 &dir, const idVec3 &pushVelocity,
                             const float timeSinceFire, const float power, const float dmgPower)
{
    ...
    for (int e = 0; e < numListedEntities; e++)
    {
        ...
        if (ent == this || ent == owner.GetEntity() || ent->IsHidden() || !ent->IsActive() || !ent->fl.takedamage ||
            ent->health <= 0 || !ent->IsType(idActor::Type))
        {
            continue;
        }
...
void idDebris::Launch(void)
{
    ...
    if (*smokeName != '\0')
    {
        smokeFly = static_cast<const idDeclParticle *>(declManager->FindType(DECL_PARTICLE, smokeName));
        smokeFlyTime = gameLocal.time;
        gameLocal.smokeParticles->EmitSmoke(smokeFly, smokeFlyTime, gameLocal.random.CRandomFloat(),
                                            GetPhysics()->GetOrigin(), GetPhysics()->GetAxis(), timeGroup /*_D3XP*/);
    }
...
void idDebris::Think(void)
{
    ...
    if (smokeFly && smokeFlyTime)
    {
        if (!gameLocal.smokeParticles->EmitSmoke(smokeFly, smokeFlyTime, gameLocal.random.CRandomFloat(),
