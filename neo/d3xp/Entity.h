...
ID_INLINE void SetTimeState::PushState(int timeGroup)
{
    ...
    if (!gameLocal.isMultiplayer)
    {
        ...
        if (fast)
        {
            gameLocal.fast.Get(gameLocal.time, gameLocal.previousTime, gameLocal.msec, gameLocal.framenum,
                               gameLocal.realClientTime);
        }
        else
        {
            gameLocal.slow.Get(gameLocal.time, gameLocal.previousTime, gameLocal.msec, gameLocal.framenum,
                               gameLocal.realClientTime);
        }
    }
}
