...
typedef void (*WriteVariableType_t)(const char *varName, const char *varType, const char *scope, const char *prefix,
                                    const char *postfix, const char *value, const void *varPtr, int varSize);
...
class idTypeInfoTools
{
  ...
  private:
    ...
    static void PrintVariable(const char *varName, const char *varType, const char *scope, const char *prefix,
                              const char *postfix, const char *value, const void *varPtr, int varSize);
    static void WriteVariable(const char *varName, const char *varType, const char *scope, const char *prefix,
                              const char *postfix, const char *value, const void *varPtr, int varSize);
    static void WriteGameStateVariable(const char *varName, const char *varType, const char *scope, const char *prefix,
                                       const char *postfix, const char *value, const void *varPtr, int varSize);
    static void InitVariable(const char *varName, const char *varType, const char *scope, const char *prefix,
                             const char *postfix, const char *value, const void *varPtr, int varSize);
    static void VerifyVariable(const char *varName, const char *varType, const char *scope, const char *prefix,
                               const char *postfix, const char *value, const void *varPtr, int varSize);
    static int WriteVariable_r(const void *varPtr, const char *varName, const char *varType, const char *scope,
                               const char *prefix, const int pointerDepth);
    static void WriteClass_r(const void *classPtr, const char *className, const char *classType, const char *scope,
                             const char *prefix, const int pointerDepth);
};
...
================
*/
bool IsAllowedToChangedFromSaveGames(const char *varName, const char *varType, const char *scope, const char *prefix,
                                     const char *postfix, const char *value)
{
...
================
*/
bool IsRenderHandleVariable(const char *varName, const char *varType, const char *scope, const char *prefix,
                            const char *postfix, const char *value)
{
...
================
*/
void idTypeInfoTools::PrintVariable(const char *varName, const char *varType, const char *scope, const char *prefix,
                                    const char *postfix, const char *value, const void *varPtr, int varSize)
{
...
================
*/
void idTypeInfoTools::WriteVariable(const char *varName, const char *varType, const char *scope, const char *prefix,
                                    const char *postfix, const char *value, const void *varPtr, int varSize)
{

    for (int i = idStr::FindChar(value, '#', 0); i >= 0; i = idStr::FindChar(value, '#', i + 1))
    {
        if (idStr::Icmpn(value + i + 1, "INF", 3) == 0 || idStr::Icmpn(value + i + 1, "IND", 3) == 0 ||
            idStr::Icmpn(value + i + 1, "NAN", 3) == 0 || idStr::Icmpn(value + i + 1, "QNAN", 4) == 0 ||
...
================
*/
void idTypeInfoTools::WriteGameStateVariable(const char *varName, const char *varType, const char *scope,
                                             const char *prefix, const char *postfix, const char *value,
                                             const void *varPtr, int varSize)
{
...
================
*/
void idTypeInfoTools::InitVariable(const char *varName, const char *varType, const char *scope, const char *prefix,
                                   const char *postfix, const char *value, const void *varPtr, int varSize)
{
...
================
*/
void idTypeInfoTools::VerifyVariable(const char *varName, const char *varType, const char *scope, const char *prefix,
                                     const char *postfix, const char *value, const void *varPtr, int varSize)
{
    idToken token;
...
================
*/
int idTypeInfoTools::WriteVariable_r(const void *varPtr, const char *varName, const char *varType, const char *scope,
                                     const char *prefix, const int pointerDepth)
{
...
#error "Unexpected pointer size"
    ...
    else if (token == "idVecX")
    {
        ...
        {
            Write(varName, varType, scope, prefix, "", vec->ToString(8), vec->ToFloatPtr(),
                  vec->GetSize() * sizeof(float));
        }
        else
        {
            Write(varName, varType, scope, prefix, "", "<NULL>", varPtr, 0);
    ...
    else if (token == "idMatX")
    {
        ...
        {
            Write(varName, varType, scope, prefix, "", mat->ToString(8), mat->ToFloatPtr(),
    ...
    else if (token == "idBounds")
    {
        ...
        else
        {
            Write(varName, varType, scope, prefix, "",
                  va("(%s)-(%s)", (*bounds)[0].ToString(8), (*bounds)[1].ToString(8)), varPtr, typeSize);
        }
    }
    ...
    else if (token == "idStrList")
    {
        ...
        if (list->Num())
        {
            for (i = 0; i < list->Num(); i++)
            {
                Write(varName, varType, scope, prefix, va("[%d]", i), OutputString((*list)[i].c_str()),
                      (*list)[i].c_str(), (*list)[i].Length());
            }
        }
        else
        {
        ...
        if (dict->GetNumKeyVals())
        {
            for (i = 0; i < dict->GetNumKeyVals(); i++)
            {
                ...
                Write(varName, varType, scope, prefix, va("[%d]", i),
                      va("\'%s\'  \'%s\'", OutputString(kv->GetKey().c_str()), OutputString(kv->GetValue().c_str())),
                      NULL, 0);
            }
        }
        else
    ...
    else if (token == "idExtrapolate")
    {
        ...
        Write(varName, varType, scope, prefix, ".extrapolationType", idStr(interpolate->GetExtrapolationType()).c_str(),
              &interpolate->extrapolationType, sizeof(interpolate->extrapolationType));
        Write(varName, varType, scope, prefix, ".startTime", idStr(interpolate->GetStartTime()).c_str(),
              &interpolate->startTime, sizeof(interpolate->startTime));
        Write(varName, varType, scope, prefix, ".duration", idStr(interpolate->GetDuration()).c_str(),
              &interpolate->duration, sizeof(interpolate->duration));
        ...
        if (ParseTemplateArguments(typeSrc, templateArgs))
        {
            if (templateArgs == "int")
            {
                ...
                Write(varName, varType, scope, prefix, ".startValue", idStr(interpolate->GetStartValue()).c_str(),
                      &interpolate->startValue, sizeof(interpolate->startValue));
                Write(varName, varType, scope, prefix, ".baseSpeed", idStr(interpolate->GetBaseSpeed()).c_str(),
                      &interpolate->baseSpeed, sizeof(interpolate->baseSpeed));
                Write(varName, varType, scope, prefix, ".speed", idStr(interpolate->GetSpeed()).c_str(),
                      &interpolate->speed, sizeof(interpolate->speed));
            ...
            else if (templateArgs == "idVec3")
            {
                ...
                Write(varName, varType, scope, prefix, ".startValue", interpolate->GetStartValue().ToString(8),
                      &interpolate->startValue, sizeof(interpolate->startValue));
                Write(varName, varType, scope, prefix, ".baseSpeed", interpolate->GetBaseSpeed().ToString(8),
                      &interpolate->baseSpeed, sizeof(interpolate->baseSpeed));
                Write(varName, varType, scope, prefix, ".speed", interpolate->GetSpeed().ToString(8),
                      &interpolate->speed, sizeof(interpolate->speed));
            ...
            {
                Write(varName, varType, scope, prefix, "",
                      va("<unknown template argument type '%s' for idExtrapolate>", templateArgs.c_str()), NULL, 0);
            }
        }
    }
    else if (token == "idInterpolate")
    {
        ...
        if (ParseTemplateArguments(typeSrc, templateArgs))
        {
            if (templateArgs == "int")
            {
                ...
                Write(varName, varType, scope, prefix, ".startValue", idStr(interpolate->GetStartValue()).c_str(),
                      &interpolate->startValue, sizeof(interpolate->startValue));
                Write(varName, varType, scope, prefix, ".endValue", idStr(interpolate->GetEndValue()).c_str(),
                      &interpolate->endValue, sizeof(interpolate->endValue));
            ...
            {
                Write(varName, varType, scope, prefix, "",
                      va("<unknown template argument type '%s' for idInterpolate>", templateArgs.c_str()), NULL, 0);
            }
        }
    }
    else if (token == "idInterpolateAccelDecelLinear")
    {
        ...
        Write(varName, varType, scope, prefix, ".startTime", idStr(interpolate->GetStartTime()).c_str(),
              &interpolate->startTime, sizeof(interpolate->startTime));
        Write(varName, varType, scope, prefix, ".accelTime", idStr(interpolate->GetAcceleration()).c_str(),
              &interpolate->accelTime, sizeof(interpolate->accelTime));
        Write(varName, varType, scope, prefix, ".linearTime", idStr(interpolate->linearTime).c_str(),
              &interpolate->linearTime, sizeof(interpolate->linearTime));
        Write(varName, varType, scope, prefix, ".decelTime", idStr(interpolate->GetDeceleration()).c_str(),
    ...
    else if (token == "idEntityPtr")
    {
        ...
        if (entPtr->GetEntity())
        {
            ...
            Write(varName, varType, scope, prefix, ".",
                  va("entity %d: \'%s\'", entity->entityNumber, entity->name.c_str()), varPtr, typeSize);
        }
        else
        {
    ...
    else if (token == "idEntity::entityFlags_s")
    {
        ...
        Write(varName, varType, scope, prefix, ".forcePhysicsUpdate", flags->forcePhysicsUpdate ? "true" : "false",
              NULL, 0);
    ...
    else
    {
        ...
        if (classTypeInfo != NULL)
        {
            ...
            else if (IsSubclassOf(token, "idEntity"))
            {
                ...
                Write(varName, varType, scope, prefix, "",
                      va("entity %d: \'%s\'", entity->entityNumber, entity->name.c_str()), NULL, 0);
            }
            ...
            else if (pointerDepth == 0 && (token == "idAFBody" || token == "idAFTree" || token == "idClipModel" ||
...
================
*/
void idTypeInfoTools::WriteClass_r(const void *classPtr, const char *className, const char *classType,
...
void ListTypeInfo_f(const idCmdArgs &args)
{
    ...
    {
        j = index[i];
        common->Printf("%-32s : %-32s %d\n", classTypeInfo[j].typeName, classTypeInfo[j].superType,
                       classTypeInfo[j].size);
    }
}
