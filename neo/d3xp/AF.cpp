...
void idAF::SetupPose(idEntity *ent, int time)
{
    ...
    for (i = 0; i < jointMods.Num(); i++)
    {
        ...
        body->SetWorldOrigin(renderEntity->origin +
                             (origin + jointMods[i].jointBodyOrigin * axis) * renderEntity->axis);
...
int idAF::EntitiesTouchingAF(afTouch_t touchList[MAX_GENTITIES]) const
{
    ...
    for (i = 0; i < jointMods.Num(); i++)
    {
        ...
        for (j = 0; j < numClipModels; j++)
        {
            ...
            if (gameLocal.clip.ContentsModel(body->GetWorldOrigin(), body->GetClipModel(), body->GetWorldAxis(), -1,
                                             cm->Handle(), cm->GetOrigin(), cm->GetAxis()))
            {
...
void idAF::AddBody(idAFBody *body, const idJointMat *joints, const char *jointName, const AFJointModType_t mod)
{
    ...
    {
        gameLocal.Error("idAF for entity '%s' at (%s) modifies unknown joint '%s'", self->name.c_str(),
...
bool idAF::LoadBody(const idDeclAF_Body *fb, const idJointMat *joints)
{
    ...
    for (i = 0; i < jointList.Num(); i++)
    {
        ...
        {
            gameLocal.Warning("%s: joint '%s' is already contained by body '%s'", name.c_str(),
...
bool idAF::Load(idEntity *ent, const char *fileName)
{
    ...
    if (animator == NULL)
    {
        gameLocal.Warning("Couldn't load af '%s' for entity '%s' at (%s): NULL animator\n", name.c_str(),
                          ent->name.c_str(), ent->GetPhysics()->GetOrigin().ToString(0));
        return false;
    }
    ...
    gameEdit->ANIM_CreateAnimFrame(model, animator->GetAnim(modifiedAnim)->MD5Anim(0), numJoints, joints, 1,
    ...
    physicsObj.SetDefaultFriction(file->defaultLinearFriction, file->defaultAngularFriction,
...
bool idAF::TestSolid(void) const
{
    ...
    for (i = 0; i < physicsObj.GetNumBodies(); i++)
    {
        ...
        if (gameLocal.clip.Translation(trace, body->GetWorldOrigin(), body->GetWorldOrigin(), body->GetClipModel(),
                                       body->GetWorldAxis(), body->GetClipMask(), self))
        {
            ...
            gameLocal.DWarning("%s: body '%s' stuck in %d (normal = %.2f %.2f %.2f, depth = %.2f)", self->name.c_str(),
                               body->GetName().c_str(), trace.c.contents, trace.c.normal.x, trace.c.normal.y,
                               trace.c.normal.z, depth);
            solid = true;
        }
    }
    return solid;
...
void idAF::LoadState(const idDict &args)
{
    ...
    while (kv)
    {
        ...
        if (body)
        {
            sscanf(kv->GetValue(), "%f %f %f %f %f %f", &origin.x, &origin.y, &origin.z, &angles.pitch, &angles.yaw,
                   &angles.roll);
            body->SetWorldOrigin(origin);
...
void idAF::AddBindConstraints(void)
{
    ...
    for (kv = args.MatchPrefix("bindConstraint ", NULL); kv; kv = args.MatchPrefix("bindConstraint ", kv))
    {
        ...
        if (!body)
        {
            gameLocal.Warning("idAF::AddBindConstraints: body '%s' not found on entity '%s'", bodyName.c_str(),
                              self->name.c_str());
            lexer.FreeSource();
            continue;
        }
