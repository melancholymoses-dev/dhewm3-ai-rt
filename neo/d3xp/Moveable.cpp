...
void idMoveable::Spawn(void)
{
    ...
    if (health)
    {
        ...
        {
            gameLocal.Error("idMoveable '%s' at (%s): cannot load broken model '%s'", name.c_str(),
                            GetPhysics()->GetOrigin().ToString(0), brokenModel.c_str());
        }
    }
            ...
            if (ent && v > minDamageVelocity)
            {
                f = v > maxDamageVelocity ? 1.0f
                                          : idMath::Sqrt(v - minDamageVelocity) *
                                                (1.0f / idMath::Sqrt(maxDamageVelocity - minDamageVelocity));
                dir = velocity;
                dir.NormalizeFast();
                if (ent->IsType(idAI::Type) && hasMonsterDamage)
                {
                    ...
                    {
                        ent->Damage(this, GetPhysics()->GetClipModel()->GetOwner(), dir, monsterDamage, f,
                                    INVALID_JOINT);
                    }
#else
...
void idExplodingBarrel::AddParticles(const char *name, bool burn)
{
    if (name && *name)
    {
        ...
        const idDeclModelDef *modelDef =
            static_cast<const idDeclModelDef *>(declManager->FindType(DECL_MODELDEF, name));
        if (modelDef)
        {
            particleRenderEntity.origin = physicsObj.GetAbsBounds().GetCenter();
            particleRenderEntity.axis = mat3_identity;
...
================
*/
void idExplodingBarrel::Damage(idEntity *inflictor, idEntity *attacker, const idVec3 &dir, const char *damageDefName,
