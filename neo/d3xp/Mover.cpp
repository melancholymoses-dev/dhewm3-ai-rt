...
void idMover::Event_UpdateMove(void)
{
    ...
    {
    case ACCELERATION_STAGE: {
        physicsObj.SetLinearExtrapolation(EXTRAPOLATION_ACCELLINEAR, gameLocal.slow.time, move.acceleration, org,
    ...
    case LINEAR_STAGE: {
        physicsObj.SetLinearExtrapolation(EXTRAPOLATION_LINEAR, gameLocal.slow.time, move.movetime, org, move.dir,
                                          vec3_origin);
    ...
    case DECELERATION_STAGE: {
        physicsObj.SetLinearExtrapolation(EXTRAPOLATION_DECELLINEAR, gameLocal.slow.time, move.deceleration, org,
                                          move.dir, vec3_origin);
        move.stage = FINISHED_STAGE;
        break;
    }
...
void idMover::Event_UpdateRotation(void)
{
    ...
    {
    case ACCELERATION_STAGE: {
        physicsObj.SetAngularExtrapolation(EXTRAPOLATION_ACCELLINEAR, gameLocal.slow.time, rot.acceleration, ang,
                                           rot.rot, ang_zero);
        if (rot.movetime > 0)
        {
    ...
    case LINEAR_STAGE: {
        ...
        {
            physicsObj.SetAngularExtrapolation(EXTRAPOLATION_LINEAR, gameLocal.slow.time, rot.movetime, ang, rot.rot,
                                               ang_zero);
        }
    ...
    case DECELERATION_STAGE: {
        physicsObj.SetAngularExtrapolation(EXTRAPOLATION_DECELLINEAR, gameLocal.slow.time, rot.deceleration, ang,
                                           rot.rot, ang_zero);
        rot.stage = FINISHED_STAGE;
        break;
    }
...
void idMover::Event_TeamBlocked(idEntity *blockedEntity, idEntity *blockingEntity)
{
    if (g_debugMover.GetBool())
    {
        gameLocal.Printf("%d: '%s' stopped due to team member '%s' blocked by '%s'\n", gameLocal.slow.time,
                         name.c_str(), blockedEntity->name.c_str(), blockingEntity->name.c_str());
    }
}
...
void idRotater::Spawn(void)
{
    ...
    physicsObj.SetLinearExtrapolation(EXTRAPOLATION_NONE, gameLocal.slow.time, 0, GetPhysics()->GetOrigin(),
                                      vec3_origin, vec3_origin);
    physicsObj.SetAngularExtrapolation(extrapolation_t(EXTRAPOLATION_LINEAR | EXTRAPOLATION_NOSTOP),
                                       gameLocal.slow.time, 0, GetPhysics()->GetAxis().ToAngles(), ang_zero, ang_zero);
    SetPhysics(&physicsObj);
...
void idRiser::Event_Activate(idEntity *activator)
{
    ...
    else
    {
        ...
        physicsObj.SetLinearExtrapolation(EXTRAPOLATION_LINEAR, gameLocal.slow.time, time * 1000,
                                          physicsObj.GetOrigin(), delta, vec3_origin);
    }
}
