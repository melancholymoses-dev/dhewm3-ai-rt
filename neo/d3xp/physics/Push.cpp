...
============
*/
float idPush::ClipTranslationalPush(trace_t &results, idEntity *pusher, const int flags, const idVec3 &newOrigin,
                                    const idVec3 &translation)
{
    ...
    if (flags & PUSHFL_CLIP)
    {
        ...
        numListedEntities =
            GetPushableEntitiesForTranslation(pusher, pusher, flags, translation, entityList, MAX_GENTITIES);
    ...
    for (i = 0; i < pushedGroupSize;)
    {
        ...
        numListedEntities =
            GetPushableEntitiesForTranslation(curPusher, pusher, flags, realTranslation, entityList, MAX_GENTITIES);
...
============
*/
float idPush::ClipRotationalPush(trace_t &results, idEntity *pusher, const int flags, const idMat3 &newAxis,
                                 const idRotation &rotation)
{
    ...
    for (i = 0; i < pushedGroupSize;)
    {
        ...
        numListedEntities =
            GetPushableEntitiesForRotation(curPusher, pusher, flags, realRotation, entityList, MAX_GENTITIES);
...
============
*/
void idPush::ClipEntityRotation(trace_t &trace, const idEntity *ent, const idClipModel *clipModel, idClipModel *skip,
                                const idRotation &rotation)
{
...
============
*/
void idPush::ClipEntityTranslation(trace_t &trace, const idEntity *ent, const idClipModel *clipModel, idClipModel *skip,
                                   const idVec3 &translation)
{
                                ...
                                const idMat3 &newAxis, const idRotation &rotation)
{
    ...
    else
    {
        ...
        if (results.fraction >= 1.0f)
        {
            ...
            clipModel->Link(gameLocal.clip, clipModel->GetEntity(), clipModel->GetId(), clipModel->GetOrigin(),
                            newAxis);
            if (physics->ClipContents(clipModel))
            {
                if (!startsolid)
                                    ...
                                    const idVec3 &translation)
{
    ...
    if (flags & PUSHFL_CLIP)
    {
        ...
        gameLocal.clip.Translation(results, clipModel->GetOrigin(), clipModel->GetOrigin() + translation, clipModel,
                                   clipModel->GetAxis(), pusher->GetPhysics()->GetClipMask(), NULL);
    ...
    for (i = 0; i < listedEntities; i++)
    {
        ...
        {
            // set the pusher in the translated position
            clipModel->Link(gameLocal.clip, clipModel->GetEntity(), clipModel->GetId(), newOrigin,
                            clipModel->GetAxis());
            // the entity might be pushed off the ground
            physics->EvaluateContacts();
            // put pusher back in old position
            clipModel->Link(gameLocal.clip, clipModel->GetEntity(), clipModel->GetId(), oldOrigin,
                            clipModel->GetAxis());
        ...
        {
            check->Damage(clipModel->GetEntity(), clipModel->GetEntity(), vec3_origin, "damage_crush", 1.0f,
                                 ...
                                 const idRotation &rotation)
{
    ...
    if (flags & PUSHFL_CLIP)
    {
        ...
        gameLocal.clip.Rotation(results, clipModel->GetOrigin(), rotation, clipModel, clipModel->GetAxis(),
...
============
*/
float idPush::ClipPush(trace_t &results, idEntity *pusher, const int flags, const idVec3 &oldOrigin,
                       const idMat3 &oldAxis, idVec3 &newOrigin, idMat3 &newAxis)
{
    idVec3 translation;
    idRotation rotation;
    float mass;
