...
void idPlayerView::DamageImpulse(idVec3 localKickDir, const idDict *damageDef)
{
    if (g_hitEffect.GetBool())
    {
        ...
        if (blobTime)
        {
            ...
            blob->finishTime =
                gameLocal.slow.time + blobTime * g_blobTime.GetFloat() * ((float)gameLocal.msec / USERCMD_MSEC);
...
void idPlayerView::CalculateShake()
{
    ...
    float shakeVolume =
        gameSoundWorld->CurrentShakeAmplitudeForPosition(gameLocal.slow.time, player->firstPersonViewOrigin);
    //
...
void idPlayerView::SingleView(idUserInterface *hud, const renderView_t *view)
{
    ...
    gameSoundWorld->PlaceListener(view->vieworg, view->viewaxis, player->entityNumber + 1, gameLocal.slow.time,
    ...
    if (!pm_thirdPerson.GetBool() && !g_skipViewEffects.GetBool())
    {
        for (int i = 0; i < MAX_SCREEN_BLOBS; i++)
        {
            ...
            if (fade)
            {
                ...
                renderSystem->DrawStretchPic(blob->x, blob->y, blob->w, blob->h, blob->s1, blob->t1, blob->s2, blob->t2,
                                             blob->material);
            }
        }
        player->DrawHUD(hud);
        ...
        if (alpha < 1.0f)
        {
            renderSystem->SetColor4((player->health <= 0.0f) ? MS2SEC(gameLocal.slow.time) : lastDamageTime, 1.0f, 1.0f,
                                    (player->health <= 0.0f) ? 0.0f : alpha);
...
void FullscreenFX_Helltime::AccumPass(const renderView_t *view)
{
    ...
    else
    {
        ...
        renderSystem->DrawStretchPic(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, 0, shiftScale.y, shiftScale.x, 0,
...
void FullscreenFX_DoubleVision::HighQuality()
{
    ...
    if (!gameLocal.isMultiplayer && (gameLocal.fast.time < player->inventory.powerupEndTime[HELLTIME] ||
    ...
    renderSystem->DrawStretchPic(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, 0, shiftScale.y, (1 - shift) * shiftScale.x, 0,
                                 material);
}
...
void FullscreenFX_InfluenceVision::HighQuality()
{
    ...
    {
        distance =
            (player->GetInfluenceEntity()->GetPhysics()->GetOrigin() - player->GetPhysics()->GetOrigin()).Length();
    ...
    if (player->GetInfluenceMaterial())
    {
        ...
        renderSystem->DrawStretchPic(0.0f, 0.0f, 640.0f, 480.0f, 0.0f, 0.0f, 1.0f, 1.0f,
...
void FullscreenFXManager::Blendback(float alpha)
{
    ...
    if (alpha < 1.f)
    {
        ...
        renderSystem->DrawStretchPic(0.0f, 0.0f, 640.0f, 480.0f, 0.0f, shiftScale.y, shiftScale.x, 0.f,
                                     blendBackMaterial);
    }
}
