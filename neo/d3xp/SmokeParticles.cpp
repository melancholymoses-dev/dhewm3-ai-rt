...
================
*/
bool idSmokeParticles::EmitSmoke(const idDeclParticle *smoke, const int systemStartTime, const float diversity,
                                 const idVec3 &origin, const idMat3 &axis, int timeGroup /*_D3XP*/)
{
    ...
    for (int stageNum = 0; stageNum < smoke->stages.Num(); stageNum++)
    {
        ...
        else
        {
            ...
            prevCount =
                floor(((float)(deltaMsec - gameLocal.msec /*_D3XP - FIX - was USERCMD_MSEC*/) / finalParticleTime) *
        ...
        for (prevCount++; prevCount <= nowCount; prevCount++)
        {
            if (!freeSmokes)
            {
                gameLocal.Printf("idSmokeParticles::EmitSmoke: no free smokes with %d "
                                 "active stages\n",
                                 activeStages.Num());
                return true;
            }
