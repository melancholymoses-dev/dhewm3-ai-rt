...
bool idMD5Anim::LoadAnim(const char *filename)
{
    ...
    for (i = 0; i < numJoints; i++)
    {
        ...
        if ((numAnimatedComponents > 0) &&
            ((jointInfo[i].firstComponent < 0) || (jointInfo[i].firstComponent >= numAnimatedComponents)))
        {
...
void idMD5Anim::GetBounds(idBounds &bnds, int time, int cyclecount) const
{
    ...
    if (jointInfo[0].animBits & (ANIM_TX | ANIM_TY | ANIM_TZ))
    {
        const float *componentPtr1 =
            &componentFrames[numAnimatedComponents * frame.frame1 + jointInfo[0].firstComponent];
        const float *componentPtr2 =
            &componentFrames[numAnimatedComponents * frame.frame2 + jointInfo[0].firstComponent];
