...
bool idModelExport::CheckMayaInstall(void)
{
    ...
    lres = RegQueryValueEx(hKey, "MAYA_INSTALL_LOCATION", NULL, (unsigned long *)&lType, (unsigned char *)NULL,
                           (unsigned long *)NULL);
...
int idModelExport::ParseExportSection(idParser &parser)
{
    ...
    lex.SetFlags(LEXFL_NOSTRINGCONCAT | LEXFL_ALLOWPATHNAMES | LEXFL_ALLOWMULTICHARLITERALS |
    ...
    while (1)
    {
        ...
        else if ((command == "mesh") || (command == "anim") || (command == "camera"))
        {
            ...
            if (ParseOptions(lex))
            {
                ...
                sprintf(commandLine, "%s %s -dest %s -game %s%s", command.c_str(), src.c_str(), dest.c_str(), game,
                        commandLine.c_str());
                if (ConvertMayaToMD5())
                {
                    count++;
...
{
    idParser parser(LEXFL_NOSTRINGCONCAT | LEXFL_ALLOWPATHNAMES | LEXFL_ALLOWMULTICHARLITERALS |
                    LEXFL_ALLOWBACKSLASHSTRINGCONCAT);
    idToken token;
    int count;
