...
idCVar com_forceGenericSIMD("com_forceGenericSIMD", "0", CVAR_BOOL | CVAR_SYSTEM,
...
const char *idGameLocal::sufaceTypeNames[MAX_SURFACE_TYPES] = {
    "none",    "metal",    "stone",      "flesh",      "wood",       "cardboard",  "liquid",     "glass",
    "plastic", "ricochet", "surftype10", "surftype11", "surftype12", "surftype13", "surftype14", "surftype15"};
...
void idGameLocal::Init(void)
{
    ...
    cmdSystem->AddCommand("listModelDefs", idListDecls_f<DECL_MODELDEF>, CMD_FL_SYSTEM | CMD_FL_GAME,
                          "lists model defs");
    cmdSystem->AddCommand("printModelDefs", idPrintDecls_f<DECL_MODELDEF>, CMD_FL_SYSTEM | CMD_FL_GAME,
    ...
    if (clientNum >= 0 && clientNum < MAX_CLIENTS)
    {
        ...
        if (canModify)
        {
            ...
            for (i = 0; i < numClients; i++)
            {
                ...
                {
                    if (!idStr::Icmp(idGameLocal::userInfo[clientNum].GetString("ui_name"),
                                     idGameLocal::userInfo[i].GetString("ui_name")))
                    {
                        idGameLocal::userInfo[clientNum].Set(
                            "ui_name", va("%s_", idGameLocal::userInfo[clientNum].GetString("ui_name")));
                        modifiedInfo = true;
                        i = -1; // rescan
                        continue;
                    }
...
===================
*/
void idGameLocal::InitFromNewMap(const char *mapName, idRenderWorld *renderWorld, idSoundWorld *soundWorld,
                                 bool isServer, bool isClient, int randseed)
{
...
=================
*/
bool idGameLocal::InitFromSaveGame(const char *mapName, idRenderWorld *renderWorld, idSoundWorld *soundWorld,
                                   idFile *saveGameFile)
{
    int i;
    int num;
    ...
    for (i = 0; i < declManager->GetNumDecls(DECL_SOUND); i++)
    {
        ...
        if (soundShader->EverReferenced() && soundShader->GetState() != DS_DEFAULTED)
        {
            ...
            for (j = 0; j < soundShader->GetNumSounds(); j++)
            {
                ...
                if (soundName.Find("/vo/", false) == -1 && soundName.Find("/combat_chatter/", false) == -1 &&
                    soundName.Find("/bfgcarnage/", false) == -1 && soundName.Find("/enpro/", false) == -1 &&
                    soundName.Find("/soulcube/energize_01.wav", false) == -1)
                {
                    // don't OGG weapon sounds
                    if (soundName.Find("weapon", false) != -1 || soundName.Find("gun", false) != -1 ||
                        soundName.Find("bullet", false) != -1 || soundName.Find("bfg", false) != -1 ||
...
void idGameLocal::CacheDictionaryMedia(const idDict *dict)
{
    ...
    while (kv)
    {
        if (kv->GetValue().Length())
        {
            if (!idStr::Icmp(kv->GetKey(), "gui_noninteractive") || !idStr::Icmpn(kv->GetKey(), "gui_parm", 8) ||
...
pvsHandle_t idGameLocal::GetClientPVS(idPlayer *player, pvsType_t type)
{
    ...
    {
        return pvs.SetupCurrentPVS(player->GetPrivateCameraView()->GetPVSAreas(),
        ...
        do
        {
            ...
            if (g_frametime.GetBool())
            {
                Printf("game %d: all:%u th:%u ev:%u %d ents \n", time,
                       timer_think.Milliseconds() + timer_events.Milliseconds(), timer_think.Milliseconds(),
                       timer_events.Milliseconds(), num);
            }
...
void idGameLocal::ShowTargets(void)
{
    ...
    for (ent = spawnedEntities.Next(); ent != NULL; ent = ent->spawnNode.Next())
    {
        ...
        gameRenderWorld->DebugBounds((ent->IsHidden() ? colorLtGrey : colorOrange) * frac,
        ...
        for (i = 0; i < ent->targets.Num(); i++)
        {
            ...
            if (target)
            {
                gameRenderWorld->DebugArrow(colorYellow * frac, ent->GetPhysics()->GetAbsBounds().GetCenter(),
...
void idGameLocal::RunDebugInfo(void)
{
    ...
    if (g_showEntityInfo.GetBool())
    {
        ...
        for (ent = spawnedEntities.Next(); ent != NULL; ent = ent->spawnNode.Next())
        {
            ...
            if (viewTextBounds.IntersectsBounds(entBounds))
            {
                ...
                gameRenderWorld->DrawText(va("#%d", ent->entityNumber), entBounds.GetCenter() + up, 0.1f, colorWhite,
                                          axis, 1);
            }
        }
    }
    ...
    if (ai_showObstacleAvoidance.GetInteger() == 2)
    {
        ...
        if (aas)
        {
            ...
            idAI::FindPathAroundObstacles(player->GetPhysics(), aas, NULL, player->GetPhysics()->GetOrigin(), seekPos,
                                          path);
        }
    }
...
============
*/
void idGameLocal::RadiusDamage(const idVec3 &origin, idEntity *inflictor, idEntity *attacker, idEntity *ignoreDamage,
                               idEntity *ignorePush, const char *damageDefName, float dmgPower)
{
    ...
    for (e = 0; e < numListedEntities; e++)
    {
        ...
        if (ent == inflictor ||
            (ent->IsType(idAFAttachment::Type) && static_cast<idAFAttachment *>(ent)->GetBody() == inflictor))
        {
            continue;
        }

        if (ent == ignoreDamage ||
            (ent->IsType(idAFAttachment::Type) && static_cast<idAFAttachment *>(ent)->GetBody() == ignoreDamage))
        {
            continue;
        }

        // don't damage a dead player
        if (isMultiplayer && ent->entityNumber < MAX_CLIENTS && ent->IsType(idPlayer::Type) &&
        ...
        if (ent->CanDamage(origin, damagePoint))
        {
            ...
            if (ent == attacker ||
                (ent->IsType(idAFAttachment::Type) && static_cast<idAFAttachment *>(ent)->GetBody() == attacker))
            {
...
==============
*/
void idGameLocal::RadiusPush(const idVec3 &origin, const float radius, const float push, const idEntity *inflictor,
                             const idEntity *ignore, float inflictorScale, const bool quake)
{
    ...
    for (i = 0; i < numListedClipModels; i++)
    {
        ...
        if (ent == ignore ||
            (ent->IsType(idAFAttachment::Type) && static_cast<idAFAttachment *>(ent)->GetBody() == ignore))
        {
            continue;
        }
                               ...
                               const char *material, float angle)
{
    ...
    static idVec3 decalWinding[4] = {idVec3(1.0f, 1.0f, 0.0f), idVec3(-1.0f, 1.0f, 0.0f), idVec3(-1.0f, -1.0f, 0.0f),
    ...
    gameRenderWorld->ProjectDecalOntoWorld(winding, projectionOrigin, parallel, depth * 0.5f,
...
void idGameLocal::BloodSplat(const idVec3 &origin, const idVec3 &dir, float size, const char *material)
{
    ...
    idVec3 verts[] = {idVec3(0.0f, +halfSize, +halfSize), idVec3(0.0f, +halfSize, -halfSize),
...
idEntity *idGameLocal::SelectInitialSpawnPoint(idPlayer *player)
{
    ...
    if (!isMultiplayer || !spawnSpots.Num() ||
        (mpGame.IsGametypeFlagBased() && (!teamSpawnSpots[0].Num() || !teamSpawnSpots[1].Num())))
    { /* CTF */
#else
    if (!isMultiplayer || !spawnSpots.Num())
    {
    ...
    else
    {
        ...
        if (mpGame.IsGametypeFlagBased()) /* CTF */
        {
            ...
            for (i = 0; i < teamSpawnSpots[team].Num(); i++)
            {
                ...
                for (j = 0; j < MAX_CLIENTS; j++)
                {
                    if (!entities[j] || !entities[j]->IsType(idPlayer::Type) || entities[j] == player ||
            ...
            // sort the list
            qsort((void *)teamSpawnSpots[team].Ptr(), teamSpawnSpots[team].Num(), sizeof(spawnSpot_t),
