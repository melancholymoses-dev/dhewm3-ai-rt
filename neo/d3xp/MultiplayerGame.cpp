...
const char *idMultiplayerGame::GlobalSoundStrings[] = {
    "sound/feedback/voc_youwin.wav",    "sound/feedback/voc_youlose.wav",   "sound/feedback/fight.wav",
    "sound/feedback/vote_now.wav",      "sound/feedback/vote_passed.wav",   "sound/feedback/vote_failed.wav",
    "sound/feedback/three.wav",         "sound/feedback/two.wav",           "sound/feedback/one.wav",
    "sound/feedback/sudden_death.wav",
#ifdef CTF
    "sound/ctf/flag_capped_yours.wav",  "sound/ctf/flag_capped_theirs.wav", "sound/ctf/flag_return.wav",
    "sound/ctf/flag_taken_yours.wav",   "sound/ctf/flag_taken_theirs.wav",  "sound/ctf/flag_dropped_yours.wav",
    "sound/ctf/flag_dropped_theirs.wav"
#endif
};

// handy verbose
const char *idMultiplayerGame::GameStateStrings[] = {"INACTIVE",    "WARMUP",     "COUNTDOWN", "GAMEON",
                                                     "SUDDENDEATH", "GAMEREVIEW", "NEXTGAME"};

const char *idMultiplayerGame::MPGuis[] = {"guis/mphud.gui", "guis/mpmain.gui", "guis/mpmsgmode.gui",
                                           "guis/netmenu.gui", NULL};

const char *idMultiplayerGame::ThrottleVars[] = {"ui_spectate", "ui_ready", "ui_team", NULL};

const char *idMultiplayerGame::ThrottleVarsInEnglish[] = {"#str_06738", "#str_06737", "#str_01991", NULL};
        ...
        for (j = 0; j < numRankedPlayers; j++)
        {
            ...
            { /* CTF */
                if (player->team != players[j]->team)
                {
                    ...
                    else if (playerState[i].teamFragCount == playerState[players[j]->entityNumber].teamFragCount &&
...
void idMultiplayerGame::UpdateScoreboard(idUserInterface *scoreBoard, idPlayer *player)
{
    ...
    scoreBoard->SetStateString("scoretext", gameLocal.gameType == GAME_LASTMAN
        ...
        for (i = 0; i < MAX_CLIENTS; i++)
        {
            ...
            else
            {
                ...
                if (gameState == WARMUP)
                {
                    ...
                    {
                        scoreBoard->SetStateString(va("player%i_score", iline),
                                                   p->IsReady() ? common->GetLanguageDict()->GetString("#str_04247")
    ...
    gameinfo =
        va("%s: %s", common->GetLanguageDict()->GetString("#str_02376"), gameLocal.serverInfo.GetString("si_gameType"));
    if (gameLocal.gameType == GAME_LASTMAN)
    {
        ...
        {
            livesinfo = va("%s: %i", common->GetLanguageDict()->GetString("#str_04264"),
...
void idMultiplayerGame::UpdateCTFScoreboard(idUserInterface *scoreBoard, idPlayer *player)
{
    ...
    if (gameState != WARMUP)
    {
        for (i = 0; i < numRankedPlayers; i++)
        {
            ...
            /* Player Name */
            scoreBoard->SetStateString(va("player%i_%s", ilines[player->team], curTeam),
            ...
            /* Ping */
            scoreBoard->SetStateInt(va("player%i_%s_ping", ilines[player->team], curTeam),
...
idPlayer *idMultiplayerGame::FragLeader(void)
{
    ...
    for (i = 0; i < gameLocal.numClients; i++)
    {
        ...
        if (gameLocal.gameType == GAME_TOURNEY && ent->entityNumber != currentTourneyPlayer[0] &&
...
void idMultiplayerGame::PlayerStats(int clientNum, char *data, const int len)
{
    ...
    *data = 0;
    ...
    idStr::snPrintf(data, len, "team=%d score=%d tks=%d", team, playerState[clientNum].fragCount,
...
idUserInterface *idMultiplayerGame::StartMenu(void)
{
    ...
    if (currentMenu == 1)
    {
        ...
        const char *map =
            gameLocal.serverInfo.GetString("si_map"); // what if server changes this strings while user in UI?
        int num = declManager->GetNumDecls(DECL_MAPDEF);

        for (i = 0; i < num; i++)
        {
            const idDeclEntityDef *mapDef =
                static_cast<const idDeclEntityDef *>(declManager->DeclByIndex(DECL_MAPDEF, i));
...
bool idMultiplayerGame::Draw(int clientNum)
{
    ...
    else
    {
        ...
        if (player->spectating)
        {
            ...
            {
                spectatetext[ispecline] = va(common->GetLanguageDict()->GetString("#str_07008"),
            ...
            if (vote != VOTE_NONE)
            {
                spectateGui->SetStateString("vote",
                                            va("%s (y: %d n: %d)", voteString.c_str(), (int)yesVotes, (int)noVotes));
            }
            else
            {
                spectateGui->SetStateString("vote", "");
...
void idMultiplayerGame::UpdateHud(idPlayer *player, idUserInterface *hud)
{
    ...
    hud->SetStateString("timer", (Warmup())                   ? common->GetLanguageDict()->GetString("#str_04251")
                                 : (gameState == SUDDENDEATH) ? common->GetLanguageDict()->GetString("#str_04252")
                                                              : GameTime());
    if (vote != VOTE_NONE)
    {
...
void idMultiplayerGame::PrintMessageEvent(int to, msg_evt_t evt, int parm1, int parm2)
{
    ...
    case MSG_KILLED:
        ...
        AddChatLine(common->GetLanguageDict()->GetString("#str_04292"), gameLocal.userInfo[parm1].GetString("ui_name"),
                    gameLocal.userInfo[parm2].GetString("ui_name"));
        break;
    case MSG_KILLEDTEAM:
        assert(parm1 >= 0 && parm2 >= 0);
        AddChatLine(common->GetLanguageDict()->GetString("#str_04291"), gameLocal.userInfo[parm1].GetString("ui_name"),
                    gameLocal.userInfo[parm2].GetString("ui_name"));
        break;
    case MSG_TELEFRAGGED:
        assert(parm1 >= 0 && parm2 >= 0);
        AddChatLine(common->GetLanguageDict()->GetString("#str_04290"), gameLocal.userInfo[parm1].GetString("ui_name"),
                    gameLocal.userInfo[parm2].GetString("ui_name"));
        break;
    case MSG_DIED:
        assert(parm1 >= 0);
    ...
    case MSG_FRAGLIMIT:
        ...
        break;
    case MSG_JOINTEAM:
        AddChatLine(common->GetLanguageDict()->GetString("#str_04280"), gameLocal.userInfo[parm1].GetString("ui_name"),
    ...
    case MSG_FLAGTAKEN:
        ...
        {
            AddChatLine(common->GetLanguageDict()->GetString("#str_11101"),
    ...
    case MSG_FLAGRETURN:
        ...
        if (parm2 >= 0 && parm2 < MAX_CLIENTS)
        {
            if (gameLocal.GetLocalPlayer()->team != parm1)
            {
                AddChatLine(common->GetLanguageDict()->GetString("#str_11120"),
                            gameLocal.userInfo[parm2].GetString("ui_name")); // your team
            }
            else
            {
    ...
    case MSG_FLAGCAPTURE:
        ...
        if (gameLocal.GetLocalPlayer()->team != parm1)
        {
            AddChatLine(common->GetLanguageDict()->GetString("#str_11122"),
                        gameLocal.userInfo[parm2].GetString("ui_name")); // your team
        }
        else
        {
...
void idMultiplayerGame::ServerCallVote(int clientNum, const idBitMsg &msg)
{
    ...
    case VOTE_FRAGLIMIT:
        ...
        ClientStartVote(clientNum,
                        va(common->GetLanguageDict()->GetString("#str_04303"),
                           gameLocal.gameType == GAME_LASTMAN ? common->GetLanguageDict()->GetString("#str_04264")
    ...
    case VOTE_KICK:
        ...
        ClientStartVote(clientNum, va(common->GetLanguageDict()->GetString("#str_04302"), vote_clientNum,
    ...
    case VOTE_MAP: {
        ...
        ServerStartVote(clientNum, voteIndex, value);
        ClientStartVote(clientNum, va(common->GetLanguageDict()->GetString("#str_04256"),
                                      common->GetLanguageDict()->GetString(dict ? dict->GetString("name") : value)));
        break;
    }
    case VOTE_SPECTATORS:
    ...
    default:
        gameLocal.ServerSendChatMessage(clientNum, "server",
                                        va(common->GetLanguageDict()->GetString("#str_04297"), (int)voteIndex));
    ...
    if (gameLocal.gameType == GAME_TOURNEY && gameState == WARMUP)
    {
        ...
        for (i = 0; i < 2; i++)
        {
            if (!gameLocal.entities[currentTourneyPlayer[i]] ||
                static_cast<idPlayer *>(gameLocal.entities[currentTourneyPlayer[i]])->spectating)
            {
...
void idMultiplayerGame::MapRestart(void)
{
    ...
    if (g_balanceTDM.GetBool() && lastGameType != GAME_TDM && lastGameType != GAME_CTF &&
        gameLocal.mpGame.IsGametypeTeamBased())
    {
#else
...
void idMultiplayerGame::ThrottleUserInfo(void)
{
    ...
    while (ThrottleVars[i])
    {
                        ...
                        cvarSystem->GetCVarString(ThrottleVars[i])))
        {
            ...
            {
                AddChatLine(common->GetLanguageDict()->GetString("#str_04299"),
                            common->GetLanguageDict()->GetString(ThrottleVarsInEnglish[i]),
                            (switchThrottle[i] - gameLocal.time) / 1000 + 1);
                cvarSystem->SetCVarString(ThrottleVars[i],
                                          gameLocal.userInfo[gameLocal.localClientNum].GetString(ThrottleVars[i]));
            }
            else
            {
...
void idMultiplayerGame::EnterGame(int clientNum)
{
    ...
    if (!playerState[clientNum].ingame)
    {
        ...
        if (gameLocal.isMultiplayer)
        {
            ...
            gameLocal.ServerSendChatMessage(-1, common->GetLanguageDict()->GetString("#str_02047"),
