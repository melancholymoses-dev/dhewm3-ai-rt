...
void idIK::Restore(idRestoreGame *savefile)
{
    ...
    if (self)
    {
        ...
        {
            gameLocal.Warning("idIK::Restore: IK for entity '%s' at (%s) has no model set.", self->name.c_str(),
                              self->GetPhysics()->GetOrigin().ToString(0));
        }
        modifiedAnim = animator->GetAnim(anim);
        if (modifiedAnim == 0)
        {
            gameLocal.Warning("idIK::Restore: IK for entity '%s' at (%s) has no "
                              "modified animation.",
...
bool idIK::Init(idEntity *self, const char *anim, const idVec3 &modelOffset)
{
    ...
    if (animator == NULL || animator->ModelDef() == NULL)
    {
        gameLocal.Warning("idIK::Init: IK for entity '%s' at (%s) has no model set.", self->name.c_str(),
                          self->GetPhysics()->GetOrigin().ToString(0));
        return false;
    }
    if (animator->ModelDef()->ModelHandle() == NULL)
    {
        gameLocal.Warning("idIK::Init: IK for entity '%s' at (%s) uses default model.", self->name.c_str(),
                          self->GetPhysics()->GetOrigin().ToString(0));
        return false;
    }
    ...
    if (modifiedAnim == 0)
    {
        gameLocal.Warning("idIK::Init: IK for entity '%s' at (%s) has no modified animation.", self->name.c_str(),
                          self->GetPhysics()->GetOrigin().ToString(0));
        return false;
    }
...
================
*/
bool idIK::SolveTwoBones(const idVec3 &startPos, const idVec3 &endPos, const idVec3 &dir, float len0, float len1,
                         idVec3 &jointPos)
{
...
bool idIK_Walk::Init(idEntity *self, const char *anim, const idVec3 &modelOffset)
{
    ...
    static idVec3 footWinding[4] = {idVec3(1.0f, 1.0f, 0.0f), idVec3(-1.0f, 1.0f, 0.0f), idVec3(-1.0f, -1.0f, 0.0f),
    ...
    gameEdit->ANIM_CreateAnimFrame(animator->ModelHandle(), animator->GetAnim(modifiedAnim)->MD5Anim(0), numJoints,
                                   joints, 1, animator->ModelDef()->GetVisualOffset() + modelOffset,
...
void idIK_Walk::Evaluate(void)
{
    ...
    for (i = 0; i < numLegs; i++)
    {
        ...
        gameLocal.clip.Translation(results, start, end, footModel, mat3_identity, CONTENTS_SOLID | CONTENTS_IKCLIP,
                                   self);
    ...
    animator->SetJointPos(waistJoint, JOINTMOD_WORLD_OVERRIDE,
