    ...
    for (int i = 0; i < particleCount; i++)
    {
        ...
        if (newParticle.smoke)
        {
            newParticle.particle =
                static_cast<const idDeclParticle *>(declManager->FindType(DECL_PARTICLE, particlename, false));
        }
        else
        {
...
void idWeapon::GetWeaponDef(const char *objectname, int ammoinclip)
{
    ...
    if (projectileName[0] != '\0')
    {
        ...
        else
        {
            ...
            {
                gameLocal.Warning("Invalid spawnclass '%s' on projectile '%s' (used by weapon '%s')", spawnclass,
    ...
    if (!gameLocal.isMultiplayer)
    {
        ...
        while (pkv)
        {
            ...
            {
                newParticle.particle =
                    static_cast<const idDeclParticle *>(declManager->FindType(DECL_PARTICLE, particle, false));
            }
            else
            {
                idDict args;
...
bool idWeapon::GetGlobalJointTransform(bool viewModel, const jointHandle_t jointHandle, idVec3 &offset, idMat3 &axis)
{
    ...
    else
    {
        // world model
        if (worldModel.GetEntity() &&
            worldModel.GetEntity()->GetAnimator()->GetJointTransform(jointHandle, gameLocal.time, offset, axis))
        {
...
idEntity *idWeapon::DropItem(const idVec3 &velocity, int activateDelay, int removeDelay, bool died)
{
    ...
    return idMoveableItem::DropItem(classname, worldModel.GetEntity()->GetPhysics()->GetOrigin(),
...
void idWeapon::AlertMonsters(void)
{
    ...
    gameLocal.clip.TracePoint(tr, muzzleFlash.origin, end,
                              CONTENTS_OPAQUE | MASK_SHOT_RENDERMODEL | CONTENTS_FLASHLIGHT_TRIGGER, owner);
    if (g_debugWeapon.GetBool())
    {
...
void idWeapon::PresentWeapon(bool showViewModel)
{
    ...
    if (showViewModel && !disabled && weaponSmoke && (weaponSmokeStartTime != 0))
    {
        ...
        // spit out a particle
        if (!gameLocal.smokeParticles->EmitSmoke(weaponSmoke, weaponSmokeStartTime, gameLocal.random.RandomFloat(),
    ...
    {
        // spit out a particle
        if (!gameLocal.smokeParticles->EmitSmoke(strikeSmoke, strikeSmokeStartTime, gameLocal.random.RandomFloat(),
        ...
        for (int i = 0; i < weaponParticles.Num(); i++)
        {
            ...
            if (part->active)
            {
                if (part->smoke)
                {
                    ...
                    if (!gameLocal.smokeParticles->EmitSmoke(part->particle, part->startTime,
...
bool idWeapon::ClientReceiveEvent(int event, int time, const idBitMsg &msg)
{
    ...
    case EVENT_CHANGESKIN: {
        ...
        renderEntity.customSkin =
            (index != -1) ? static_cast<const idDeclSkin *>(declManager->DeclByIndex(DECL_SKIN, index)) : NULL;
        UpdateVisuals();
        if (worldModel.GetEntity())
        {
...
void idWeapon::Event_PlayAnim(int channel, const char *animname)
{
    ...
    else
    {
        ...
        if (worldModel.GetEntity())
        {
            ...
            if (anim)
            {
                worldModel.GetEntity()->GetAnimator()->PlayAnim(channel, anim, gameLocal.time,
...
================
*/
void idWeapon::Event_LaunchProjectiles(int num_projectiles, float spread, float fuseOffset, float launchPower,
                                       float dmgPower)
{
    idProjectile *proj;
    idEntity *ent;
    ...
    if (gameLocal.isClient)
    {
        ...
        if (projectileDict.GetBool("net_instanthit"))
        {
            ...
            for (i = 0; i < num_projectiles; i++)
            {
                ...
                dir.Normalize();
                gameLocal.clip.Translation(tr, muzzle_pos, muzzle_pos + dir * 4096.0f, NULL, mat3_identity,
    ...
    else
    {
        ...
        for (i = 0; i < num_projectiles; i++)
        {
            ...
            if (i == 0)
            {
                ...
                if (boundsSubLegal &&
                    (ownerBounds - projBounds).RayIntersection(muzzle_pos, playerViewAxis[0], distance))
                {
                ...
                gameLocal.clip.Translation(tr, start, muzzle_pos, proj->GetPhysics()->GetClipModel(),
                                           proj->GetPhysics()->GetClipModel()->GetAxis(), MASK_SHOT_RENDERMODEL, owner);
...
================
*/
void idWeapon::Event_LaunchProjectilesEllipse(int num_projectiles, float spreada, float spreadb, float fuseOffset,
                                              float power)
{
    idProjectile *proj;
    idEntity *ent;
...
void idWeapon::Event_Melee(void)
{
    ...
    if (!gameLocal.isClient)
    {
        ...
        if (g_debugWeapon.GetBool())
        {
            ...
            if (ent)
            {
                gameRenderWorld->DebugBounds(colorRed, ent->GetPhysics()->GetBounds(), ent->GetPhysics()->GetOrigin(),
                                             100);
            }
        }
...
void idWeapon::Event_EjectBrass(void)
{
    ...
    if (!ent || !ent->IsType(idDebris::Type))
    {
        gameLocal.Error("'%s' is not an idDebris",
                        weaponDef ? weaponDef->dict.GetString("def_ejectBrass") : "def_ejectBrass");
    }
    ...
    angular_velocity.Set(10 * gameLocal.random.CRandomFloat(), 10 * gameLocal.random.CRandomFloat(),
