...
| 2 | **Froxel volumetrics + probe GI** — move vol/GI sampling out of screen space into world-space caches; deletes most of the GI noise-fighting chain structurally. | `20260906_froxel_probe_gi.md` | 🟡 **Part A F0-F2 landed 2026-09-13, in-game validated** — vol 1.63 → 0.29 ms median (5.6×), and visually better. F3 dropped (payoff gone); F4/F5 open. **Part B G0/G1 landed 2026-09-13, in-game validated** — probes trace and converge, per-pixel GI still the default (`r_rtGIProbes 0`). Next: G2 resolve switch. |
